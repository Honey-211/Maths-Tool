...
draw.questionLayout = {
  ...
  fixTableWidth: function (obj) {
    ...
    for (var p = 0; p < draw.path.length; p++) {
      ...
      if (
        !un(path.trigger) &&
        path.trigger[0] === false &&
        path.obj.length === 1 &&
        path.obj[0].type === "table2" &&
        path.obj[0].widths.length === obj.widths.length &&
        path.obj[0].heights.length === obj.heights.length &&
        path._center[0] > obj.left &&
        path._center[0] < obj.left + obj.width &&
        path._center[1] > obj.top &&
        path._center[1] < obj.top + obj.height
      ) {
        ansTable = path.obj[0];
        continue;
      }
  ...
  fixTextUnderlining: function (textObj) {
    ...
    for (var p = 0; p < draw.path.length; p++) {
      ...
      for (var o = 0; o < path.obj.length; o++) {
        ...
        if (
          obj.startPos[0] < rect[0] ||
          obj.startPos[0] > rect[0] + rect[2] ||
          obj.startPos[1] < rect[1] ||
          obj.startPos[1] > rect[1] + rect[3]
        )
          continue;
        if (
          obj.finPos[0] < rect[0] ||
          obj.finPos[0] > rect[0] + rect[2] ||
          obj.finPos[1] < rect[1] + 0.5 * rect[3] ||
          obj.finPos[1] > rect[1] + rect[3]
        )
          continue;
        var line = true;
        path.obj.splice(o, 1);
  ...
  splitTextByLines: function (obj) {
    if (!un(obj.box) && (obj.box.type === "loose" || obj.box.type === "tight"))
      return;
  ...
  textToTable: function (obj) {
    ...
    for (var t = 0; t < obj.text.length; t++) {
      ...
      if (typeof element !== "string") {
        ...
        for (var s = 0; s < splitLines.length; s++) {
          ...
          while (splitLine.indexOf(br) === 0) splitLine = splitLine.slice(1);
          lines.last().push(splitLine);
          if (splitLines.length > 1 && s < splitLines.length - 1)
            lines.push([""]);
        }
      }
    }
    ...
    for (var r = 0; r < rows; r++)
      while (cells[r].length < cols) cells[r].push({ text: [""] });
    var heights = [];
  ...
  add: function () {
    ...
    for (var q = 0; q < questions.length; q++) {
      ...
      question.paths = draw.questionLayout.getPagePathsInRect([
        20,
        question.top,
        1160,
        question.height,
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var q = 0; q < obj.questions.length; q++) {
      ...
      if (question.height > question.minHeight) {
        ctx.fillRect(
          rect[0],
          question.top + question.minHeight,
          rect[2],
          question.height - question.minHeight
        );
      }
    }
    ctx.lineWidth = 4;
  ...
  getPageQuestionTextPaths: function () {
    ...
    return paths.filter(function (path) {
      ...
      text1 = text1.slice(startTags.length);
      if (
        !isNaN(Number(text1[0])) &&
        text1[1] === "." &&
        (text1[2] === tab || text1[2] === " ")
      ) {
        return true;
      } else if (
        !isNaN(Number(text1.slice(0, 2))) &&
        text1[2] === "." &&
        (text1[3] === tab || text1[3] === " ")
      ) {
        return true;
  ...
  getQuestionMinHeight: function (question) {
    if (question.paths.length === 0)
      draw.questionLayout.getPagePathsInRect([
        20,
        question.top,
        1160,
        question.height,
  ...
  distributeAllQuestions: function (obj) {
    ...
    var questionPadding =
      (bottom - top - totalMinHeight) / obj.questions.length;
    //console.log(questionPadding);

    for (var q = 0; q < obj.questions.length; q++) {
      var question = obj.questions[q];
      //console.log(top);
      draw.questionLayout.setQuestionPosition(
        question,
        top,
        question.minHeight + questionPadding
      );
  ...
  dragVertStart: function (e) {
    ...
    draw._drag = {
      ...
      q: draw.currCursor.q,
      offset:
        draw.mouse[1] - draw.currCursor.obj.questions[draw.currCursor.q].top,
    };
    ...
    draw.animate(
      draw.questionLayout.dragVertMove,
      draw.questionLayout.dragVertStop,
      drawCanvasPaths
    );
  },
  dragVertMove: function (e) {
    ...
    var nextTop =
      q < obj.questions.length - 1 ? obj.questions[q + 1].top : 1640;
  ...
  dragBottomStart: function (e) {
    ...
    draw.animate(
      draw.questionLayout.dragBottomMove,
      draw.questionLayout.dragBottomStop,
      drawCanvasPaths
    );
  },
...
draw.mixedPath = {
  groupPaths: function () {
    ...
    for (var p = 0; p < selected.length; p++) {
      ...
      for (var o = 0; o < path.obj.length; o++) {
        var obj = path.obj[o];
        if (["line", "angle", "curve", "curve2"].indexOf(obj.type) === -1)
          continue;
        objs.push(obj);
        path.obj.splice(o, 1);
        o--;
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var p = 0; p < obj.paths.length; p++) {
      ...
      switch (obj2.type) {
        case "line":
          if (!arraysEqual(prev, obj2.startPos))
        ...
        case "angle":
          if (obj2.clockwise === true) {
            obj2.a = pointAddVector(
              obj2.b,
              [Math.cos(obj2.angleA), Math.sin(obj2.angleA)],
              obj2.radius
            );
            obj2.c = pointAddVector(
              obj2.b,
              [Math.cos(obj2.angleC), Math.sin(obj2.angleC)],
              obj2.radius
            );
...
draw.protractor2 = {
  ...
  drawOverlay: function (ctx, obj, path) {
    if (!un(obj.button)) {
      ...
      if (obj.protractorVisible == false) {
        ...
        for (var i = 0; i < 7; i++) {
          ctx.moveTo(
            27.5 + 4 * Math.cos((1 + i / 6) * Math.PI),
            34.5 + 4 * Math.sin((1 + i / 6) * Math.PI)
          );
          ctx.lineTo(
            27.5 + 16 * Math.cos((1 + i / 6) * Math.PI),
            34.5 + 16 * Math.sin((1 + i / 6) * Math.PI)
          );
        }
        ctx.stroke();
        ctx.beginPath();
        ...
        for (var i = 0; i < 19; i++) {
          ctx.moveTo(
            27.5 + 17 * Math.cos((1 + i / 18) * Math.PI),
            34.5 + 17 * Math.sin((1 + i / 18) * Math.PI)
          );
          ctx.lineTo(
            27.5 + 19 * Math.cos((1 + i / 18) * Math.PI),
            34.5 + 19 * Math.sin((1 + i / 18) * Math.PI)
          );
        }
    ...
    if (obj.protractorVisible !== false) {
      ...
      if (boolean(obj.numbers, true) === true) {
        ...
        ctx.moveTo(
          0 + radius[2] * Math.cos((275 * Math.PI) / 180),
          0 + radius[2] * Math.sin((275 * Math.PI) / 180)
        );
        ...
        for (var i = 0; i < 181; i++) {
          if (i == 90) {
            ...
            ctx.translate(
              0 + 0.8 * rad * Math.cos(angle),
              0 + 0.8 * rad * Math.sin(angle)
            );
            ...
            ctx.moveTo(
              0 + radius[0] * Math.cos(angle),
              0 + radius[0] * Math.sin(angle)
            );
            ctx.lineTo(
              0 + (radius[1] + 3) * Math.cos(angle),
              0 + (radius[1] + 3) * Math.sin(angle)
            );
            ctx.moveTo(
              0 + (radius[2] - 3) * Math.cos(angle),
              0 + (radius[2] - 3) * Math.sin(angle)
            );
            ctx.lineTo(
              0 + (radius[2] + 3) * Math.cos(angle),
              0 + (radius[2] + 3) * Math.sin(angle)
            ...
            ctx.moveTo(
              0 + radius[3] * Math.cos(angle),
              0 + radius[3] * Math.sin(angle)
            );
            ctx.lineTo(
              0 + radius[5] * Math.cos(angle),
              0 + radius[5] * Math.sin(angle)
            );

            ctx.translate(
              0 + 0.75 * rad * Math.cos(angle),
              0 + 0.75 * rad * Math.sin(angle)
            );
            ctx.rotate(angle + 0.5 * Math.PI);
            ...
            ctx.translate(
              0 + 0.845 * rad * Math.cos(angle),
              0 + 0.845 * rad * Math.sin(angle)
            );
            ctx.rotate(angle + 0.5 * Math.PI);
            ctx.fillText(i, 0, 0);
            ctx.rotate(-(angle + 0.5 * Math.PI));
            ...
            ctx.moveTo(
              0 + radius[4] * Math.cos(angle),
              0 + radius[4] * Math.sin(angle)
            );
            ctx.lineTo(
              0 + radius[5] * Math.cos(angle),
              0 + radius[5] * Math.sin(angle)
  ...
  moveProtractorStart: function () {
    ...
    draw.animate(
      draw.protractor2.moveProtractorMove,
      draw.protractor2.moveProtractorStop,
      drawCanvasPaths
    );
  ...
  rotateProtractorStart: function () {
    ...
    draw.animate(
      draw.protractor2.rotateProtractorMove,
      draw.protractor2.rotateProtractorStop,
      drawCanvasPaths
    );
  ...
  rotateProtractorMove: function (e) {
    ...
    var obj = draw._drag.obj;
    obj.angle =
      getAngleTwoPoints(obj.center, draw.mouse) - draw._drag.offsetAngle;
  },
...
draw.ruler2 = {
  ...
  drawOverlay: function (ctx, obj, path) {
    if (!un(obj.button)) {
      ctx.translate(obj.button[0], obj.button[1]);
      var color =
        obj.rulerVisible == true ? draw.buttonSelectedColor : draw.buttonColor;
      roundedRect(ctx, 3, 3, 49, 49, 8, 6, "#000", color);
  ...
  getCursorPositionsInteract: function (obj, path) {
    ...
    if (obj.rulerVisible === true) {
      ...
      obj._verticesArray1 = [
        ...
        [
          left + length * Math.cos(angle) - width * Math.sin(angle),
          top + length * Math.sin(angle) + width * Math.cos(angle),
        ],
      ...
      obj._verticesArray2 = [
        [left, top],
        [
          left + 0.1 * length * Math.cos(angle),
          top + 0.1 * length * Math.sin(angle),
        ],
        [
          left - width * Math.sin(angle) + 0.1 * length * Math.cos(angle),
          top + width * Math.cos(angle) + 0.1 * length * Math.sin(angle),
        ],
        [left - width * Math.sin(angle), top + width * Math.cos(angle)],
      ];
      obj._verticesArray3 = [
        [
          left + length * Math.cos(angle) - 0.1 * length * Math.cos(angle),
          top + length * Math.sin(angle) - 0.1 * length * Math.sin(angle),
        ],
        [left + length * Math.cos(angle), top + length * Math.sin(angle)],
        [
          left + length * Math.cos(angle) - width * Math.sin(angle),
        ...
        [
          left +
            ...
            0.1 * length * Math.cos(angle),
          top +
            length * Math.sin(angle) +
            width * Math.cos(angle) -
            0.1 * length * Math.sin(angle),
        ],
      ];
      obj._center = [
      ...
      obj._verticesArray4 = [
        ...
        [
          left + length * Math.cos(angle) + 40 * Math.sin(angle),
          top + length * Math.sin(angle) - 40 * Math.cos(angle),
        ],
        [left + length * Math.cos(angle), top + length * Math.sin(angle)],
        [left, top],
      ];
      if (obj.markings == true) {
        obj._edgePos1 = [
          left +
            0.02 * length * Math.cos(angle) +
            (0.5 * draw.thickness + 2) * Math.sin(angle),
          top +
            0.02 * length * Math.sin(angle) -
            (0.5 * draw.thickness + 2) * Math.cos(angle),
        ];
        obj._edgePos2 = [
          left +
            0.98 * length * Math.cos(angle) +
            (0.5 * draw.thickness + 2) * Math.sin(angle),
          top +
            0.98 * length * Math.sin(angle) -
            (0.5 * draw.thickness + 2) * Math.cos(angle),
        ];
      } else {
        obj._edgePos1 = [
      ...
      obj._verticesArray5 = [
        [
          left - width * Math.sin(angle) - 40 * Math.sin(angle),
          top + width * Math.cos(angle) + 40 * Math.cos(angle),
        ],
        [
          left +
            length * Math.cos(angle) -
      ...
      obj._penCursor1 =
        ...
        canvas.toDataURL() +
        '") ' +
        (25 - 18 * Math.sin(obj.angle)) +
        ", " +
        (25 + 18 * Math.cos(obj.angle)) +
        ", auto";
      ...
      obj._penCursor2 =
        ...
        canvas.toDataURL() +
        '") ' +
        (25 - 18 * Math.sin(Math.PI + obj.angle)) +
        ", " +
        (25 + 18 * Math.cos(Math.PI + obj.angle)) +
        ", auto";
  ...
  moveRulerStart: function () {
    ...
    draw.animate(
      draw.ruler2.moveRulerMove,
      draw.ruler2.moveRulerStop,
      drawCanvasPaths
    );
    draw.cursorCanvas.style.cursor = draw.cursors.move2;
    draw.cursorOverride = draw.cursors.move2;
  ...
  rotateRulerStart1: function () {
    ...
    draw.animate(
      draw.ruler2.rotateRulerMove1,
      draw.ruler2.rotateRulerStop,
      drawCanvasPaths
    );
    draw.cursorCanvas.style.cursor = draw.cursors.rotate;
    draw.cursorOverride = draw.cursors.rotate;
  ...
  rotateRulerMove1: function (e) {
    ...
    dAngle = measureAngle({
      c: draw.mouse,
      b: [obj._centerX2, obj._centerY2],
      a: [draw._drag.prevX, draw._drag.prevY],
      angleType: "radians",
    });
    obj.angle += dAngle;
  ...
  rotateRulerMove2: function (e) {
    ...
    obj.angle += measureAngle({
      c: draw.mouse,
      b: [obj._centerX1, obj._centerY1],
      a: [draw._drag.prevX, draw._drag.prevY],
      angleType: "radians",
    });
  ...
  drawRulerStart1: function () {
    ...
    draw.movePathToFront(obj._path);
    draw.drawing = true;
    var pos = closestPointOnLineSegment(
      draw.mouse,
      obj._edgePos1,
      obj._edgePos2
    );
    ...
    draw.animate(
      draw.ruler2.drawRulerMove1,
      draw.ruler2.drawRulerStop,
      drawCanvasPaths
    );
  ...
  drawRulerMove1: function (e) {
    updateMouse(e);
    var obj = draw._drag.obj;
    draw._drag.line.finPos = closestPointOnLineSegment(
      draw.mouse,
      obj._edgePos1,
      obj._edgePos2
    );
...
draw.compass2 = {
  ...
  drawOverlay: function (ctx, obj, path) {
    if (!un(obj.button)) {
      ctx.translate(obj.button[0], obj.button[1]);
      var color =
        obj.compassVisible == true
          ? draw.buttonSelectedColor
          : draw.buttonColor;
      roundedRect(ctx, 3, 3, 49, 49, 8, 6, "#000", color);
      ...
      var armLength = Math.sqrt(
        Math.pow(0.5 * (center3[0] - center1[0]), 2) +
          Math.pow(center2[1] - center1[1], 2)
      );

      var angle2 =
        -0.5 * Math.PI -
        Math.atan((center2[1] - center1[1]) / (center2[0] - center1[0]));
      var angle3 =
        0.5 * Math.PI -
        Math.atan((center3[1] - center2[1]) / (center3[0] - center2[0]));
    ...
    if (obj.compassVisible !== false) {
      ...
      if (drawOn == "right") {
        ...
        ctx.lineTo(
          7 + 45 * Math.cos(pAngle),
          armLength - 95 + 45 * Math.sin(pAngle)
        );
        ctx.lineTo(
          7 + 45 * Math.cos(pAngle) - 20 * Math.sin(pAngle),
          armLength - 95 + 45 * Math.sin(pAngle) + 20 * Math.cos(pAngle)
        );
  ...
  recalc: function (obj) {
    if (!un(obj._lastCalcValues)) {
      if (
        obj._lastCalcValues.armLength === obj.armLength &&
        obj._lastCalcValues.radius === obj.radius &&
        obj._lastCalcValues.angle === obj.angle &&
        obj._lastCalcValues.center1X === obj.center1[0] &&
        obj._lastCalcValues.center1Y === obj.center1[1]
      )
        return;
    }
    ...
    obj._center3 = [
      center1[0] + rad * Math.cos(angle),
      center1[1] + rad * Math.sin(angle),
    ];
    ...
    if (angleX > 0.5 * Math.PI && angleX < 1.5 * Math.PI) {
      obj._drawOn = "left";
      obj._center2 = [
        center1[0] + 0.5 * rad * Math.cos(angle) - obj._h * Math.sin(angle),
        center1[1] + 0.5 * rad * Math.sin(angle) + obj._h * Math.cos(angle),
      ];
    } else {
      obj._drawOn = "right";
      obj._center2 = [
  ...
  getCursorPositionsInteract: function (obj, path) {
    ...
    if (obj.compassVisible === true) {
      ...
      if (obj._drawOn == "right") {
        ...
        var center4 = [
          obj._center3[0] - 200 * Math.sin(angle3),
          obj._center3[1] - 200 * Math.cos(angle3),
        ];
      } else {
        var angle3 = -(angle2 + obj.angle - Math.PI / 14);
        var center4 = [
      ...
      obj._pencilPolygon = [
        [
          center4[0] + 20 * Math.cos(angle3),
          center4[1] - 20 * Math.sin(angle3),
        ],
        [
          center4[0] - 20 * Math.cos(angle3),
          center4[1] + 20 * Math.sin(angle3),
        ...
        [
          obj._center3[0] - 20 * Math.cos(angle3),
          obj._center3[1] + 20 * Math.sin(angle3),
        ],
        [
          obj._center3[0] + 20 * Math.cos(angle3),
          obj._center3[1] - 20 * Math.sin(angle3),
      ...
      if (obj._drawOn == "right") {
        obj._topPolygon = [
          [
            obj._center2[0] -
              62 * Math.sin(-obj.angle) +
              10 * Math.cos(obj.angle),
            obj._center2[1] -
              62 * Math.cos(obj.angle) -
              10 * Math.sin(-obj.angle),
          ],
          [
            obj._center2[0] -
              62 * Math.sin(-obj.angle) -
          ...
          [
            obj._center2[0] - 10 * Math.cos(obj.angle),
            obj._center2[1] + 10 * Math.sin(-obj.angle),
          ],
          [
            obj._center2[0] + 10 * Math.cos(obj.angle),
            obj._center2[1] - 10 * Math.sin(-obj.angle),
  ...
  moveCompassStart1: function () {
    ...
    draw.animate(
      draw.compass2.moveCompassMove1,
      draw.compass2.moveCompassStop,
      drawCanvasPaths
    );
  ...
  moveCompassMove1: function (e) {
    ...
    if (snapToObj2On || draw.snapLinesTogether)
      obj.center1 = snapToObj2(obj.center1);
  },
  moveCompassStart2: function () {
    ...
    draw._drag = {
      obj: obj,
      offset: [
        obj._center3[0] - draw.mouse[0],
        obj._center3[1] - draw.mouse[1],
      ],
    };
    draw.animate(
      draw.compass2.moveCompassMove2,
      draw.compass2.moveCompassStop,
      drawCanvasPaths
    );
    draw.cursorCanvas.style.cursor = draw.cursors.move2;
  ...
  moveCompassMove2: function (e) {
    ...
    if (obj.radiusLocked == false) {
      ...
      var newRadius = Math.sqrt(
        Math.pow(newcenter3[0] - obj.center1[0], 2) +
          Math.pow(newcenter3[1] - obj.center1[1], 2)
      );
      ...
      if (newRadius <= 1.85 * obj.armLength) {
        ...
        if (obj._center3[0] >= obj.center1[0]) {
          obj.angle = Math.atan(
            (obj._center3[1] - obj.center1[1]) /
              (obj._center3[0] - obj.center1[0])
          );
        } else {
          obj.angle =
            Math.PI +
        ...
        if (newcenter3[0] >= obj.center1[0]) {
          obj.angle = Math.atan(
            (newcenter3[1] - obj.center1[1]) / (newcenter3[0] - obj.center1[0])
          );
        } else {
          obj.angle =
            Math.PI +
            Math.atan(
              (newcenter3[1] - obj.center1[1]) /
                (newcenter3[0] - obj.center1[0])
            );
        }
        obj._center3[0] =
          obj.center1[0] + 1.85 * obj.armLength * Math.cos(obj.angle);
        obj._center3[1] =
          obj.center1[1] + 1.85 * obj.armLength * Math.sin(obj.angle);
  ...
  drawCompassStart: function () {
    ...
    draw.animate(
      draw.compass2.drawCompassMove,
      draw.compass2.drawCompassStop,
      drawCanvasPaths
    );
    draw.cursorCanvas.style.cursor = draw.currCursor.cursor;
    draw.cursorOverride = draw.currCursor.cursor;
  ...
  drawCompassMove: function (e) {
    ...
    var dAngle = measureAngle({
      c: draw.mouse,
      b: [obj.center1[0], obj.center1[1]],
      a: prev,
      angleType: "radians",
    });
    if (dAngle > Math.PI) {
...
draw.dropMenu = {
  ...
  getButtons: function (x1, y1, x2, y2, pathNum, path) {
    if (un(path) && un(pathNum)) return;
    if (un(path)) path = draw.path[pathNum];
    if (un(path.obj)) return;
    var obj = path.obj[0];
    var buttons = [];
    buttons.push({
  ...
  getWidthAtHeight: function (obj, h) {
    ...
    for (var s = 0; s < obj.shape.length; s++) {
      ...
      if (
        ...
        return (
          shape.pos[0][1] +
          ((h - shape.pos[0][0]) / (shape.pos[1][0] - shape.pos[0][0])) *
            (shape.pos[1][1] - shape.pos[0][1])
        );
      }
    }
    return w;
      ...
      if (shape.type == "line") {
        ...
        if (areaCount + shapeArea >= area) {
          ...
          if (w1 == w2) {
            ...
            var m = (w2 - w1) / hh;
            return (
              shape.pos[0][0] + (Math.sqrt(w1 * w1 + 2 * m * area) - w1) / m
            );
          }
        }
        areaCount += shapeArea;
...
draw.three = {
  ...
  path3d: {
        ...
        if (arraysEqual(direction, [0, 0, 1])) {
          ...
          for (var i = 0; i <= path3d.squares; i++) {
            ...
            paths.push(
              {
                pos: [
                  [-width / 2 + 0.001, pos2, 0],
                  [-width / 2 + 0.001, pos2, width],
                ],
                lineWidth: 1,
                strokeStyle: "#000",
              },
          ...
          for (var i = 0; i <= path3d.squares; i++) {
            ...
            paths.push(
              {
                pos: [
                  [pos2, -width / 2 + 0.001, 0],
                  [pos2, -width / 2 + 0.001, width],
                ],
                lineWidth: 1,
                strokeStyle: "#000",
              },
    ...
    arrow: {
      ...
      getPoints: function (path3d) {
        ...
        var remainingAngle =
          Math.PI / 2 - angleBetweenLinesRads - angleToHorizontal;
        if (gradient !== 0 && p1[1] > p0[1]) {
          // downwards
        ...
        var v1 = [
          Math.sin(remainingAngle) * arrowLength,
          Math.cos(remainingAngle) * arrowLength,
        ];
        var v2 = [
          Math.cos(angleBetweenLinesRads - angleToHorizontal) * arrowLength,
          Math.sin(angleBetweenLinesRads - angleToHorizontal) * arrowLength,
        ];

        if (
          gradient == Infinity ||
          (gradient < 0 && p1[1] < p0[1]) ||
          (gradient == 0 && p1[0] < p0[0]) ||
          (gradient < 0 && p1[1] > p0[1])
        ) {
          var signs = [1, 1, 1, -1];
      ...
      scale: function (path3d, sf) {
        ...
        path3d.pos[1][0] *= sf;
        path3d.pos[1][1] *= sf;
        if (!un(path3d.arrowLength)) path3d.arrowLength *= sf;
      },
    },
    cuboid: {
      get: function () {
          ...
          for (var a = 0; a < path3d.paths.length; a++) {
            if (path3d.paths[a].type == "angle") {
              ...
              if (angle.labelType == "none") {
                ...
                labelType = "custom";
                if (typeof angle.label == "string") angle.label = [angle.label];
              }
              f.push({
                pos3d: anglePos,
                stroke: false,
              ...
              for (var a2 = 0; a2 < polygon.pos.length; a2++) {
                if (typeof polygon.pos[a2] == "object") {
                  if (polygon.pos[a2].type == "midpoint") {
                    ...
                    polygonPos[a2] = vector.setMagnitude(
                      pos3,
                      vector.getMagnitude(pos3) / 2
                    );
                  }
                ...
                paths: [
                  {
                    ...
                    pos: [x, y],
                    lineColor:
                      lineSegment.lineColor || lineSegment.color || "#00F",
        ...
        switch (vars.type) {
          case "center":
            ...
            if (!un(obj.gridBounds)) {
              path3d.pos[0] = Math.min(
                draw.three.snapPos(obj, pos3d[0], 0),
                obj.gridBounds[0][1] * obj.gridStep - path3d.dims[0]
              );
              path3d.pos[1] = Math.min(
                draw.three.snapPos(obj, pos3d[1], 1),
                obj.gridBounds[1][1] * obj.gridStep - path3d.dims[1]
              );
            } else {
              path3d.pos[0] = Math.min(
                draw.three.snapPos(obj, pos3d[0], 0),
                obj.gridSize / 2 - path3d.dims[0]
              );
              path3d.pos[1] = Math.min(
                draw.three.snapPos(obj, pos3d[1], 1),
                obj.gridSize / 2 - path3d.dims[1]
              );
            }
            break;
          case "dim":
            if (vars.dimIndex == 2) {
              ...
              path3d.dims[2] = draw.three.snapPos(
                obj,
                draw.three.convert2dHeightTo3d(obj, pos[1] - draw.mouse[1]),
                2
              );
              ...
              if (!un(obj.gridBounds)) {
                path3d.dims[i] = bound(
                  pos[i] - path3d.pos[i],
                  obj.gridStep,
                  obj.gridBounds[i][1] * obj.gridStep - path3d.pos[i],
                  snapTo
                );
              } else {
                path3d.dims[i] = bound(
    ...
    prism: {
      get: function () {
        ...
        for (var i = 0; i < 5; i++) {
          polygon.push([
            2 * obj.gridStep * Math.cos(a),
            2 * obj.gridStep * Math.sin(a),
          ]);
          a += da;
        }
        drawSelectedPaths();
        ...
        if (!un(path3d.labels)) {
          for (var l = 0; l < labels.length; l++) {
            ...
            if (label.type == "vertex") {
              ...
              var a =
                label.pos[0] < polygon.length
                  ? v1[label.pos[0]]
                  : v2[label.pos[0] % polygon.length];

              var b =
                label.pos[1] < polygon.length
                  ? v1[label.pos[1]]
                  : v2[label.pos[1] % polygon.length];
      ...
      pointDragMove: function (obj, path3d, vars) {
        switch (vars.type) {
          case "polygonPoint":
            ...
            var snapTo = obj.snapTo || obj.gridStep || 60;
            if (!un(obj.gridBounds)) {
              polygon[vars.pointIndex][0] = bound(
                pos3d[0],
                obj.gridBounds[0][0] * obj.gridStep,
                obj.gridBounds[0][1] * obj.gridStep,
                snapTo
              );
              polygon[vars.pointIndex][1] = bound(
                pos3d[1],
                obj.gridBounds[1][0] * obj.gridStep,
                obj.gridBounds[1][1] * obj.gridStep,
                snapTo
              );
            } else {
              polygon[vars.pointIndex][0] = bound(
            ...
            if (
              polygonSelfIntersect2(polygon) == false &&
              polygonConvexTest(polygon) == true
            )
          ...
          case "center":
            ...
            var dx = bound(
              pos3d[0] - path3d.center[0],
              dxMin,
              dxMax,
              obj.snapTo
            );
            var dy = bound(
              pos3d[1] - path3d.center[1],
              dyMin,
              dyMax,
              obj.snapTo
            );
          ...
          case "height":
            ...
            var snapTo = obj.snapTo || obj.gridStep || 60;
            var height = draw.three.convert2dHeightTo3d(
              obj,
              center[1] - draw.mouse[1]
            );

            if (!un(obj.gridBounds)) {
              height = bound(
                height,
                obj.gridBounds[2][0] * obj.gridStep + snapTo,
                obj.gridBounds[2][1] * obj.gridStep,
                snapTo
              );
            } else {
    ...
    pyramid: {
      ...
      getPos3d: function (path3d) {
        ...
        var c = path3d.center;
        var h = path3d.height;
        if (polygonClockwiseTest(polygon) == false) polygon.reverse();
        var vertex = [],
          pos = [],
          v = [],
          e = [],
        ...
        if (arraysEqual(direction, [0, 0, 1])) {
          f.push({
            pos3d: [
              [pos[0][0], pos[0][1], pos[0][2] + h],
              [pos[1][0], pos[1][1], pos[1][2] + h],
              [pos[2][0], pos[2][1], pos[2][2] + h],
            ],
            fill: false,
            stroke: false,
            paths: [
        ...
        if (!un(path3d.labels)) {
          for (var l = 0; l < labels.length; l++) {
            ...
            if (label.type == "vertex") {
              ...
              var pos3 = vector.addVectors(pos1, pos2);
              label.pos3d = vector.setMagnitude(
                pos3,
                vector.getMagnitude(pos3) / 2
              );
              if (!un(label.offset)) {
              ...
              var a =
                label.pos[0] == "apex"
                  ...
                  : label.pos[0] == "center"
                  ? clone(c)
                  : polygonPoints[label.pos[0]].pos;
              var b =
                label.pos[1] == "apex"
                  ? clone(vertex)
                  : label.pos[1] == "center"
                  ? clone(c)
                  : polygonPoints[label.pos[1]].pos;
        ...
        if (!un(path3d.paths)) {
          for (var a = 0; a < path3d.paths.length; a++) {
            if (path3d.paths[a].type == "angle") {
              ...
              for (var a2 = 0; a2 < 3; a2++) {
                if (typeof angle.pos[a2] == "object") {
                  if (angle.pos[a2].type == "midpoint") {
                    ...
                    var pos3 = vector.addVectors(pos1, pos2);
                    anglePos[a2] = vector.setMagnitude(
                      pos3,
                      vector.getMagnitude(pos3) / 2
                    );
                  }
              ...
              var a =
                lineSegment.pos[0] == "apex"
                  ? clone(vertex)
                  : lineSegment.pos[0] == "center"
                  ? clone(c)
                  : polygonPoints[lineSegment.pos[0]].pos;
              var b =
                lineSegment.pos[1] == "apex"
                  ? clone(vertex)
                  : lineSegment.pos[1] == "center"
                  ? clone(c)
                  : polygonPoints[lineSegment.pos[1]].pos;
    ...
    skewPyramid: {
        ...
        if (!un(path3d.labels)) {
          for (var l = 0; l < labels.length; l++) {
            ...
            if (label.type == "vertex") {
              ...
              var a =
                label.pos[0] == "apex" ? clone(vertex) : pos[label.pos[0]];
              var b =
                label.pos[1] == "apex" ? clone(vertex) : pos[label.pos[1]];
            ...
            if (path3d.paths[a].type == "angle") {
              ...
              var b =
                lineSegment.pos[0] == "apex"
                  ...
                  : lineSegment.pos[0] == "center"
                  ? clone(c)
                  : pos[lineSegment.pos[0]];
              var c =
                lineSegment.pos[1] == "apex"
                  ? clone(vertex)
                  : lineSegment.pos[1] == "center"
                  ? clone(c)
                  : pos[lineSegment.pos[1]];
      ...
      scale: function (path3d, sf) {
        ...
        for (var i = 0; i < path3d.center.length; i++) path3d.center[i] *= sf;
        for (var i = 0; i < path3d.vertex.length; i++) path3d.vertex[i] *= sf;
      },
    },
    cylinder: {
      get: function () {
      ...
      pointDragMove: function (obj, path3d, vars) {
        switch (vars.type) {
          case "center":
            var pos3d = draw.three.convert2dPosTo3d(obj, draw.mouse);
            path3d.center[0] = bound(
              pos3d[0],
              -obj.gridSize + path3d.radius,
              obj.gridSize - path3d.radius,
              obj.snapTo
            );
            path3d.center[1] = bound(
              pos3d[1],
              -obj.gridSize + path3d.radius,
              obj.gridSize - path3d.radius,
              obj.snapTo
          ...
          case "radius":
            ...
            path3d.radius = Math.min(
              rMax,
              Math.abs(pos3d[0] - path3d.center[0])
            );
            break;
          case "height":
            var center = draw.three.convert3dPosTo2d(obj, path3d.center);
            path3d.height = draw.three.snapPos(
              obj,
              draw.three.convert2dHeightTo3d(obj, center[1] - draw.mouse[1]),
              2
            );
            break;
        }
    ...
    frustum: {
      ...
      getPos3d: function (path3d) {
        ...
        if (arraysEqual(direction, [0, 0, 1])) {
          for (var i = 0; i < density; i++) {
            v1.push([
              c[0] + r1 * Math.cos(angle),
              c[1] + r1 * Math.sin(angle),
              0,
            ]);
            v2.push([
              c[0] + r2 * Math.cos(angle),
              c[1] + r2 * Math.sin(angle),
              h,
            ]);
            angle += dAngle;
          }
      ...
      pointDragMove: function (obj, path3d, vars) {
        switch (vars.type) {
          ...
          case "radius1":
            ...
            path3d.radius[0] = Math.min(
              rMax,
              Math.abs(pos3d[0] - path3d.center[0])
            );
            break;
          case "radius2":
            var pos3d = draw.three.convert2dPosTo3d(
              obj,
              draw.mouse,
              path3d.height
            );
      ...
      getPos3d: function (path3d) {
        ...
        for (var i = 0; i < density; i++) {
          ...
          for (var j = 0; j < density; j++) {
            pos2.push([
              c[0] + r * (Math.sin(angle2) * Math.sin(angle1)),
              c[1] + r * Math.cos(angle2),
              c[2] + r * Math.sin(angle2) * Math.cos(angle1),
            ]);
            angle2 += dAngle;
          }
          pos.push(pos2);
      ...
      pointDragMove: function (obj, path3d, vars) {
        switch (vars.type) {
          case "center":
            ...
            break;
          case "height":
            var center = draw.three.convert3dPosTo2d(obj, [
              path3d.center[0],
              path3d.center[1],
              0,
            ]);
    ...
    cubeNet: {
      ...
      getPos3d: function (path3d) {
        ...
        for (var f2 = 0; f2 < 4; f2++) {
          ...
          var vector1 = draw.three.getNormalToPlaneFromThreePoints(p1, p2, [
            p1[0],
            p1[1],
            p1[2] + 1,
          ]);
          var vector2 = [0, 0, w];
          var angle =
            o * draw.three.getAngleBetweenTwo3dVectors(vector1, vector2);
          var vector3 = [
    ...
    coneNet: {
      ...
      getPos3d: function (path3d) {
        path3d.foldAngle = draw.three.path3d.coneNet.getConeFoldAngle(
          path3d.radius,
          path3d.angle,
          path3d.faces
        );
        ...
        for (var i = 1; i < path3d.faces; i++) {
          ...
          var p1 = [0, 0, 0];
          var p2 = clone(prev[2]);
          var p3 = draw.three.rotatePointAboutLine(
            clone(prev[1]),
            p1,
            p2,
            foldAngle
      ...
      getConeFoldAngle: function (radius, angle, faces) {
        ...
        var cosA =
          (RR + (2 * HH + RR) * Math.cos(t)) / (2 * HH + RR + RR * Math.cos(t));
    ...
    cylinderNet: {
      ...
      getPos3d: function (path3d) {
        ...
        for (var f = 0; f < path3d.faces / 2; f++) {
          ...
          var dir = [
            faceWidth * Math.cos(angle),
            faceWidth * Math.sin(angle),
            0,
          ];
  ...
  cubeDrawing: {
    getCursorPositionsBuild: function (obj) {
      ...
      for (var i = -squares / 2; i < squares / 2; i++) {
        for (var j = -squares / 2; j < squares / 2; j++) {
          ...
          for (var p = 0; p < pos3d.length; p++) {
            var pos = [
              pos3d[p][0] * obj.gridStep,
              pos3d[p][1] * obj.gridStep,
              pos3d[p][2] * obj.gridStep,
            ];
      ...
      if (un(obj._cubeFaces)) return polygons;
      // get cube face polygons
      for (var f = 0; f < obj._cubeFaces.length; f++) {
        var face = obj._cubeFaces[f];
        var center = [
          face.pos[0] / size + face.normal[0],
          face.pos[1] / size + face.normal[1],
          face.pos[2] / size + face.normal[2],
        ];
        polygons.push({
          pos3d: face.pos3d,
          pos2d: face.pos2d,
    ...
    isCubeAtPosition: function (obj, pos) {
      ...
      for (var p = 0; p < obj.paths3d.length; p++) {
        ...
        if (path3d.type !== "cuboid") continue;
        if (
          path3d.pos[0] !== pos[0] * size ||
          path3d.pos[1] !== pos[1] * size ||
          path3d.pos[2] !== pos[2] * size
        )
          continue;
        if (
          path3d.dims[0] !== size ||
    ...
    addCubeAtPosition: function (obj, pos) {
      if (draw.three.cubeDrawing.isCubeAtPosition(obj, pos)) return;
      var size = obj.gridStep;
      if (!un(obj.gridBounds)) {
  ...
  scale: function (obj, sf, center) {
    ...
    for (var p = 0; p < obj.paths3d.length; p++) {
      var path3d = obj.paths3d[p];
      if (
        un(draw.three.path3d[path3d.type]) ||
        un(draw.three.path3d[path3d.type].scale)
      )
        continue;
  ...
  draw: function (ctx, obj, path) {
    ...
    obj._camera =
      obj.tilt == 1
        ...
        : [
            Math.sin(-obj.angle),
            Math.cos(-obj.angle),
            draw.three.convert2dHeightTo3d(obj, obj.tilt),
          ];
    obj._cameraPos = [
      cameraDist * obj._camera[0],
      cameraDist * obj._camera[1],
      cameraDist * obj._camera[2],
    ];
    ...
    for (var p = 0; p < obj.paths3d.length; p++) {
      ...
      if (path3d.type == "custom") {
        ...
        un(draw.three.path3d[path3d.type]) ||
        un(draw.three.path3d[path3d.type].getPos3d)
      ) {
        continue;
      } else {
      ...
      for (var f = 0; f < path3d._faces.length; f++) {
        path3d._meanPoints.push(
          draw.three.getMeanPoint(path3d._faces[f].pos3d)
        );

        for (var v = 0; v < path3d._faces[f].pos3d.length; v++) {
          path3d._min = un(path3d._min)
            ? path3d._faces[f].pos3d[v][2]
            : Math.min(path3d._min, path3d._faces[f].pos3d[v][2]);
        }
      }
      path3d._meanPoint = draw.three.getMeanPoint(path3d._meanPoints);
      path3d._distanceToCamera = draw.three.getDistanceTwoPoints(
        [path3d._meanPoint[0], path3d._meanPoint[1], 0],
        [obj._cameraPos[0], obj._cameraPos[1], 0]
      );
    ...
    paths3d.sort(function (a, b) {
      if (
        a.drawFirst == true ||
        b.drawLast == true ||
        ["grid", "dotGrid", "arrow"].includes(a.type)
      )
        return -1;
      if (
        b.drawFirst == true ||
        a.drawLast == true ||
        ["grid", "dotGrid", "arrow"].includes(b.type)
      )
        return 1;
    ...
    for (var p = 0; p < paths3d.length; p++) {
      ...
      for (var f = 0; f < path3d._faces.length; f++) {
        ...
        face.angleToCamera = draw.three.getAngleBetweenTwo3dVectors(
          face.normal,
          obj._camera
        );
        face.angleToLight = draw.three.getAngleBetweenTwo3dVectors(
          face.normal,
          obj._light
        );
        face.meanPoint = draw.three.getMeanPoint(face.pos3d);
        face.distanceToCamera = draw.three.getDistanceTwoPoints(
          face.meanPoint,
          obj._cameraPos
        );
        if (draw.three.getAngleBetweenTwo3dVectors(face.normal, [0, 0, 1]) == 0)
          face.up = true;
        if (
          draw.three.getAngleBetweenTwo3dVectors(face.normal, [0, 0, -1]) == 0
        )
          face.down = true;
        face.type2 = "face";
        elements.push(face);
      ...
      for (var l = 0; l < path3d._labels.length; l++) {
        ...
        label.distanceToCamera = draw.three.getDistanceTwoPoints(
          label.pos3d,
          obj._cameraPos
        );
        label.type2 = "label";
      ...
      elements.sort(function (a, b) {
        ...
        for (var p = 0; p < a.pos2d.length; p++) {
          ...
          var relLen = roundToNearest(
            vector.dotProduct(v1, b.normal) / vector.dotProduct(v2, b.normal) -
              1,
            0.001
          );
        ...
        for (var p = 0; p < b.pos2d.length; p++) {
          ...
          var relLen = roundToNearest(
            vector.dotProduct(v1, a.normal) / vector.dotProduct(v2, a.normal) -
              1,
            0.001
          );
          if (Math.abs(relLen) > 0.01) return -relLen;
      ...
      for (var f = 0; f < elements.length; f++) {
        ...
        if (element.type2 == "face") {
          ...
          if (obj.directionalColors == true) {
            ...
            var brightness =
              obj._brightness - (obj._contrast * face.angleToLight) / Math.PI;
  ...
  drawFace: function (ctx, obj, face, path) {
    ...
    if (face.fill !== false && face.fillStyle !== "none") {
      if (
        Math.abs(face.angleToCamera - Math.PI / 2) < 0.001 &&
        face.stroke == false
      ) {
      ...
      if (!un(face.dash)) {
        for (var i = 0; i < face.pos2d.length; i++) {
          ...
          for (var k = 0; k < draw.three._dashLines.length; k++) {
            ...
            if (
              (arraysEqual(pos1, pos3) && arraysEqual(pos2, pos4)) ||
              (arraysEqual(pos1, pos4) && arraysEqual(pos2, pos3))
            ) {
              found = true;
              break;
            }
      ...
      if (obj.faceGrid == true) {
        if (face.normal[0] == 0 && face.normal[1] == 0) {
          ...
          for (var v = 0; v < face.pos3d.length; v++) {
            ...
            edges.push([
              face.pos3d[v],
              face.pos3d[(v + 1) % face.pos3d.length],
            ]);
          }
    ...
    if (!un(face.paths)) {
      for (var p = 0; p < face.paths.length; p++) {
        if (
          face.paths[p].dragOnly == true &&
          path.selected !== true &&
          (un(path.interact) || path.interact.edit3dShape !== true)
        )
          continue;
  ...
  drawPath: function (ctx, obj, path, face) {
    ...
    if (
      path.pos.length == 3 &&
      typeof path.pos[0] == "number" &&
      typeof path.pos[1] == "number" &&
      typeof path.pos[2] == "number"
    ) {
      var pos2d = draw.three.convert3dPosTo2d(obj, path.pos);
      if (
        path.limit == true &&
        dist(obj.center[0], obj.center[1], pos2d[0], pos2d[1]) >
          Math.sqrt(2) * obj.gridSize
      )
        return;
  ...
  drawPathType: {
    angle: function (ctx, obj, angle, face) {
      ...
      if (Math.abs((180 * angleMeasure) / Math.PI - 90) < 0.5) {
        // right angle
        ...
        ctx.save();
        if (
          !un(angle.fillTriangle) &&
          angle.fillTriangle !== "none" &&
          angle.fillTriangle !== false
        ) {
        ...
        if (
          !un(angle.lineTriangle) &&
          angle.lineTriangle !== "none" &&
          angle.lineTriangle !== false
        ) {
        ...
        for (var a = 0.05; a < angleMeasure; a += 0.05) {
          ...
          pos = draw.three.rotatePointAboutLine(
            pos,
            [0, 0, 0],
            normalVector2,
            -0.05
        ...
        if (
          angle.labelType !== "none" &&
          Math.abs(face.angleToCamera - Math.PI / 2) > 0.1
        ) {
  ...
  getCursorPositionsSelected: function (obj, pathNum, override) {
    ...
    if (obj.mode === "cubeBuilding") {
      ...
      for (var p2 = 0; p2 < positions.length; p2++) {
        pos.push({
          ...
          cursor:
            ...
            draw.lineCursor +
            ") " +
            draw.lineCursorHotspot[0] +
            " " +
            draw.lineCursorHotspot[1] +
            ", auto",
      ...
      for (var p = 0; p < obj.paths3d.length; p++) {
        ...
        if (
          !un(draw.three.path3d[path3d.type]) &&
          !un(draw.three.path3d[path3d.type].getDragPoints)
        ) {
          pos = pos.concat(
            draw.three.path3d[path3d.type].getDragPoints(obj, path3d)
          );
        }
        if (!un(path3d._faces)) {
  ...
  getControlPanel: function (obj) {
    ...
    if (
      draw.three.checkPathForType(obj, "pyramid") ||
      draw.three.checkPathForType(obj, "prism")
    ) {
      elements.push({
        name: "base vertices",
        bold: false,
  ...
  properties: {
    dotGrid: {
      ...
      get: function (obj) {
        for (var p = 0; p < obj.paths3d.length; p++)
          if (obj.paths3d[p].type == "dotGrid") return true;
        return false;
      },
    ...
    grid: {
      ...
      get: function (obj) {
        for (var p = 0; p < obj.paths3d.length; p++)
          if (obj.paths3d[p].type == "grid") return true;
        return false;
      },
      set: function (obj, value) {
        if (value == false) {
    ...
    arrow: {
      ...
      get: function (obj) {
        for (var p = 0; p < obj.paths3d.length; p++)
          if (obj.paths3d[p].type == "arrow") return true;
        return false;
      },
      set: function (obj, value) {
        if (value == false) {
    ...
    shape: {
      ...
      get: function (obj) {
        for (var p = 0; p < obj.paths3d.length; p++) {
          if (
            ["grid", "dotGrid", "arrow"].includes(obj.paths3d[p].type) == false
          ) {
      ...
      set: function (obj, value) {
        for (var p = 0; p < obj.paths3d.length; p++)
          if (obj.paths3d[p].type == value) return;

        for (p = obj.paths3d.length - 1; p >= 0; p--) {
          var path3d = obj.paths3d[p];
          if (["grid", "arrow", "dotGrid"].includes(path3d.type)) continue;
          obj.paths3d.splice(p, 1);
        }
    ...
    netOpen: {
      ...
      get: function (obj) {
        for (var p = 0; p < obj.paths3d.length; p++) {
          var path3d = obj.paths3d[p];
          if (!un(path3d.open)) return path3d.open;
        }
      },
      increment: function (obj, inc) {
        for (var p = 0; p < obj.paths3d.length; p++) {
          var path3d = obj.paths3d[p];
          if (!un(path3d.open)) path3d.open += inc;
        }
      },
      set: function (obj, value) {
        for (var p = 0; p < obj.paths3d.length; p++) {
          var path3d = obj.paths3d[p];
          if (!un(path3d.open)) path3d.open = value;
        }
      },
    },
  ...
  checkPathForType: function (obj, type) {
    for (var p = 0; p < obj.paths3d.length; p++)
      if (obj.paths3d[p].type == type) return true;
    return false;
  },
  ...
  setPath3d: function () {
    ...
    if (draw.three.checkPathForType(obj, path.type)) return;
    var toDelete = [];
    for (p = obj.paths3d.length - 1; p >= 0; p--) {
      var path3d = obj.paths3d[p];
      if (["grid", "arrow"].includes(path3d.type)) continue;
      obj.paths3d.splice(p, 1);
    }
  ...
  toggleGrid: function (obj, dir) {
    ...
    if (draw.three.checkPathForGrid(obj, dir) == true) {
      for (var p = 0; p < obj.paths3d.length; p++) {
        if (
          obj.paths3d[p].type == "grid" &&
          arraysEqual(dir, obj.paths3d[p].direction)
        ) {
          obj.paths3d.splice(p, 1);
          break;
        }
  ...
  setShape: function (obj, type) {
    if (draw.three.checkPathForType(obj, type)) return;
    var toDelete = [];
    for (p = obj.paths3d.length - 1; p >= 0; p--) {
      var path3d = obj.paths3d[p];
      if (["grid", "arrow", "dotGrid"].includes(path3d.type)) continue;
  ...
  setView: function (obj, angle, tilt) {
    ...
    if (!un(angle)) obj.angle = angle;
    if (!un(tilt)) obj.tilt = tilt;
    drawCanvasPaths();
  },
  ...
  getView: function (obj) {
    if (obj.angle == 1.5 * Math.PI && obj.tilt == 1) return "plan";
    if (obj.angle == 1 * Math.PI && obj.tilt == 0) return "side";
    if (obj.angle == 1.5 * Math.PI && obj.tilt == 0) return "front";
  ...
  rotatePointAboutLine: function (pos, linePos1, linePos2, angle) {
    ...
    var c2 = 1 - c;
    var dir = draw.three.getNormalisedVector([
      linePos2[0] - linePos1[0],
      linePos2[1] - linePos1[1],
      linePos2[2] - linePos1[2],
    ]);
    var x = dir[0];
    var y = dir[1];
    var z = dir[2];
  ...
  convert3dPosTo2d: function (obj, pos) {
    ...
    return [
      obj.center[0] + r * Math.cos(angle),
      obj.center[1] + r * obj.tilt * Math.sin(angle) - height,
    ];
  },
  ...
  getFaceNormal: function (face) {
    ...
    for (var p = 0; p < face.pos3d.length; p++) {
      ...
      for (var i = 0; i < 3; i++) sumVector[i] += cross[i];
    }
  ...
  drawNormalToFace: function (ctx, obj, face) {
    ...
    var normalMag = Math.sqrt(
      Math.pow(face.normal[0], 2) +
        Math.pow(face.normal[1], 2) +
        Math.pow(face.normal[2], 2)
    );
    face.normal = [
      face.normal[0] / normalMag,
      face.normal[1] / normalMag,
      face.normal[2] / normalMag,
    ];
    var normalPoint = [
      meanPoint[0] + 20 * face.normal[0],
      meanPoint[1] + 20 * face.normal[1],
      meanPoint[2] + 20 * face.normal[2],
    ];
  ...
  getMeanPoint: function (polygon) {
    ...
    return [
      total[0] / polygon.length,
      total[1] / polygon.length,
      total[2] / polygon.length,
    ];
  },
  ...
  getRelAngle: function (angle) {
    // gets angle relative to back (ie. 1.5*Math.PI)
    if (angle < 0.5 * Math.PI) return angle + 0.5 * Math.PI;
  ...
  getColor: function (color, brightness, alpha) {
    ...
    return (
      "rgba(" +
      Math.round(c.r * brightness) +
      "," +
      Math.round(c.g * brightness) +
      "," +
      Math.round(c.b * brightness) +
      "," +
      alpha +
      ")"
    );
  ...
  pointDragStart: function (e) {
    draw.animate(
      draw.three.pointDragMove,
      draw.three.pointDragStop,
      drawCanvasPaths
    );
  ...
  pointDragMove: function (e) {
    ...
    if (typeof draw.three.path3d[path3d.type].pointDragMove == "function") {
      draw.three.path3d[path3d.type].pointDragMove(
        draw.drag.obj,
        path3d,
        draw.drag.vars
      );
  ...
  snapPos: function (obj, value, direction) {
    ...
    if (!un(obj.gridBounds)) {
      return bound(
        value,
        obj.gridBounds[direction][0] * obj.gridStep,
        obj.gridBounds[direction][1] * obj.gridStep,
        snapTo
      );
    } else {
      if (direction < 2) {
        return Math.max(
          -obj.gridSize,
          Math.min(obj.gridSize, roundToNearest(value, snapTo))
        );
      } else {
        return Math.max(
          0,
          Math.min(2 * obj.gridSize, roundToNearest(value, snapTo))
        );
      }
    }
  },
  ...
  drag3dStart: function (e) {
    updateMouse(e);
    draw.animate(
      draw.three.drag3dMove,
      draw.three.drag3dStop,
      drawSelectedPaths
    );
  ...
  drag3dMove: function (e) {
    ...
    while (obj.angle < 0) obj.angle += 2 * Math.PI;
    while (obj.angle > 2 * Math.PI) obj.angle -= 2 * Math.PI;
...
draw.text2 = {
  ...
  getRect: function (obj) {
    ...
    if (!un(obj.box) && obj.box.type == "flowArrow") {
      ...
      if (obj.box.dir == "left") rect[0] -= obj.box.arrowWidth / 2 || 20;
    }
    return rect;
  },
  getCursorPositionsSelected: function (obj, pathNum) {
  ...
  textInput: {
    ...
    ansNext: function () {
      ...
      if (arraysEqual(removeTags(clone(currText)), [""])) {
        if (obj.ansIndex >= obj.ans.length - 1) return;
        obj.ans.splice(obj.ansIndex, 1);
      } else {
        obj.ans[obj.ansIndex].text = currText;
      }
  ...
  extractStartTags: function (obj) {
    if (!un(obj.tags)) {
      for (var key in obj.tags) {
        if (obj.tags[key] !== defaultTags[key]) obj[key] = obj.tags[key];
      }
      delete obj.tags;
    }

    if (un(obj.text) || un(obj.text[0])) return;
    while (
      obj.text[0].length > 2 &&
      obj.text[0].slice(0, 2) == "<<" &&
      obj.text[0].slice(0, 3) !== "<<<" &&
      obj.text[0].indexOf(">>") !== -1
    ) {
      ...
      var value = tag.slice(pos2 + 1, -2);
      if (key == "align") {
        if (un(obj.align)) obj.align = [-1, def([obj.vertAlign, -1])];
  ...
  setLineWidth: function (obj, lineWidth) {
    if (un(obj.box)) draw.text2.addBox(obj);
  ...
  getRadius: function (obj) {
    if (un(obj.box)) return 0;
    if (un(obj.box.radius)) return 0;
    if (!isNaN(Number(obj.box.radius))) return Number(obj.box.radius);
    return 0;
  },
  ...
  draw: function (ctx, obj, path) {
    ...
    if (typeof obj.color === "string" && obj.color.length === 3)
      obj.color = "#" + obj.color;
    var obj2 = clone(obj);
    obj2.ctx = ctx;
    ...
    if (!un(obj2.underline)) {
      if (
        !un(measure.textLoc) &&
        !un(measure.textLoc[0]) &&
        measure.textLoc[0].length > 1
      ) {
        var loc = measure.textLoc[0];
        var pos = [];
        for (var i = 0; i < loc.length; i++) {
          if (pos.length == 0) {
            if (boolean(loc[i].markupTag, false) == true) continue;
            pos = [
              [loc[i].left, loc[i].top + loc[i].height],
              [loc[i].left + loc[i].width, loc[i].top + loc[i].height],
            ];
          } else {
            if (boolean(loc[i].markupTag, false) == true) break;
    ...
    if (obj2.textEdit == true) {
      if (
        (un(path.isInput) || path.isInput.type !== "text") &&
        obj2.autoHeight !== false
      ) {
        obj.rect[3] = Math.max(obj2.rect[3], measure.tightRect[3]);
      }
      textEdit.cursorMap = textEdit.mapTextLocs(
        obj2,
        measure.textLoc,
        measure.softBreaks,
        measure.hardBreaks
  ...
  convertToTable: function () {
    ...
    for (var r = 0; r < cells.length; r++) {
      for (var c = 0; c < cells[r].length; c++) {
        if (cells[r][c].length == 0) cells[r][c] = [""];
    ...
    repositionObj(
      table,
      obj.rect[0] - table.left,
      obj.rect[1] - table.top,
      0,
      0
    );
    ...
    if (!un(trigger)) draw.path.last().trigger = trigger;
    console.log(table);
    ...
    function text2split(obj, sep) {
      ...
      for (var i = 0; i < obj.length; i++) {
        if (typeof obj[i] !== "string") {
          ...
          if (obj[i].length > 0) ret.last().push(obj[i]);
        }
      }
      for (var i = ret.length - 1; i >= 0; i--) {
        if (
          arraysEqual(ret[i], []) ||
          arraysEqual(ret[i], [""]) ||
          arraysEqual(ret[i], [tab]) ||
          arraysEqual(ret[i], [tab + tab])
        )
          ret.splice(i, 1);
      }
      if (arraysEqual(ret, [])) ret = [""];
      return ret;
    }
  },
  convertGroupToTable: function (rows, cols) {
    ...
    for (var i = 1; i < x.length; i++) {
      ...
      for (var c2 = 0; c2 < c.length; c2++) {
        if (Math.abs(x[i] - c[c2]) < tol) found = true;
      }
      if (found == false) c.push(x[i]);
    }
    for (var i = 1; i < y.length; i++) {
      var found = false;
      for (var r2 = 0; r2 < r.length; r2++) {
        if (Math.abs(y[i] - r[r2]) < tol) found = true;
      }
      if (found == false) r.push(y[i]);
    }
    if (un(rows)) {
      rows = r.length;
      cols = c.length;
    ...
    var widths = [];
    var heights = [];
    for (var r = 0; r < rows; r++) heights[r] = 50;
    for (var c = 0; c < cols; c++) widths[c] = 50;
  ...
  horizResize: function (e) {
    ...
    draw.animate(
      draw.text2.horizResizeMove,
      draw.text2.horizResizeStop,
      drawCanvasPaths
    );
  },
  ...
  scale: function (obj, sf, center) {
    ...
    if (!un(obj.box)) {
      if (!un(obj.box.borderWidth)) obj.box.borderWidth *= sf;
      if (!un(obj.box.radius)) obj.box.radius *= sf;
      if (!un(obj.box.padding)) obj.box.padding *= sf;
      if (!un(obj.box.dash)) {
        if (!un(obj.box.dash[0])) obj.box.dash[0] *= sf;
        if (!un(obj.box.dash[1])) obj.box.dash[1] *= sf;
      }
    }
...
draw.multiSelectTable = {
  add: function () {
    ...
    for (var j = 0; j < 3; j++) widths.push(width);
    var path = {
      obj: [
        {
          type: "table2",
...
draw.singleSelectTable = {
  add: function () {
    ...
    for (var j = 0; j < 2; j++) widths.push(width);
    var path = {
      obj: [
        {
          type: "table2",
...
draw.table2 = {
  ...
  selectTable: {
        ...
        for (var c = 0; c < cols; c++) {
          pos.push({
            ...
            dims: [
              left + paddingH,
              top + paddingV,
              obj.widths[c] - 2 * paddingH,
              obj.heights[r] - 2 * paddingV,
            ],
            cursor: draw.cursors.pointer,
    ...
    cellToggle: function () {
      ...
      if (draw.mode == "interact") {
        if (cell.toggle == true) {
          delete cell.toggle;
          if (interact.multiSelect !== true) delete interact._value;
        } else {
    ...
    shuffleToggle: function () {
      ...
      if (un(isInput.shuffle)) isInput.shuffle = false;
    ...
    reset: function (obj, path) {
      if (un(path)) path = obj._path || draw.getPathOfObj(obj);
  ...
  add: function (r, c) {
    ...
    for (var j = 0; j < c; j++) widths.push(width);
  ...
  draw: function (ctx, obj, path) {
        ...
        for (var c = 0; c < obj.cells[r].length; c++) {
          ...
          if (!un(cell.box)) {
            if (!un(cell.selColors)) {
              cell.box.color =
                selected === true ? cell.selColors[1] : cell.selColors[0];
            } else if (!un(obj.selColors)) {
              cell.box.color =
                selected === true ? obj.selColors[1] : obj.selColors[0];
            } else if (!un(interact.selColors)) {
              cell.box.color =
                selected === true
                  ? interact.selColors[1]
                  : interact.selColors[0];
            }
          }
        }
      }
    ...
    for (var r = 0; r < obj.cells.length; r++) {
      for (var c = 0; c < obj.cells[r].length; c++) {
        ...
        if (!un(cell.paths)) {
          var cellCenter = [
            (table.xPos[c] + table.xPos[c + 1]) / 2,
            (table.yPos[r] + table.yPos[r + 1]) / 2,
          ];
          for (var p = 0; p < cell.paths.length; p++) {
            ...
            var pathCenter = [
              path2.tightBorder[0] + 0.5 * path2.tightBorder[2],
              path2.tightBorder[1] + 0.5 * path2.tightBorder[3],
            ];
    ...
    if (obj.textEdit == true) {
      if (
        !un(textEdit.tableCell) &&
        !un(textEdit.tableCell[0]) &&
        !un(textEdit.tableCell[1]) &&
        !un(table.cellTextMeasure[textEdit.tableCell[0]]) &&
        !un(table.cellTextMeasure[textEdit.tableCell[0]][textEdit.tableCell[1]])
      ) {
        var measure =
          table.cellTextMeasure[textEdit.tableCell[0]][textEdit.tableCell[1]];
        textEdit.cursorMap = textEdit.mapTextLocs(
          obj,
          measure.textLoc,
          measure.softBreaks,
  ...
  getCursorPositionsInteract: function (obj, path, pathNum) {
    ...
    if (!un(path)) {
      ...
      if (
        !un(interact) &&
        interact.type === "select" &&
        interact._disabled !== true
      ) {
        pos = pos.concat(
          draw.table2.selectTable.getCursorPositionsInteract(obj, path, pathNum)
        );
      }
    }
    return pos;
  ...
  getButtons: function (x1, y1, x2, y2, pathNum) {
    ...
    if (!un(pathNum)) {
      ...
      if (
        !un(path) &&
        !un(path.isInput) &&
        path.isInput.type == "select" &&
        path.obj.length == 1
      ) {
        buttons = buttons.concat(
          draw.table2.selectTable.getButtons(x1, x2, y1, y2, pathNum)
        );
      }
    }
    return buttons;
  ...
  getSelectedCells: function (obj) {
    ...
    for (var r = 0; r < obj.cells.length; r++) {
      for (var c = 0; c < obj.cells[r].length; c++) {
        if (obj.cells[r][c].selected == true) selected.push(obj.cells[r][c]);
      }
    }
    return selected;
  },
  ...
  countSelectedCells: function (obj) {
    ...
    for (var r = 0; r < obj.cells.length; r++) {
      for (var c = 0; c < obj.cells[r].length; c++) {
        if (obj.cells[r][c].selected == true) selected++;
      }
    }
    return selected;
  },
  ...
  reorder: function (obj) {
    ...
    for (var c = 0; c < obj.widths.length; c++) {
      // get cells column by column
      for (var r = 0; r < cells.length; r++) {
        allCells.push(cells[r][c]);
      }
    }
    for (var r = 0; r < cells.length; r++) {
      // replace cells row by row
      for (var c = 0; c < cells[r].length; c++) {
  ...
  setColumns: function (cols, obj) {
    if (un(obj)) obj = sel();
    if (un(obj)) return;
    if (un(cols)) cols = obj.widths.length - 1;

    var width = 0;
    for (var c = 0; c < obj.widths.length; c++) width += obj.widths[c];
    obj.widths = [];
    for (var c = 0; c < cols; c++) obj.widths[c] = width / cols;
    ...
    while (obj.heights.length < rows) obj.heights.push(obj.heights.last());
  ...
  deleteContent: function (obj) {
    ...
    if (cells.length == 0) cells = draw.table2.getAllCells(obj);
    for (var c = 0; c < cells.length; c++) cells[c].text = [""];
    drawCanvasPaths();
  },
  grabPaths: function () {
    var s = selPaths();
  ...
  resizeCol: function () {
    ...
    draw.selectedPath = draw.currCursor.pathNum;
    draw.animate(
      draw.table2.resizeMove,
      draw.table2.resizeStop,
      drawCanvasPaths
    );
  ...
  resizeMove: function (e) {
    ...
    if (obj.type == "table") {
      ...
      if (draw.tableColResizing == obj.widths.length) {
        ...
        tableRight += width;
        if (
          Math.abs(tableRight - (draw.drawArea[2] - draw.gridMargin[2])) <
          draw.snapTolerance
        )
  ...
  resizeRow: function () {
    ...
    draw.selectedPath = draw.currCursor.pathNum;
    draw.animate(
      draw.table2.rowResizeMove,
      draw.table2.rowResizeStop,
      drawCanvasPaths
    );
  ...
  rowResizeMove: function (e) {
    ...
    if (path.obj[0].type == "table") {
      for (
        var i = 0;
        i < path.obj[0].cells[draw.tableRowResizing - 1].length;
        i++
      ) {
  ...
  selectCol: function () {
    ...
    draw.animate(
      draw.table2.cellSelectMove,
      draw.table2.cellSelectStop,
      drawCanvasPaths
    );
  ...
  cellSelectMove: function (e) {
    ...
    if (draw.drawMode == "tableRowSelect") {
      ...
      for (var k = 0; k < xPos.length - 1; k++) {
        if (
          (xPos[k] > xMin && xPos[k] < xMax) ||
          (xPos[k] < xMin && xPos[k + 1] > xMax) ||
          (xPos[k + 1] > xMin && xPos[k + 1] < xMax)
        ) {
          colsSelected[k] = true;
        } else {
    ...
    if (draw.drawMode == "tableColSelect") {
      ...
      for (var k = 0; k < yPos.length - 1; k++) {
        if (
          (yPos[k] > yMin && yPos[k] < yMax) ||
          (yPos[k] < yMin && yPos[k + 1] > yMax) ||
          (yPos[k + 1] > yMin && yPos[k + 1] < yMax)
        ) {
          rowsSelected[k] = true;
        } else {
    ...
    if (
      getArrayCount(colsSelected, true) == 1 &&
      getArrayCount(rowsSelected, true) == 1
    ) {
      var selectCount = 1;
    } else if (
      getArrayCount(colsSelected, true) == 0 &&
      getArrayCount(rowsSelected, true) == 0
    ) {
      var selectCount = 0;
    } else {
      var selectCount = 2;
  ...
  deselectTables: function () {
    ...
    for (var i = 0; i < path.length; i++) {
          ...
          for (var r = 0; r < cells.length; r++) {
            for (var c = 0; c < cells[r].length; c++) {
              if (cells[r][c].selected == true) {
                delete cells[r][c].selected;
                delete cells[r][c].highlight;
                cells[r][c].text = removeTagsOfType(
                  cells[r][c].text,
                  "selected"
                );
                changed = true;
      ...
      if (changed) drawSelectedPaths();
    }
  },
  paddingHPlus: function () {
    var pathNum = draw.currCursor.pathNum;
    var obj = draw.path[pathNum].obj[0];
    if (un(obj.paddingH)) obj.paddingH = 0;
    obj.paddingH += 5;
    drawSelectedPaths();
  },
  ...
  paddingVPlus: function () {
    var pathNum = draw.currCursor.pathNum;
    var obj = draw.path[pathNum].obj[0];
    if (un(obj.paddingV)) obj.paddingV = 0;
    obj.paddingV += 5;
    drawSelectedPaths();
  },
  ...
  questionGrid: function (border, dir) {
    ...
    for (var p = 0; p < draw.path.length; p++) {
      if (
        draw.path[p].selected &&
        draw.path[p].obj.length == 1 &&
        draw.path[p].obj[0].type == "table2"
      ) {
        var path = draw.path[p];
        var obj = path.obj[0];
        obj.fontSize = 28;
        if (draw.gridVertMargins.length > 0) {
          var totalWidth =
            draw.drawArea[2] - (draw.gridVertMargins[0] + draw.gridMargin[2]);
        } else {
          var totalWidth =
            draw.drawArea[2] - (draw.gridMargin[0] + draw.gridMargin[2]);
        }
        ...
        if (draw.gridVertMargins.length > 0) {
          repositionPath(
            path,
            draw.gridVertMargins[0] - path.tightBorder[0],
            0,
            0,
            0
          );
          ...
          repositionPath(
            path,
            draw.gridMargin[0] - path.tightBorder[0],
            0,
            0,
            0
          );
  ...
  addRows: function (path, below) {
    if (path.obj[0].type !== "table2") return;
    var obj = path.obj[0];
    ...
    if (boolean(below, true) == false) {
      var newCel = obj.cells
        .slice(0, selRows[0])
        .concat(newRows)
        .concat(obj.cells.slice(selRows[0]));
      var newHei = obj.heights
        .slice(0, selRows[0])
        .concat(newHeights)
        .concat(obj.heights.slice(selRows[0]));
    } else {
      var newCel = obj.cells
        .slice(0, selRows[selRows.length - 1] + 1)
        .concat(newRows)
        .concat(obj.cells.slice(selRows[selRows.length - 1] + 1));
      var newHei = obj.heights
        .slice(0, selRows[selRows.length - 1] + 1)
        .concat(newHeights)
        .concat(obj.heights.slice(selRows[selRows.length - 1] + 1));
    }
    obj.cells = newCel;
    obj.heights = newHei;
  ...
  addCols: function (path, right) {
    ...
    for (var r = 0; r < cells.length; r++) {
      ...
      if (boolean(right, true) == false) {
        var newRow = obj.cells[r]
          .slice(0, selCols[0])
          .concat(newCells)
          .concat(obj.cells[r].slice(selCols[0]));
      } else {
        var newRow = obj.cells[r]
          .slice(0, selCols[selCols.length - 1] + 1)
          .concat(newCells)
          .concat(obj.cells[r].slice(selCols[selCols.length - 1] + 1));
      }
      obj.cells[r] = newRow;
    }
    if (boolean(right, true) == false) {
      obj.widths = obj.widths
        .slice(0, selCols[0])
        .concat(newWidths)
        .concat(obj.widths.slice(selCols[0]));
    } else {
      obj.widths = obj.widths
        .slice(0, selCols[selCols.length - 1] + 1)
        .concat(newWidths)
        .concat(obj.widths.slice(selCols[selCols.length - 1] + 1));
    }
    draw.drawMode = draw.drawModeSave;
    updateBorder(path);
    drawCanvasPaths();
  ...
  mergeTablesV: function () {
    ...
    for (var p = 1; p < paths.length; p++) {
      ...
      for (r = 0; r < cells.length; r++) {
        for (c = 0; c < cells[r].length; c++) {
          if (un(table.cells[rows + r])) table.cells[rows + r] = [];
  ...
  scale: function (obj, sf, center) {
    ...
    if (!un(obj.innerBorder)) {
      if (!un(obj.innerBorder.width)) obj.innerBorder.width *= sf;
      if (!un(obj.innerBorder.dash)) {
        if (!un(obj.innerBorder.dash[0])) obj.innerBorder.dash[0] *= sf;
        if (!un(obj.innerBorder.dash[1])) obj.innerBorder.dash[1] *= sf;
      }
    }
    if (!un(obj.outerBorder)) {
      if (!un(obj.outerBorder.width)) obj.outerBorder.width *= sf;
      if (!un(obj.outerBorder.dash)) {
        if (!un(obj.outerBorder.dash[0])) obj.outerBorder.dash[0] *= sf;
        if (!un(obj.outerBorder.dash[1])) obj.outerBorder.dash[1] *= sf;
      }
    }
    if (!un(obj.text)) {
      if (!un(obj.text.size)) obj.text.size *= sf;
    }
    if (!un(obj.padding)) obj.padding *= sf;
    for (var r = 0; r < obj.cells.length; r++) {
      for (var c = 0; c < obj.cells[r].length; c++) {
  ...
  getStartTags: function (obj) {
    ...
    if (!un(obj.cells) && !un(obj.cells[0]) && !un(obj.cells[0][0])) {
      ...
      if (!un(cell.align)) tags.align = clone(cell.align);
      if (!un(cell.bold)) tags.bold = cell.bold;
      if (!un(cell.italic)) tags.italic = cell.italic;
      if (!un(cell.font)) tags.font = cell.font;
      if (!un(cell.fontSize)) tags.fontSize = cell.fontSize;
      if (!un(cell.textColor)) tags.color = cell.textColor;
...
draw.skewGrid = {
  ...
  draw: function (ctx, obj, path, backColor, backColorFill) {
    ...
    for (var v = 0; v <= obj.vSquares; v++) {
      ctx.moveTo(
        obj.left + v * obj.baseVectors[1][0],
        obj.top + v * obj.baseVectors[1][1]
      );
      ctx.lineTo(
        obj.left +
          v * obj.baseVectors[1][0] +
          obj.hSquares * obj.baseVectors[0][0],
        obj.top +
          v * obj.baseVectors[1][1] +
          obj.hSquares * obj.baseVectors[0][1]
      );
    }
    for (var h = 0; h <= obj.hSquares; h++) {
      ctx.moveTo(
        obj.left + h * obj.baseVectors[0][0],
        obj.top + h * obj.baseVectors[0][1]
      );
      ctx.lineTo(
        obj.left +
          h * obj.baseVectors[0][0] +
          obj.vSquares * obj.baseVectors[1][0],
        obj.top +
          h * obj.baseVectors[0][1] +
          obj.vSquares * obj.baseVectors[1][1]
      );
    }
    ctx.stroke();
    if (!un(path) && path.selected == true) {
      ...
      ctx.moveTo(
        obj.left + obj.baseVectors[0][0] + 8,
        obj.top + obj.baseVectors[0][1]
      );
      ctx.arc(
        obj.left + obj.baseVectors[0][0],
        obj.top + obj.baseVectors[0][1],
        8,
        0,
        2 * Math.PI
      ...
      ctx.moveTo(
        obj.left + obj.baseVectors[1][0] + 8,
        obj.top + obj.baseVectors[1][1]
      );
      ctx.arc(
        obj.left + obj.baseVectors[1][0],
        obj.top + obj.baseVectors[1][1],
        8,
        0,
        2 * Math.PI
  ...
  dragBaseVectorStart: function () {
    ...
    updateSnapPoints();
    draw.animate(
      draw.skewGrid.dragBaseVectorMove,
      draw.skewGrid.dragBaseVectorEnd,
      drawCanvasPaths
    );
  ...
  drawButton: function (ctx, size) {
    ...
    for (var i = 0; i < 5; i++) {
      for (var j = 0; j < 5; j++) {
        ctx.beginPath();
        ctx.arc(
          0.2 * size + i * 0.15 * size,
          0.2 * size + j * 0.15 * size,
          0.02 * size,
          0,
          2 * Math.PI
        );
...
draw.numberline = {
  ...
  toggleHorizVert: function () {
    ...
    if (boolean(obj.vertical) == true) {
      ...
      obj.rect = [
        obj.rect[0],
        obj.rect[1],
        Math.max(obj.rect[2], obj.rect[3]),
        Math.min(obj.rect[2], obj.rect[3]),
      ];
  ...
  getButtons: function (x1, y1, x2, y2, pathNum) {
    ...
    var dims =
      obj.vertical == true ? [x1, y1 + 60, 20, 20] : [x1 + 60, y1, 20, 20];
    var button = {
    ...
    var dims =
      obj.vertical == true ? [x1, y1 + 80, 20, 20] : [x1 + 80, y1, 20, 20];
    var button = {
    ...
    var dims =
      obj.vertical == true ? [x1, y1 + 100, 20, 20] : [x1 + 100, y1, 20, 20];
    var button = {
    ...
    var dims =
      obj.vertical == true ? [x1, y1 + 120, 20, 20] : [x1 + 120, y1, 20, 20];
    var button = {
  ...
  scale: function (obj, sf, center) {
    ...
    obj.rect[3] *= sf;
    if (!un(obj.lineWidth)) obj.lineWidth *= sf;
    if (!un(obj.majorWidth)) obj.majorWidth *= sf;
    if (!un(obj.minorWidth)) obj.minorWidth *= sf;
    if (!un(obj.fontSize)) obj.fontSize *= sf;
    if (!un(obj.arrows)) obj.arrows *= sf;
  },
};
draw.isoDotGrid = {
  resizable: true,
  ...
  scale: function (obj, sf, center) {
    ...
    obj.width *= sf;
    obj.height *= sf;
    if (!un(obj.spacingFactor)) obj.spacingFactor *= sf;
...
draw.cylinder = {
  ...
  getRect: function (obj) {
    return [
      obj.center[0] - obj.radiusX,
      obj.center[1] - obj.radiusY,
      2 * obj.radiusX,
  ...
  scale: function (obj, sf, center) {
    ...
    if (!un(obj.radiusX)) obj.radiusX *= sf;
    if (!un(obj.radiusY)) obj.radiusY *= sf;
...
draw.frustum = {
  ...
  getRect: function (obj) {
    ...
    return [
      obj.center[0] - obj.radiusX,
      obj.center[1] - radiusY2,
      2 * obj.radiusX,
      obj.height + obj.radiusY + radiusY2,
    ];
  },
  changePosition: function (obj, dl, dt, dw, dh) {
    obj.center[0] += dl + 0.5 * dw;
  ...
  scale: function (obj, sf, center) {
    ...
    if (!un(obj.radiusX)) obj.radiusX *= sf;
    if (!un(obj.radiusX2)) obj.radiusX2 *= sf;
    if (!un(obj.radiusY)) obj.radiusY *= sf;
    if (!un(obj.height)) obj.height *= sf;
    if (!un(obj.thickness)) obj.thickness *= sf;
  },
...
draw.isoCuboid = {
  ...
  getPoints: function (obj) {
    var baseVector = [
      [obj.mag1 * (Math.sqrt(3) / 2), -obj.mag1 * (1 / 2)],
      [-obj.mag2 * (Math.sqrt(3) / 2), -obj.mag2 * (1 / 2)],
      [0, -obj.mag3],
    ];
    var pos = [obj.center];
    ...
    for (var i = 0; i < 4; i++) pos.push(pointAddVector(pos[i], baseVector[2]));
    return pos;
  },
  getRect: function (obj) {
    var pos = draw.isoCuboid.getPoints(obj);
  ...
  scale: function (obj, sf, center) {
    ...
    if (!un(obj.mag1)) obj.mag1 *= sf;
    if (!un(obj.mag2)) obj.mag2 *= sf;
    if (!un(obj.mag3)) obj.mag3 *= sf;
    if (!un(obj.thickness)) obj.thickness *= sf;
  },
};
draw.editableText = {
...
draw.grid = {
  ...
  draw: function (ctx, obj, path, backColor, backColorFill) {
    ...
    if (typeof obj2.points == "object") {
      for (var p = 0; p < obj2.points.length; p++) {
        drawCoord(
          ...
          obj.points[p][0],
          obj.points[p][1],
          getShades(mainCanvasFillStyle, true)[12]
        );
      }
    }
    if (typeof obj2.lineSegments == "object") {
      ...
      for (var p = 0; p < obj2.lineSegments.length; p++) {
        if (
          typeof obj2.lineSegments[p][1] !== "undefined" &&
          obj2.lineSegments[p][1].length == 2
        ) {
          if (
            obj2.drawing == "lineSegment" &&
            p == obj2.lineSegments.length - 1
          )
            showPoints = true;
          ...
          drawLine(
            ...
            0,
            obj2,
            obj2.lineSegments[p][0][0],
            obj2.lineSegments[p][0][1],
            obj2.lineSegments[p][1][0],
            obj2.lineSegments[p][1][1],
            "#00F",
            2,
            showPoints,
            true
    ...
    if (typeof obj2.lines == "object") {
      ...
      for (var p = 0; p < obj2.lines.length; p++) {
        if (
          typeof obj2.lines[p][1] !== "undefined" &&
          obj2.lines[p][1].length == 2
        ) {
    ...
    if (obj2.path instanceof Array) {
      var recalc = false;
      if (
        un(obj2._gridValuesForPathCalcs) ||
      ...
      for (var p = 0; p < obj2.path.length; p++) {
        ...
        switch (obj3.type) {
          case "point":
            ...
            if (
              obj3._canvasPos[0] < obj2.left ||
              obj3._canvasPos[0] > obj2.left + obj2.width
            )
              continue;
            if (
              obj3._canvasPos[1] < obj2.top ||
              obj3._canvasPos[1] > obj2.top + obj2.height
            )
              continue;
            ctx.save();
            ctx.beginPath();
            ...
            if (style == "circle") {
              ...
              ctx.lineWidth = !un(obj3.lineWidth)
                ? obj3.lineWidth
                : obj3._selected
                ? 6
                : 3;
          ...
          case "line":
            if (
              obj3.pos[0][0] !== obj3.pos[1][0] ||
              obj3.pos[0][1] !== obj3.pos[1][1]
            ) {
              var lineWidth = !un(obj3.lineWidth)
                ? obj3.lineWidth
                : obj3._selected
                ? 6
                : 3;
          ...
          case "function":
            ...
            break;
          case "function2":
            if (recalc === true || un(obj3._gridPos))
              obj3._gridPos = calcFuncImplicitGridPos(obj2, obj3.func);
            if (recalc === true || un(obj3._canvasPos))
              obj3._canvasPos = calcFuncImplicitCanvasPos(obj2, obj3._gridPos);
            var lineWidth = !un(obj3.lineWidth)
              ? obj3.lineWidth
              : obj3._selected
              ? 6
          ...
          case "path":
            ...
            for (var i = 0; i < obj3.pos.length; i++) {
              ...
              var xPos =
                obj2.left +
                obj2.width * ((x - obj2.xMin) / (obj2.xMax - obj2.xMin));
              var yPos =
                obj2.top +
                obj2.height -
                obj2.height * ((y - obj2.yMin) / (obj2.yMax - obj2.yMin));
          ...
          case "rect":
            obj3.points = [
              ...
              getPosOfCoord(
                [obj3.rect[0] + obj3.rect[2], obj3.rect[1] + obj3.rect[3]],
                obj2
              ),
  ...
  drawOverlay: function (ctx, obj, path) {
    ...
    if (controlsStyle === "buttons") {
      ...
      for (var p = 0; p < paths.length; p++) {
        ...
        var type =
          path.type === "point" && path.style === "circle"
            ? "pointCircle"
            : path.type;
        draw.grid.drawInteractButton(
          ctx,
  ...
  gridPaths: {
    functionTextInputEnd: function (textObj) {
      ...
      if (
        /[a-wzA-WZ]/g.test(stringCopy) == true ||
        (stringCopy.match(/=/g) || []).length !== 1 ||
        stringCopy.charAt(0) == "=" ||
        stringCopy.charAt(stringCopy.length - 1) == "="
      ) {
      ...
      if (js.indexOf("y=") == 0) {
        try {
          var func = new Function(
            "return " + "function(x) {return " + js.slice(2) + ";}"
          )();
        } catch (err) {
          gridPath.type = "function";
          gridPath.valid = false;
    ...
    convertTextToJS: function (textArray) {
      ...
      function removeAllTagsFromString(str) {
        for (var char = str.length - 1; char > -1; char--) {
          if (
            str.slice(char).indexOf(">>") == 0 &&
            str.slice(char - 1).indexOf(">>>") !== 0
          ) {
      ...
      function subJS(elem, addMultIfNecc) {
        ...
        if (typeof elem == "string") {
          ...
          for (var c = 0; c < subText.length - 2; c++) {
            if (
              subText.slice(c).indexOf("sin") == 0 ||
              subText.slice(c).indexOf("cos") == 0 ||
              subText.slice(c).indexOf("tan") == 0
            ) {
          ...
          if (jsArray[depth] !== "") {
            if (jsArray[depth].charAt(jsArray[depth].length - 1) == ")") {
              ...
              jsArray[depth].slice(jsArray[depth].length - 3) == "sin" ||
              jsArray[depth].slice(jsArray[depth].length - 3) == "coa" ||
              jsArray[depth].slice(jsArray[depth].length - 3) == "tan"
            ) {
              trigPower = true;
            } else if (
              /[A-Za-z]/g.test(
                jsArray[depth].charAt(jsArray[depth].length - 1)
              ) == true
            ) {
              baseSplitPoint = jsArray[depth].length - 1;
            } else if (
              /[0-9]/g.test(jsArray[depth].charAt(jsArray[depth].length - 1)) ==
              true
            ) {
              var decPoint = false;
              for (jsChar = jsArray[depth].length - 2; jsChar >= 0; jsChar--) {
                if (decPoint == false && jsArray[depth].charAt(jsChar) == ".") {
                  ...
                  decPoint == true &&
                  jsArray[depth].charAt(jsChar) == "."
                ) {
                  baseSplitPoint = jsChar + 1;
                  break;
                } else if (
                  /[0-9]/g.test(jsArray[depth].charAt(jsChar)) == false
                ) {
                  baseSplitPoint = jsChar + 1;
                  break;
                }
          ...
          if (trigPower == true) {
            if (jsArray[depth] == "-1") {
              jsArray[depth - 1] =
                jsArray[depth - 1].slice(0, -3) +
                "Math.a" +
                jsArray[depth - 1].slice(-3);
            }
          } else {
      ...
      function timesBeforeLetters(testText) {
        for (q = 0; q < testText.length; q++) {
          if (q > 0) {
            if (
              /[a-zA-Z]/g.test(testText.charAt(q)) == true &&
              /[a-zA-Z0-9)]/.test(testText.charAt(q - 1)) == true
            ) {
              testText = testText.slice(0, q) + "*" + testText.slice(q);
            }
            if (
              /[\[(]/g.test(testText.charAt(q)) == true &&
              testText.length > q &&
              /[A-Za-z0-9)]/g.test(testText.charAt(q - 1)) == true
            ) {
              testText = testText.slice(0, q) + "*" + testText.slice(q);
            }
          }
  ...
  getSnapPos: function (obj) {
    ...
    var xAxisPoint = x0;
    while (
      roundToNearest(xAxisPoint, 0.01) <=
      roundToNearest(obj.left + obj.width, 0.01)
    ) {
      if (
        roundToNearest(xAxisPoint, 0.01) > roundToNearest(obj.left, 0.01) &&
        xPos.indexOf(xAxisPoint) == -1
      )
    ...
    var yAxisPoint = y0;
    while (
      roundToNearest(yAxisPoint, 0.01) <=
      roundToNearest(obj.top + obj.height, 0.01)
    ) {
      if (
        roundToNearest(yAxisPoint, 0.01) > roundToNearest(obj.top, 0.01) &&
        yPos.indexOf(yAxisPoint) == -1
      )
  ...
  gridPathStart: function () {
    ...
    if (
      draw.gridDrawMode == "grid-drawLineSegmentPoints" ||
      draw.currCursor.mode == "drawLineSegmentPoints"
    ) {
      draw.gridDrawPoints = [];
      var found = false;
      for (var p = 0; p < obj.path.length; p++) {
        if (obj.path[p].type !== "point") continue;
    ...
    drawCanvasPaths();
    draw.animate(
      draw.grid.gridPathMove,
      draw.grid.gridPathStop,
      drawCanvasPaths
    );
  ...
  gridPathMove: function (e) {
    ...
    if (draw.gridDrawMode == "grid-drawLineSegmentPoints") {
      for (var p = 0; p < draw.gridDrawPoints.length; p++) {
        ...
        if (arraysEqual(pos2, path.pos[0])) continue;
        if (dist(pos[0], pos[1], pos2[0], pos2[1]) < 1) {
          pos = clone(pos2);
          break;
        }
  ...
  gridPathStop: function (e) {
    ...
    if (draw.gridDrawMode == "grid-drawLineSegmentPoints") {
      ...
      for (var p = 0; p < draw.gridDrawPoints.length; p++) {
        var pos2 = draw.gridDrawPoints[p];
        if (arraysEqual(pos2, path.pos[0])) continue;
        if (arraysEqual(pos2, path.pos[1])) found = true;
      }
      if (found == false) {
        obj.path.pop();
        drawCanvasPaths();
      ...
      if (arraysEqual(path.pos[0], path.pos[1]) == true) {
        obj.path.pop();
        if (obj.path.length == 0) delete obj.path;
        drawCanvasPaths();
      }
    }
  ...
  scale: function (obj, sf, center) {
    ...
    obj.width *= sf;
    obj.height *= sf;
    if (!un(obj.xScaleOffset)) obj.xScaleOffset *= sf;
    if (!un(obj.yScaleOffset)) obj.yScaleOffset *= sf;
    if (!un(obj.minorWidth)) obj.minorWidth *= sf;
    if (!un(obj.majorWidth)) obj.majorWidth *= sf;
    if (!un(obj.fontSize)) obj.fontSize *= sf;
  },
  ...
  getCursorPositionsSelected: function (obj, pathNum) {
    ...
    var pos = [];
    if (
      !un(obj._interactMode) &&
      obj._interactMode !== "none" &&
      !un(draw.grid.interact[obj._interactMode])
    ) {
      pos.push({
        shape: "rect",
  ...
  getCursorPositionsInteract: function (obj, path, pathNum) {
    ...
    if (
      ...
      if (obj._interactMode !== "none") {
        if (un(obj._interactCursor))
          obj._interactCursor = draw.grid.getInteractCursor(path, obj);
        var func =
          obj._interactMode == "move"
            ? draw.grid.moveStart
            : draw.grid.interact[obj._interactMode];
        pos.push({
          shape: "rect",
          dims: [obj.left - 10, obj.top - 10, obj.width + 20, obj.height + 20],
          cursor: obj._interactCursor,
  ...
  drawInteractButton: function (ctx, obj, type, l, t, w, h, color, backColor) {
    ...
    switch (type) {
      ...
      case "plot":
      case "point":
        ctx.fillStyle = !un(backColor)
          ? backColor
          : obj._interactMode == "plot"
          ? colorA("#00F", 1)
          : colorA("#99F", 1);
        ctx.fillRect(l, t, w, h);
        ctx.strokeStyle = "#000";
  ...
  toggleInteract: function () {
    ...
    if (un(path.interact.color)) path.interact.color = "#F00";
    if (un(path.interact.lineWidth)) path.interact.lineWidth = 5;
    if (un(path.interact.max)) path.interact.max = 1000;
    if (un(path.interact.mode2)) path.interact.mode2 = "none";
    if (un(path.interact.buttons))
      path.interact.buttons = clone(draw.grid.interactDefaultButtons);
    if (path.interact.type === "grid") console.log(path.interact);
    updateBorder(path);
    drawCanvasPaths();
  },
  ...
  undo: function () {
    ...
    for (var p = obj.path.length - 1; p >= 0; p--) {
      if (draw.mode == "interact" && obj.path[p]._deletable !== true) continue;
      obj.path.splice(p, 1);
      drawCanvasPaths();
      return;
    }
  ...
  interact: {
    ...
    line: function (type2) {
      ...
      draw.animate(
        draw.grid.interact.lineMove,
        draw.grid.interact.lineStop,
        drawCanvasPaths
      );
    },
...
draw.arc = {
  ...
  draw: function (ctx, obj, path) {
    ...
    if (obj.clockwise == true) {
      ...
      obj._startPos = [
        obj.center[0] + obj.radius * Math.cos(obj.startAngle),
        obj.center[1] + obj.radius * Math.sin(obj.startAngle),
      ];
      obj._finPos = [
        obj.center[0] + obj.radius * Math.cos(obj.finAngle),
        obj.center[1] + obj.radius * Math.sin(obj.finAngle),
      ];
    } else {
      ctx.arc(
        obj.center[0],
...
draw.segment = {
  ...
  posMove: function (e) {
    ...
    if (drag.angle == "startAngle") {
      ...
      if (snapToObj2On == true) pos = snapToObj2(pos);
      obj.center = pos;
    }
  ...
  scale: function (obj, sf, center) {
    if (un(center)) var center = obj.center;
    obj.center[0] = center[0] + sf * (obj.center[0] - center[0]);
    obj.center[1] = center[1] + sf * (obj.center[1] - center[1]);
    if (!un(obj.radius)) obj.radius *= sf;
    if (!un(obj.lineWidth)) obj.lineWidth *= sf;
...
draw.pen = {
  ...
  scale: function (obj, sf, center) {
    if (un(center)) var center = obj.pos[0];
    for (var p = 0; p < obj.pos.length; p++) {
...
draw.line = {
  ...
  draw: function (ctx, obj, path) {
    if (
      un(obj.finPos) ||
      typeof obj.finPos[0] !== "number" ||
      typeof obj.finPos[1] !== "number"
    )
      return;
    ctx.strokeStyle = obj.color;
    ctx.lineWidth = obj.thickness;
  ...
  scale: function (obj, sf, center) {
    if (un(center)) var center = obj.startPos;
    ...
    if (!un(obj.thickness)) obj.thickness *= sf;
    if (!un(obj.endStartSize)) obj.endStartSize *= sf;
    if (!un(obj.endMidSize)) obj.endMidSize *= sf;
    if (!un(obj.endFinSize)) obj.endFinSize *= sf;
    if (!un(obj.dash)) {
      if (!un(obj.dash[0])) obj.dash[0] *= sf;
      if (!un(obj.dash[1])) obj.dash[1] *= sf;
    }
  },
  getControlPanel: function (obj) {
    var elements = [
...
draw.curve = {
  ...
  draw: function (ctx, obj, path) {
    ...
    obj.points = getBezierPoints(
      [obj.startPos[0], obj.startPos[1]],
      [obj.controlPos[0], obj.controlPos[1]],
      [obj.finPos[0], obj.finPos[1]],
      30
    );
  ...
  getRect: function (obj) {
    ...
    obj._width =
      Math.max(obj.startPos[0], obj.finPos[0], obj.controlPos[0]) - obj._left;
    obj._height =
      Math.max(obj.startPos[1], obj.finPos[1], obj.controlPos[1]) - obj._top;
    return [obj._left, obj._top, obj._width, obj._height];
  },
  getCursorPositionsSelected: function (obj, pathNum) {
    var pos = [];
...
draw.rect = {
  draw: function (ctx, obj, path) {
    if (un(obj.finPos) || obj.finPos.length == 0) return;
    ctx.strokeStyle = obj.color;
    ctx.lineWidth = obj.thickness;
    if (typeof ctx.setLineDash !== "function") ctx.setLineDash = function () {};
    if (typeof obj.dash !== "undefined") ctx.setLineDash(obj.dash);
...
draw.prism = {
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var p = 0; p < obj.points; p++) {
      ...
      obj._baseVertices.push([
        obj.center[0] + obj.radiusX * Math.cos(angle),
        obj.center[1] + obj._radiusY * Math.sin(angle),
        angle,
      ]);
      obj._topVertices.push([
        obj.center[0] + obj.radiusX * Math.cos(angle),
        obj.center[1] + obj._radiusY * Math.sin(angle) - obj.height,
        angle,
      ]);
    }
    ...
    if (
      (obj.points == 3 || obj.points == 4) &&
      hitTestPolygon(obj._top, obj._baseVertices) == false
    ) {
      var negDiff = [];
      var posDiff = [];
    ...
    for (var i = 0; i < obj._baseVertices.length; i++) {
      ...
      ctx.lineTo(
        obj._topVertices[(i + 1) % p][0],
        obj._topVertices[(i + 1) % p][1]
      );
      ctx.stroke();
    }
    function isPointVisible(i) {
      ...
      for (var j = 0; j < arr.length - 1; j++) {
        if (
          hitTestPolygon(obj._baseVertices[i], [
            arr[j],
            arr[j + 1],
            obj._top,
          ]) == true
  ...
  getRect: function (obj) {
    ...
    obj._top = [
      obj.center[0],
      obj.center[1] - (1 - obj.tilt) * obj.height - obj._radiusY,
    ];
    if (obj._top[1] < obj.center[1] - obj._radiusY) {
      return [
        obj.center[0] - obj.radiusX,
        obj._top[1] - obj._radiusY,
        obj.radiusX * 2,
        obj.center[1] + 2 * obj._radiusY - obj._top[1],
      ];
    } else {
      return [
        obj.center[0] - obj.radiusX,
        obj.center[1] - obj._radiusY,
        obj.radiusX * 2,
        2 * obj._radiusY,
      ];
    }
  ...
  tiltPlusStart: function () {
    ...
    draw.prism.tiltInterval = setInterval(function () {
      ...
      if (un(obj.tilt)) obj.tilt = obj.radiusY / obj.radiusX;
      if (obj.tilt < 1) {
  ...
  altDragMove: function (obj, dx, dy) {
    ...
    while (obj.startAngle < 0) obj.startAngle += 2 * Math.PI;
    while (obj.startAngle > 2 * Math.PI) obj.startAngle -= 2 * Math.PI;
  },
  scale: function (obj, sf, center) {
    if (un(center)) var center = obj.center;
    obj.center[0] = center[0] + sf * (obj.center[0] - center[0]);
  ...
  getRect: function (obj) {
    ...
    if (obj._top[1] < obj.center[1] - obj._radiusY) {
      return [
        ...
        obj._top[1],
        obj.radiusX * 2,
        obj.center[1] + obj._radiusY - obj._top[1],
      ];
    } else {
      return [
        obj.center[0] - obj.radiusX,
...
draw.octahedron = {
  ...
  getRect: function (obj) {
    ...
    if (obj._top[1] < obj.center[1] - obj._radiusY) {
      return [
        ...
        obj._top[1],
        obj.radiusX * 2,
        obj._bottom[1] + obj._radiusY - obj._top[1],
      ];
    } else {
      return [
        obj.center[0] - obj.radiusX,
  ...
  rotateStart: function () {
    draw.octahedron.rotateIntervalObj =
      draw.path[draw.currCursor.pathNum].obj[0];
...
draw.polygon = {
  ...
  draw: function (ctx, obj, path) {
    ...
    if (draw.mode === "edit" && path.obj.length == 1) {
      ...
      for (var k = 0; k < obj.points.length; k++) {
        if (
          (path.selected == true ||
            (obj.drawing == true && k > 0 && k < obj.points.length - 1)) &&
          !un(obj.angles) &&
          !un(obj.angles[k]) &&
          (obj.angles[k].measureLabelOnly == true ||
            obj.angles[k].drawCurve == false)
        ) {
          var angleObj = {
            ctx: ctx,
            drawLines: false,
          ...
          if (!un(obj.angles[k])) angleObj.drawCurve = !obj.angles[k].drawCurve;
        ...
        if (
          ...
          (typeof obj.outerAngles[k] !== "object" ||
            obj.outerAngles[k].measureLabelOnly == true ||
            obj.outerAngles[k].drawCurve == false)
        ) {
          var angleObj = {
            ctx: ctx,
            drawLines: false,
        ...
        if (path.selected == true && obj.anglesMode == "exterior") {
          ...
          if (obj.exteriorAngles[k].line1.show == false) {
            ...
            ctx.lineTo(
              obj.exteriorAngles[k].line1.pos[0],
              obj.exteriorAngles[k].line1.pos[1]
            );
            ctx.stroke();
          }
          if (obj.exteriorAngles[k].line2.show == false) {
            ...
            ctx.moveTo(obj.points[k][0], obj.points[k][1]);
            ctx.lineTo(
              obj.exteriorAngles[k].line2.pos[0],
              obj.exteriorAngles[k].line2.pos[1]
            );
            ctx.stroke();
          }
          ctx.restore();
          ...
          if (
            un(obj.exteriorAngles[k].a3) ||
            obj.exteriorAngles[k].a3.measureLabelOnly == true ||
            obj.exteriorAngles[k].a3.drawCurve == false
          ) {
          ...
          if (
            un(obj.exteriorAngles[k].a2) ||
            obj.exteriorAngles[k].a2.measureLabelOnly == true ||
            obj.exteriorAngles[k].a2.drawCurve == false
          ) {
          ...
          if (
            un(obj.exteriorAngles[k].a1) ||
            obj.exteriorAngles[k].a1.measureLabelOnly == true ||
            obj.exteriorAngles[k].a1.drawCurve == false
          ) {
        ...
        if (path.selected == true) {
          ...
          if (obj.points.length == 4 && !un(obj.polygonType)) {
            switch (k) {
              ...
              case 3:
                if (
                  ["rect", "para", "rhom", "kite"].indexOf(obj.polygonType) > -1
                ) {
                  continue;
                } else if (["trap"].indexOf(obj.polygonType) > -1) {
                  ctx.fillStyle = "#6F6";
  ...
  getCursorPositionsSelected: function (obj, pathNum) {
    var pos = [];
    for (var k = 0; k < obj.points.length; k++) {
      if (obj.closed == false && k == obj.points.length - 1) continue;
      var p1 = obj.points[k];
    ...
    for (var k = 0; k < obj.points.length; k++) {
      ...
      if (obj.clockwise == true) {
        var next = k - 1;
        var prev = k + 1;
        if (k == 0) next = obj.points.length - 1;
        if (k == obj.points.length - 1) prev = 0;
      } else {
        var prev = k - 1;
        var next = k + 1;
        if (k == 0) prev = obj.points.length - 1;
        if (k == obj.points.length - 1) next = 0;
      }
      pos.push({
        shape: "sector",
        dims: [
          ...
          obj.points[k][1],
          32,
          getAngleTwoPoints(obj.points[k], obj.points[next]),
          getAngleTwoPoints(obj.points[k], obj.points[prev]),
        ],
        cursor: draw.cursors.pointer,
      ...
      if (obj.anglesMode == "outer" && !un(obj.outerAngles)) {
        ...
        pos.push({
          ...
          dims: [
            ...
            32,
            getAngleTwoPoints(obj.points[k], obj.points[prev]),
            getAngleTwoPoints(obj.points[k], pos11),
          ],
          cursor: draw.cursors.pointer,
        ...
        pos.push({
          ...
          dims: [
            ...
            32,
            getAngleTwoPoints(obj.points[k], pos11),
            getAngleTwoPoints(obj.points[k], pos22),
          ],
          cursor: draw.cursors.pointer,
          func: draw.polygon.toggleExteriorAngle,
          pathNum: pathNum,
        ...
        if (
          !un(obj._exteriorAngleLabelPos) &&
          !un(obj._exteriorAngleLabelPos[k])
        ) {
  ...
  changePosition: function (obj, dl, dt, dw, dh) {
    ...
    if (dw !== 0 || dh !== 0) {
      ...
      if (shiftOn == true) {
        var sf = Math.min(
          (x - obj._left) / obj._width,
          (y - obj._top) / obj._height
        );
        var xsf = sf;
        var ysf = sf;
      } else {
      ...
      for (var k = 0; k < obj.points.length; k++) {
        obj.points[k][0] =
          obj._left +
          obj._width * xsf * ((obj.points[k][0] - obj._left) / obj._width);
        obj.points[k][1] =
          obj._top +
          obj._height * ysf * ((obj.points[k][1] - obj._top) / obj._height);
      }
    }
    if (!un(obj.exteriorAngles)) {
      for (var v = 0; v < obj.exteriorAngles.length; v++) {
        ...
        var pos1 = pointAddVector(
          obj.points[v],
          getUnitVector(vector1),
          obj.exteriorAngles[v].line2.dist
        );
        var vector2 = getVectorAB(obj.points[next], obj.points[v]);
        var pos2 = pointAddVector(
          obj.points[v],
          getUnitVector(vector2),
          obj.exteriorAngles[v].line1.dist
        );
  ...
  posMove: function (e) {
    ...
    if (snapToObj2On || draw.snapLinesTogether) {
      var pos = snapToObj2([x, y], pathNum);
    } else if (shiftOn && obj.polygonType !== "square") {
      // snap horiz / vert to adjacent vertices
      var pos = null;
      if (
        !un(obj.polygonType) &&
        obj.points.length == 4 &&
        (obj.polygonType == "rhom" || obj.polygonType == "kite")
      ) {
      ...
      if (
        pos == null &&
        ["square", "rect", "para", "trap"].includes(obj.polygonType) == false
      ) {
        var prev = p - 1;
        ...
        var diff = [
          Math.abs(x - points[prev][0]),
          Math.abs(y - points[prev][1]),
          Math.abs(x - points[next][0]),
          Math.abs(y - points[next][1]),
        ];
        var min = diff.indexOf(arrayMin(diff));
        if (
          pos == null &&
          diff[0] < draw.snapTolerance &&
          diff[3] < draw.snapTolerance
        ) {
          pos = [points[prev][0], points[next][1]];
        }
        if (
          pos == null &&
          diff[1] < draw.snapTolerance &&
          diff[2] < draw.snapTolerance
        ) {
    ...
    if (obj.anglesMode == "exterior" && !un(obj.exteriorAngles)) {
      for (var v = 0; v < obj.exteriorAngles.length; v++) {
        obj.exteriorAngles[v].line1.dist = getDist(
          points[v],
          obj.exteriorAngles[v].line1.pos
        );
        obj.exteriorAngles[v].line2.dist = getDist(
          points[v],
          obj.exteriorAngles[v].line2.pos
        );
      }
    }
    ...
    if (
      !un(obj.polygonType) &&
      (obj.points.length == 4 || obj.points.length == 3) &&
      ...
      switch (obj.polygonType) {
        case "square":
          if (!shiftOn) {
            ...
            if (!un(pivot)) {
              ...
              if (pivot - p == 1 || p - pivot == 3) {
                ...
                points[p2[3]] = pointAddVector(
                  points[p],
                  setVectorMag(vector2, sideLen)
                );
                points[p2[2]] = pointAddVector(
                  points[pivot],
                  setVectorMag(vector2, sideLen)
                );
                break;
          ...
          if (shiftOn) {
            ...
            points[p] = [
              points[p2[2]][0] + xDir * diff,
              points[p2[2]][1] + yDir * diff,
            ];
          } else {
            points[p] = pos;
          }
        ...
        case "trap":
          ...
          switch (p) {
            ...
            case 1:
              ...
              break;
            case 2:
              points[3] = getVectorLinesIntersection(
                points[2],
                vector4a,
                points[0],
                vector3a
              ...
              break;
            case 3:
              points[2] = getVectorLinesIntersection(
                points[3],
                vector4a,
                points[1],
                vector2a
        ...
        case "rhom":
          ...
          if (p == 0 || p == 2) {
            ...
            var vector4 = setVectorMag(
              getPerpVector(vector2),
              getVectorMag(vector1)
            );
  ...
  posStop: function (e) {
    ...
    if (!un(obj.polygonType) && obj.polygonType == "square") {
      if (un(obj._pivotPoint)) obj._pivotPoint = [];
  ...
  startPrismPointDrag: function () {
    ...
    updateSnapPoints(); // update intersection points
    draw.animate(
      draw.polygon.prismPointMove,
      draw.polygon.prismPointStop,
      drawCanvasPaths
    );
  ...
  startExtAnglePointDrag: function () {
    ...
    updateSnapPoints(); // update intersection points
    draw.animate(
      draw.polygon.extAnglePointMove,
      draw.polygon.extAnglePointStop,
      drawCanvasPaths
    );
  ...
  extAnglePointMove: function (e) {
    ...
    if (draw.currLine == 1) {
      ...
      obj.exteriorAngles[v].line1.vector = setVectorMag(
        obj.exteriorAngles[v].line1.vector,
        d
      );
      obj.exteriorAngles[v].line1.pos = pointAddVector(
        obj.points[v],
        obj.exteriorAngles[v].line1.vector
      );
    } else {
      obj.exteriorAngles[v].line2.dist = d;
      obj.exteriorAngles[v].line2.vector = setVectorMag(
        obj.exteriorAngles[v].line2.vector,
        d
      );
      obj.exteriorAngles[v].line2.pos = pointAddVector(
        obj.points[v],
        obj.exteriorAngles[v].line2.vector
      );
    }
  ...
  verticesMinus: function () {
    ...
    if (p < 2) return;
    if (!un(obj.angles) && !un(obj.angles[p])) obj.angles[p] = null;
    obj.points.pop();
    updateBorder(draw.path[pathNum]);
    drawCanvasPaths();
  },
  verticesPlus: function () {
    ...
    var last =
      draw.path[pathNum].obj[0].points[
        draw.path[pathNum].obj[0].points.length - 1
      ];
    draw.path[pathNum].obj[0].points.push([
      (first[0] + last[0]) / 2,
      (first[1] + last[1]) / 2,
    ]);
    updateBorder(draw.path[pathNum]);
  ...
  setExteriorAngles: function () {
    ...
    for (var p = 0; p < obj.points.length; p++) {
      if (!un(obj.exteriorAngles[p])) continue;
      var prev = p - 1;
      if (prev == -1) prev = obj.points.length - 1;
      var next = p + 1;
      if (next == obj.points.length) next = 0;
  ...
  setAngleStyle: function () {
    ...
    if (obj.anglesMode == "outer") {
      ...
      var a2 = posToAngle(
        mouse.x - draw.drawRelPos[0],
        mouse.y - draw.drawRelPos[1],
        obj.points[p][0],
        obj.points[p][1]
      );
      var a3 = posToAngle(
      ...
      if (anglesInOrder(a1, aMouse, a2) == true) {
        obj.exteriorAngles[p].a3 = angleStyleIncrement(
          obj.exteriorAngles[p].a3
        );
      } else if (anglesInOrder(a2, aMouse, a3) == true) {
        obj.exteriorAngles[p].a2 = angleStyleIncrement(
          obj.exteriorAngles[p].a2
        );
      } else if (anglesInOrder(a3, aMouse, a4) == true) {
        obj.exteriorAngles[p].a1 = angleStyleIncrement(
          obj.exteriorAngles[p].a1
        );
      } else {
        obj.angles[p] = angleStyleIncrement(obj.angles[p]);
      }
  ...
  toggleExteriorAngle: function () {
    ...
    if (un(obj.exteriorAngles)) obj.exteriorAngles = [];
    if (un(obj.exteriorAngles[p])) obj.exteriorAngles[p] = {};
    var a = obj.exteriorAngles[p];
    if (s == 1) var angle = a.a1;
    if (s == 2) var angle = a.a2;
    if (s == 3) var angle = a.a3;
    if (un(angle))
      angle = {
        style: 0,
        radius: 30,
    ...
    if (s == 3) a.a3 = angle;
    if (
      (!un(a.a3) && a.a3.drawCurve !== false) ||
      (!un(a.a2) && a.a2.drawCurve !== false)
    ) {
      a.line1.show = true;
    } else {
      a.line1.show = false;
  ...
  setLineDecoration: function () {
    ...
    if (
      typeof obj.lineDecoration[p] == "undefined" ||
      obj.lineDecoration[p] == null
    ) {
  ...
  rotate: function (angle) {
    ...
    function rotate(center, point, angle) {
      var radians = (Math.PI / 180) * angle,
        ...
        nx =
          cos * (point[0] - center[0]) +
          sin * (point[1] - center[1]) +
          center[0],
        ny =
          cos * (point[1] - center[1]) -
          sin * (point[0] - center[0]) +
          center[1];
      return [nx, ny];
    }
  },
  ...
  scale: function (obj, sf, center) {
    if (un(center)) var center = obj.points[0];
    for (var p = 0; p < obj.points.length; p++) {
  ...
  addLabels: function (obj) {
    if (un(obj)) obj = sel();
    if (obj.type !== "polygon" || un(obj._textSnapPos)) return;
    var count = 0;
...
draw.polySquare = {
  ...
  drawButton: function (ctx, size) {
    ...
    for (var d = 0; d < lineDec.length; d++) {
      if (un(lineDec[d])) continue;
      lineDec[d].length = lineDec[d].length * (size / 100);
    }
    for (var a = 0; a < angles.length; a++) {
      if (un(angles[a])) continue;
...
draw.polyEqui = {
  ...
  drawButton: function (ctx, size) {
    var pos = [
      [25, 50 + 12.5 * Math.sqrt(3)],
      [50, 50 - 12.5 * Math.sqrt(3)],
      [75, 50 + 12.5 * Math.sqrt(3)],
    ];
    var lineDec = [
      {
        type: "dash",
...
draw.image = {
  ...
  cropStart: function (e) {
    ...
    var obj = draw._drag.obj;
    if (un(obj.crop))
      obj.crop = [0, 0, obj._image.naturalWidth, obj._image.naturalHeight];
  ...
  cropMove: function (e) {
    ...
    if (side === "left") {
      ...
      var right = Math.min(
        Math.max(draw.mouse[0], obj.left + 40),
        obj._image.naturalWidth / obj._xsf
      );
      ...
      var bottom = Math.min(
        Math.max(draw.mouse[1], obj.top + 40),
        obj._image.naturalHeight / obj._ysf
      );
  ...
  load: function (obj) {
    ...
    if (pdfIndex > -1) {
      ...
      PDFJS.getDocument(url).then(function getPdf(_pdf) {
        ...
        var currentPage = 1;
        if (currentPage <= _pdf.numPages) getPage();
...
draw.circle = {
  ...
  draw: function (ctx, obj, path) {
    ...
    if (
      obj.showCenter == true ||
      (draw.mode === "edit" && path.selected == true && path.obj.length == 1)
    ) {
      ctx.save();
      ctx.fillStyle = "#000";
      ctx.beginPath();
...
draw.angle = {
  ...
  add: function (x, y, r1, r2) {
    ...
    var obj = {
      ...
      calcD: function () {
        ...
        this.d = [
          this.b[0] + this.radius * Math.cos(this.angleA),
          this.b[1] + this.radius * Math.sin(this.angleA),
        ];
      },
    };
    obj.calcD();
    ...
    if (!un(obj.d)) {
      ...
      if (obj.isArc == true || obj.isSector == true) {
        ...
        if (
          isPointInSector(
            [obj.b[0] - 1, obj.b[1]],
            [obj.b[0], obj.b[1], 10, obj.angleA, obj.angleC]
          ) == true
        ) {
  ...
  getButtons: function (x1, y1, x2, y2, pathNum) {
    var buttons = [];
    if (un(pathNum)) return buttons;
    if (un(draw.path[pathNum])) return buttons;
    var obj = draw.path[pathNum].obj[0];
    if (un(obj.d)) {
      buttons.push({
    ...
    if (obj.isSector !== true && obj.isArc !== true) {
      ...
      pos.push({
        ...
        dims: [
          ...
          obj.b[1],
          obj.radius,
          getAngleTwoPoints(obj.b, obj.a),
          getAngleTwoPoints(obj.b, obj.c),
        ],
        cursor: draw.cursors.pointer,
  ...
  startDragB: function () {
    ...
    if (!un(obj.d)) draw.relPosD = getVectorAB(obj.b, obj.d);
    updateSnapPoints(); // update intersection points
    drawCanvasPaths();
    draw.animate(draw.angle.posMove, draw.angle.posStop, drawCanvasPaths);
    //addListenerMove(window, draw.angle.posMove);
  ...
  posMove: function (e) {
    ...
    var pos = draw.mouse;
    var pathNum = draw.currPathNum;
    if (snapToObj2On || draw.snapLinesTogether) pos = snapToObj2(pos, pathNum);
    var obj = draw.path[pathNum].obj[0];
    if (draw.drawMode == "angleDragB") {
      obj.b = pos;
      obj.a = pointAddVector(obj.b, draw.relPosA);
      obj.c = pointAddVector(obj.b, draw.relPosC);
      if (!un(obj.d)) obj.d = pointAddVector(obj.b, draw.relPosD);
    } else if (!un(obj.d)) {
      ...
      if (draw.drawMode == "angleDragA") {
        ...
        obj.a = [
          obj.b[0] + obj.radius * Math.cos(obj.angleA),
          obj.b[1] + obj.radius * Math.sin(obj.angleA),
        ];
      } else if (draw.drawMode == "angleDragC") {
        obj.angleC = getAngleTwoPoints(obj.b, pos);
        obj.c = [
          obj.b[0] + obj.radius * Math.cos(obj.angleC),
          obj.b[1] + obj.radius * Math.sin(obj.angleC),
        ];
      }
    }
    updateBorder(draw.path[pathNum]);
...
draw.anglesAroundPoint = {
    ...
    if (path.obj.length == 1) {
      ...
      for (var k = 0; k < obj.points.length; k++) {
        if (
          ...
          if (!un(obj.angles) && !un(obj.angles[k]))
            angleObj.measureLabelOnly = !obj.angles[k].measureLabelOnly;
          if (!un(obj.angles) && !un(obj.angles[k]))
    ...
    for (var k = 0; k < obj.points.length; k++) {
      ...
      pos.push({
        ...
        dims: [
          ...
          obj.center[1],
          30,
          getAngleTwoPoints(obj.center, obj.points[k]),
          getAngleTwoPoints(obj.center, obj.points[next]),
        ],
        cursor: draw.cursors.pointer,
  ...
  pointsPlus: function (path) {
    ...
    var theta2 =
      2 * Math.PI -
      posToAngle(
        obj.points[obj.points.length - 1][0],
        obj.points[obj.points.length - 1][1],
        obj.center[0],
        obj.center[1]
      );
    if (theta2 < theta1) theta1 -= 2 * Math.PI;
    obj.points.push(
      angleToPos(
        (theta1 + theta2) / 2,
        obj.center[0],
  ...
  startPointDrag: function () {
    ...
    updateSnapPoints(); // update intersection points
    draw.animate(
      draw.anglesAroundPoint.pointMove,
      draw.anglesAroundPoint.pointStop,
      drawCanvasPaths
    );
  ...
  pointMove: function (e) {
    ...
    if (shiftOn) {
      ...
      var snap =
        roundToNearest(roundToNearest(angle, Math.PI / 2) / (Math.PI / 2), 1) %
        4;
      if (snap == 0) {
    ...
    var angle1 = posToAngle(
      obj.points[prevPoint][0],
      obj.points[prevPoint][1],
      obj.center[0],
      obj.center[1]
    );
    var angle2 = posToAngle(
      obj.points[point][0],
      obj.points[point][1],
      obj.center[0],
      obj.center[1]
    ...
    var angle3 = posToAngle(
      obj.points[nextPoint][0],
      obj.points[nextPoint][1],
      obj.center[0],
      obj.center[1]
    );

    if (
      (angle1 < angle2 && angle2 < angle3) || // order 123
  ...
  pointStop: function (e) {
    ...
    if (draw.gridSnap == true) {
      obj.points[point][0] = roundToNearest(
        obj.points[point][0],
        draw.gridSnapSize
      );
      obj.points[point][1] = roundToNearest(
        obj.points[point][1],
        draw.gridSnapSize
      );
      updateBorder(draw.path[pathNum]);
  ...
  setAngleStyle: function (e) {
    updateMouse(e);
    var obj = draw.path[draw.currCursor.pathNum].obj[0];
    var mouseAngle = posToAngle(
      mouse.x,
      mouse.y,
      obj.center[0] + draw.drawRelPos[0],
      obj.center[1] + draw.drawRelPos[1]
    );
  ...
  toggleLabel: function () {
    ...
    var obj = path.obj[0];
    if (obj.type !== "anglesAroundPoint") return;
    var p = draw.currCursor.point;
    if (un(obj.angles)) obj.angles = [];
    if (un(obj.angles[p]))
      obj.angles[p] = {
...
draw.tick = {
  ...
  getRect: function (obj) {
    var rect = clone(obj.rect).concat([
      obj.rect[0] + obj.rect[2],
      obj.rect[1] + obj.rect[3],
    ]);
    return rect;
  },
  changePosition: function (obj, dl, dt, dw, dh) {
...
draw.sphere = {
  ...
  draw: function (ctx, obj) {
    ...
    if (obj.hemisphere === true) ctx.lineWidth = obj.thickness;
    ctx.beginPath();
    ctx.setLineDash([]);
    ctx.ellipse(
      obj.center[0],
...
draw.ellipticalArc = {
  ...
  draw: function (ctx, obj, path) {
    ...
    if (obj.fillColor !== "none") {
      ...
      ctx.moveTo(obj.center[0], obj.center[1]);
      ctx.lineTo(
        obj.center[0] + obj.radiusX * Math.cos(obj.angle1),
        obj.center[1] + obj.radiusY * Math.sin(obj.angle1)
      );
      ctx.ellipse(
        obj.center[0],
        obj.center[1],
    ...
    if (!un(path) && path.selected == true) {
      ...
      ctx.beginPath();
      ctx.arc(
        obj.center[0] + obj.radiusX * Math.cos(obj.angle2),
        obj.center[1] + obj.radiusY * Math.sin(obj.angle2),
        8,
        0,
        2 * Math.PI
      );
  ...
  startDrag1: function (e) {
    ...
    updateSnapPoints(); // update intersection points
    draw.animate(
      draw.ellipticalArc.posMove,
      draw.ellipticalArc.posStop,
      drawCanvasPaths
    );
  ...
  posMove: function (e) {
    ...
    if (shiftOn == true || ctrlOn == true) {
      var tol = 0.3;
      if (Math.abs(angle) < tol) angle = 0;
      if (Math.abs(Math.PI / 2 - angle) < tol) angle = Math.PI / 2;
      if (Math.abs(Math.PI - angle) < tol) angle = Math.PI;
      if (Math.abs(Math.PI * (3 / 2) - angle) < tol) angle = Math.PI * (3 / 2);
      if (Math.abs(Math.PI * 2 - angle) < tol) angle = Math.PI * 2;
    }
...
draw.boxPlot = {
  ...
  draw: function (ctx, obj, path) {
    ...
    var yPos = [
      obj.top,
      obj.top + 0.25 * obj.height,
      obj.top + 0.5 * obj.height,
      obj.top + 0.75 * obj.height,
      obj.top + obj.height,
    ];
...
draw.slider = {
  ...
  getCursorPositionsSelected: function (obj, path, p) {
    ...
    if (typeof obj.control === "object") {
      ...
      pos.push({
        ...
        dims: controlRect,
        cursor: draw.cursors.move1,
        func: draw.slider.controlMoveStart,
        interact: true,
        path: path,
        obj: obj,
      });
  ...
  controlMoveStart: function () {
    ...
    var controlPos = obj.control.pos || [obj.left - 60, obj.top + obj.radius];
    draw._drag.offset = [
      draw.mouse[0] - controlPos[0],
      draw.mouse[1] - controlPos[1],
    ];
    draw.animate(
      draw.slider.controlMoveMove,
      draw.slider.controlMoveStop,
      drawSelectedPaths
    );
  },
  controlMoveMove: function (e) {
    ...
    var obj = draw._drag.obj;
    var pos = [
      draw.mouse[0] - draw._drag.offset[0],
      draw.mouse[1] - draw._drag.offset[1],
    ];
    if (Math.abs(obj.top + obj.radius - pos[1]) < 20)
      pos[1] = obj.top + obj.radius;
    obj.control.pos = pos;
  },
  ...
  editChangeFunction: function (obj) {
    ...
    if (un(obj.interact)) obj.interact = {};
    if (un(obj.interact.onchange)) obj.interact.onchange = function (obj) {};
  ...
  dragMove: function (e) {
    ...
    if (obj.vertical === true) {
      obj.value = Math.max(
        0,
        Math.min(1, (y - (obj.top + obj.radius)) / (obj.width - obj.radius * 2))
      );
    } else {
      obj.value = Math.max(
        0,
        Math.min(
          1,
          (x - (obj.left + obj.radius)) / (obj.width - obj.radius * 2)
        )
      );
    }
  ...
  animationStart: function (obj, rate, drawFunction) {
    ...
    drawCanvasPaths();
    draw._sliderAnimation.frameID = window.requestAnimationFrame(
      draw.slider.animationFrame
    );
  },
  animationFrame: function () {
    ...
    time = time / 1000;
    draw._sliderAnimation.obj.value = Math.min(
      1,
      Math.max(
        0,
        draw._sliderAnimation.initial + time * draw._sliderAnimation.rate
      )
    );
    if (
      !un(draw._sliderAnimation.obj.interact) &&
      !un(draw._sliderAnimation.obj.interact.onchange)
    ) {
      draw._sliderAnimation.obj.interact.onchange(draw._sliderAnimation.obj);
    }

    if (
      draw._sliderAnimation.stop === true ||
      draw._sliderAnimation.obj.value === 1
    ) {
      draw.slider.animationStop();
    } else {
...
draw.toggle = {
  ...
  editChangeFunction: function (obj) {
    ...
    if (un(obj.onchange)) obj.onchange = function (obj) {};
  ...
  toggle: function (obj) {
    ...
    if (typeof obj.onchange == "function") {
      ...
      !un(obj.interact) &&
      typeof obj.interact.onchange == "function"
    ) {
      obj.interact.onchange(obj);
    }
    drawCanvasPaths();
  ...
  draw: function (ctx, obj, path) {
    if (draw.mode == "interact") {
      ...
      text({
        ...
        align: [0, 0],
        text: [
          "<<font:Hobo>><<color:#060>><<fontSize:36>>Feedback will appear here",
        ],
        box: {
          color: "none",
          borderColor: "#666",
...
draw.probabilityTree = {
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var i = 0; i < obj.branches[0]; i++) {
      ...
      for (var j = 0; j < obj.branches[1]; j++) {
        ...
        if (typeof obj.branches[2] !== "number") continue;
        // third branches
  ...
  horizResizeStart: function () {
    ...
    updateSnapPoints(); // update intersection points
    draw.animate(
      draw.probabilityTree.horizResizeMove,
      draw.probabilityTree.horizResizeStop,
      drawCanvasPaths
    );
  ...
  horizResizeMove: function (e) {
    ...
    var obj = draw.path[pathNum].obj[0];
    var left = obj.left;
    for (var i = 0; i < index; i++) left += obj.widths[i];
...
draw.vennDiagram3 = {
  ...
  draw: function (ctx, obj, path) {
    ...
    var h = obj.width;
    var radius = w * 0.27;
    var centerA = obj.centerA || [
      l + w * 0.5 + 0.6 * radius * Math.cos(Math.PI * (1 / 2 + 2 / 3)),
      t + h * 0.47 + 0.6 * radius * Math.sin(Math.PI * (1 / 2 + 2 / 3)),
    ];
    var centerB = obj.centerB || [
      l + w * 0.5 + 0.6 * radius * Math.cos(Math.PI * (1 / 2 + 4 / 3)),
      t + h * 0.47 + 0.6 * radius * Math.sin(Math.PI * (1 / 2 + 4 / 3)),
    ];
    var centerC = obj.centerC || [
      l + w * 0.5 + 0.6 * radius * Math.cos(Math.PI * (1 / 2)),
      t + h * 0.47 + 0.6 * radius * Math.sin(Math.PI * (1 / 2)),
    ];
    obj._radius = radius;
    ...
    var aa = [
      getAngleFromAToB([x1, y1], abint[1]),
      getAngleFromAToB([x1, y1], acint[1]),
      getAngleFromAToB([x1, y1], abint[0]),
      getAngleFromAToB([x1, y1], acint[0]),
    ];
    var bb = [
      getAngleFromAToB([x2, y2], bcint[1]),
      getAngleFromAToB([x2, y2], abint[0]),
      getAngleFromAToB([x2, y2], bcint[0]),
      getAngleFromAToB([x2, y2], abint[1]),
    ];
    var cc = [
      getAngleFromAToB([x3, y3], acint[0]),
      getAngleFromAToB([x3, y3], bcint[0]),
      getAngleFromAToB([x3, y3], acint[1]),
      getAngleFromAToB([x3, y3], bcint[1]),
    ];
  ...
  clickToggleShade: function (e) {
    ...
    var inA =
      dist(pos[0], pos[1], obj._centerA[0], obj._centerA[1]) < obj._radius
        ? true
        : false;
    var inB =
      dist(pos[0], pos[1], obj._centerB[0], obj._centerB[1]) < obj._radius
        ? true
        : false;
    var inC =
      dist(pos[0], pos[1], obj._centerC[0], obj._centerC[1]) < obj._radius
        ? true
        : false;
    if (inA && inB && inC) {
...
draw.pieChart = {
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var a = 0; a < obj.angles.length; a++) {
      ...
      obj._pos[a] = [
        obj.center[0] + obj.radius * Math.cos(angle),
        obj.center[1] + obj.radius * Math.sin(angle),
      ];
    }
    for (var a = 0; a < obj.angles.length; a++) {
      var pos = obj._pos[a];
    ...
    if (!un(obj.showAngles)) {
      for (var a = 0; a < obj.showAngles.length; a++) {
        if (un(obj.showAngles[a])) continue;
  ...
  toggleShowAngle: function () {
    var obj = draw.currCursor.obj;
    var index = draw.currCursor.index;
    if (un(obj.showAngles)) obj.showAngles = [];
  ...
  toggleShowAngleLabel: function () {
    ...
    if (un(obj.showAngles[index])) {
      ...
      obj.showAngles[index].labelMeasure = un(
        obj.showAngles[index].labelMeasure
      )
        ? false
        : !obj.showAngles[index].labelMeasure;
    }
  ...
  startPointDrag: function () {
    ...
    draw.currIndex = draw.currCursor.index;
    draw.animate(
      draw.pieChart.pointMove,
      draw.pieChart.pointStop,
      drawCanvasPaths
    );
  ...
  pointMove: function (e) {
    ...
    obj.angles.sort();
    if (prevAngle < 1 && angle > 5.3) {
      // if angle crosses zero, shift colors
...
draw.buttonCompass = {
  ...
  initCompass: function (init) {
    ...
    var center2 = [
      center1[0] + 0.5 * radius * Math.cos(angle) + h * Math.sin(angle),
      center1[1] + 0.5 * radius * Math.sin(angle) - h * Math.cos(angle),
    ];
    var center3 = [
      center1[0] + radius * Math.cos(angle),
      center1[1] + radius * Math.sin(angle),
    ];
...
draw.buttonRuler = {
  ...
  click: function () {
    if (un(draw.ruler)) draw.buttonRuler.initRuler();
...
draw.buttonPen = {
  ...
  draw: function (ctx, obj, path) {
    ctx.translate(obj.left, obj.top);
    var color =
      draw.drawMode !== "pen" ? draw.buttonColor : draw.buttonSelectedColor;
...
draw.buttonLine = {
  ...
  draw: function (ctx, obj, path) {
    ctx.translate(obj.left, obj.top);
    var color =
      draw.drawMode !== "line" ? draw.buttonColor : draw.buttonSelectedColor;
    roundedRect(ctx, 3, 3, 49, 49, 8, 6, "#000", color);
...
draw.buttonUndo = {
  ...
  draw: function (ctx, obj, path) {
    ...
    ctx.lineTo(
      (13.5 * s) / 55 - ((10 * s) / 55) * Math.sin(1 * Math.PI),
      (27.5 * s) / 55 + ((10 * s) / 55) * Math.cos(1 * Math.PI)
    );
    ctx.lineTo(
      (13.5 * s) / 55 - ((10 * s) / 55) * Math.cos(0.95 * Math.PI),
      (27.5 * s) / 55 - ((10 * s) / 55) * Math.sin(0.95 * Math.PI)
    );
...
draw.lineWidthSelect = {
  ...
  click: function (obj) {
    ...
    if (x < 10 || x > 90 || y < 10 || y > 190) return;
...
draw.buttonColorPicker = {
  ...
  draw: function (ctx, obj, path) {
    ...
    for (var i = 0; i < 9; i++) {
      if (obj._disabled == true) {
        ctx.fillStyle = !un(colors[i])
          ? colorA(colors[i], 0.35)
          : colorA("#FFF", 0.5);
      } else {
...
draw.compassHelp = {
  ...
  draw: function (ctx, obj, path) {
    ...
    text({
      ...
      align: [0, -1],
      text: [
        "<<font:segoePrint>><<bold:true>>Drag the top of the compass to draw an arc",
      ],
    });
...
draw.buttonDash = {
  ...
  draw: function (ctx, obj, path) {
    var color = arraysEqual(draw.dash, obj.dash)
      ? draw.buttonSelectedColor
      : draw.buttonColor;
    ...
    if (typeof ctx.setLineDash !== "function") ctx.setLineDash = function () {};
    if (typeof obj.dash !== "undefined") ctx.setLineDash([4, 7]);
    ctx.strokeStyle = draw.color;
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.lineWidth = 4;
...
draw.fillPath = {
  ...
  fillPolygonAtPoint: function (x, y) {
    ...
    if (un(polygons[0])) return;
    var polygon = polygons[0];
    if (draw.fillPath.devMode == true) console.log(polygon);
  ...
  updateDrawPolygons: function () {
    ...
    for (var n1 = nodes.length - 1; n1 >= 0; n1--) {
    ...
    if (draw.fillPath.devMode == true) console.log("edges:", clone(edges));
    ...
    for (var e1 = edges.length - 1; e1 >= 0; e1--) {
      for (var e2 = e1; e2 >= 0; e2--) {
        ...
        var edge1 = edges[e1];
        var edge2 = edges[e2];
        if (edge1.pos.length !== edge2.pos.length) continue;
        ...
        for (var p = 0; p <= pMax; p++) {
          if (
            matchForward == true &&
            dist(edge1.pos[p], edge2.pos[p]) > draw.fillPath.nodeTolerance
          )
            matchForward = false;
          if (
            matchBack == true &&
            dist(edge1.pos[pMax - p], edge2.pos[p]) >
              draw.fillPath.nodeTolerance
          )
            matchBack = false;
          if (matchForward == false && matchBack == false) break;
        }
    ...
    for (var e = 0; e < edges.length; e++) {
      ...
      if (
        un(edge1.startNode) ||
        un(edge1.finNode) ||
        (edge1.startNode == edge1.finNode && edge1.pos.length == 2)
      ) {
        edges.splice(e, 1);
        e--;
        continue;
      ...
      if (!un(nodes[edge1.startNode])) {
        edge1.angle = getAngleTwoPoints(
          nodes[edge1.startNode].pos,
          edge1.pos[1]
        );
        while (edge1.angle < 0) edge1.angle += 2 * Math.PI;
        while (edge1.angle >= 2 * Math.PI) edge1.angle -= 2 * Math.PI;
      ...
      if (!un(nodes[edge2.finNode])) {
        edge2.angle = getAngleTwoPoints(
          nodes[edge2.finNode].pos,
          edge2.pos[edge2.pos.length - 2]
        );
        while (edge2.angle < 0) edge2.angle += 2 * Math.PI;
        while (edge2.angle >= 2 * Math.PI) edge2.angle -= 2 * Math.PI;
    ...
    function getObjects() {
      ...
      for (var p = 0; p < draw.path.length; p++) {
        var path = draw.path[p];
        if (un(path) || getPathVis(path) == false) continue;
        for (var o = 0; o < path.obj.length; o++) {
          var obj = path.obj[o];
          if (
            typeof obj.trigger !== "undefined" &&
            draw.ansMode == true &&
            obj.trigger[0] == false &&
            draw.showAns == false
          )
          ...
          if (["line", "arc", "circle", "pen", "polygon"].includes(obj.type)) {
            if (obj.type == "line") {
              ...
              for (var p2 = 0; p2 < pos.length; p2++) pos[p2].push(true);
        ...
        for (var p1 = obj1.pos.length - 2; p1 >= 0; p1--) {
          ...
          for (var p2 = p1 - 2; p2 >= 0; p2--) {
            ...
            if (dist(pos1, pos3) < draw.fillPath.nodeTolerance / 5) {
              ...
              if (
                int instanceof Array &&
                isPointOnLineSegment(int, pos1, pos2) == true &&
                isPointOnLineSegment(int, pos3, pos4) == true
              ) {
                ...
                obj1.pos.splice(p1 + 1, 0, [
                  int[0],
                  int[1],
                  boolean(pos1[2], false),
                  true,
                ]);
                obj1.pos.splice(p2 + 1, 0, [
                  int[0],
                  int[1],
                  boolean(pos3[2], false),
                  true,
                ]);
              }
            }
          ...
          for (var p1 = obj1.pos.length - 2; p1 >= 0; p1--) {
            ...
            for (var p2 = obj2.pos.length - 2; p2 >= 0; p2--) {
              ...
              if (dist(pos1, pos3) < draw.fillPath.nodeTolerance) {
                ...
                p1 == 0 &&
                distancePointToLineSegment(pos1, pos3, pos4) <
                  draw.fillPath.nodeTolerance
              ) {
                pos1[3] = true;
                obj2.pos.splice(p2 + 1, 0, [
                  pos1[0],
                  pos1[1],
                  boolean(pos2[2], false),
                  true,
                ]);
                nodes.push({
                  pos: [pos1[0], pos1[1]],
                ...
                p1 == obj1.pos.length - 2 &&
                distancePointToLineSegment(pos2, pos3, pos4) <
                  draw.fillPath.nodeTolerance
              ) {
                pos2[3] = true;
                obj2.pos.splice(p2 + 1, 0, [
                ...
                p2 == 0 &&
                distancePointToLineSegment(pos3, pos1, pos2) <
                  draw.fillPath.nodeTolerance
              ) {
                pos3[3] = true;
                obj1.pos.splice(p1 + 1, 0, [
                ...
                p2 == obj2.pos.length - 2 &&
                distancePointToLineSegment(pos4, pos1, pos2) <
                  draw.fillPath.nodeTolerance
              ) {
                pos4[3] = true;
                obj1.pos.splice(p1 + 1, 0, [
    ...
    function buildEdges(objects) {
      ...
      for (var o = 0; o < objects.length; o++) {
        ...
        for (var p = 0; p < obj.pos.length; p++) {
          var pos = obj.pos[p];
          if (nodeIndex > -1 || pos[3] == true) edge.push(clone(pos));
          if (pos[3] == true) {
            if (edge.length > 1) {
    ...
    function getNodeNextEdge(node, edgeId) {
      if (un(node)) return false;
      if (un(edgeId)) return node.edges[0];
    ...
    function buildPolygons(objects, edges) {
      ...
      if (draw.fillPath.devMode == true) console.log(clone(polygons));
      ...
      for (var p = 0; p < polygons.length; p++) {
        ...
        for (var p2 = 0; p2 < pos.length; p2++) {
          ...
          if (un(pos[p2]) || un(pos[p3])) continue;
        ...
        if (polygonClockwiseTest(pos) == false) pos.reverse();
        polygon.pos = pos;
      }
      if (draw.fillPath.devMode == true) console.log(clone(polygons));
      ...
      for (var p1 = polygons.length - 1; p1 >= 0; p1--) {
        ...
        for (var p2 = p1 - 1; p2 >= 0; p2--) {
          var poly2 = polygons[p2];
          if (draw.fillPath.devMode == true) console.log(p1, p2);
          if (draw.fillPath.devMode == true)
            console.log(poly1.edgeIds, poly2.edgeIds);
          var comp = comparePolygons(poly1, poly2);
          if (draw.fillPath.devMode == true) console.log(comp.type, comp);
          switch (comp.type) {
            case "same":
              polygons.splice(p1, 1);
              p2 = -1;
        ...
        for (var h1 = holes.length - 1; h1 >= 0; h1--) {
          ...
          for (var h2 = holes.length - 1; h2 >= 0; h2--) {
            ...
            for (var h3 = 0; h3 < holeIds.length; h3++) {
              if (
                holeIds[h3][0] == poly2.polygonId &&
                holeIds[h3][1] == poly1.polygonId
              ) {
    ...
    function comparePolygons(polygon1, polygon2) {
      ...
      if (
        poly1.edgeIds.length == poly2.edgeIds.length &&
        comp.edgesCommon.length == poly2.edgeIds.length
      ) {
        comp.type = "same";
        ...
        for (var p = 0; p < poly1.pos.length; p++) {
          ...
          if (
            pos1[5] !== true &&
            pointInPolygon(pos1, poly2.pos, false) == true
          ) {
        ...
        for (var p = 0; p < poly2.pos.length; p++) {
          ...
          if (
            pos1[5] !== true &&
            pointInPolygon(pos1, poly1.pos, false) == true
          ) {
        ...
        for (var p1 = 0; p1 < pos1.length; p1++) {
          if (aInsideB == true && aOutsideB == true) break;
          var curr = pos1[p1];
        ...
        var bOutsideA = false;
        for (var p2 = 0; p2 < pos2.length; p2++) {
          if (bInsideA == true && bOutsideA == true) break;
          var curr = pos2[p2];
          for (var p1 = 0; p1 < pos1.length; p1++) {
    ...
    function pointDistToPolygonBoundary(point, pos) {
      ...
      for (var p = 0; p < pos.length; p++) {
        ...
        var dist = Math.min(
          dist,
          distancePointToLineSegment(point, pos1, pos2)
        );
      }
      return dist;
    }
    function polygonHoleOfPolygon(polygon1, polygon2) {
      ...
      if (polygonsEdgeInCommon(polygon1, polygon2) == true) return false;
      if (polygonInsidePolygon(polygon1.pos, polygon2.pos, false)) return true;
      return false;
    }
    function polygonSubOfPolygon(polygon1, polygon2) {
      // is polygon1 a part of polygon2? (with at least one edge in common)
      if (polygonsEdgeInCommon(polygon1, polygon2) == false) return false;
      if (polygonInsidePolygon(polygon1.pos, polygon2.pos, true)) return true;
      return false;
    }
    function polygonsEdgeInCommon(polygon1, polygon2) {
      for (var e1 = 0; e1 < polygon1.edgeIds.length; e1++) {
        for (var e2 = 0; e2 < polygon2.edgeIds.length; e2++) {
          if (polygon1.edgeIds[e1] == polygon2.edgeIds[e2]) return true;
        }
      }
      return false;
    }
        ...
        for (var j = 0; j < polygon2.length; j++) {
          ...
          if (lineSegmentsIntersectionTest(line1, line2) == true) {
            ...
            if (
              p3 == true &&
              p4 == false &&
              pointInPolygon(line1[1], polygon2, true) == false
            )
              return false;
            if (
              p3 == false &&
              p4 == true &&
              pointInPolygon(line1[0], polygon2, true) == false
            )
              return false;
          }
        }
    ...
    function polygonsVertexInCommon(polygon1, polygon2) {
      for (var e1 = 0; e1 < polygon1.pos.length; e1++) {
        for (var e2 = 0; e2 < polygon2.pos.length; e2++) {
          if (dist(polygon1.pos[e1], polygon2.pos[e2]) < 0.1) return true;
        }
      }
      return false;
    }
    function pointInPolygon(point, pos, includeBoundary) {
      ...
      for (var i = 0, j = pos.length - 1; i < pos.length; j = i++) {
        if (
          pos[i][1] > point[1] != pos[j][1] > point[1] &&
          point[0] <
            ((pos[j][0] - pos[i][0]) * (point[1] - pos[i][1])) /
              (pos[j][1] - pos[i][1]) +
              pos[i][0]
        ) {
          result = !result;
    ...
    function drawAllEdges(edges, polygons) {
      // for dev only
  ...
  getPolygonOfCircle: function (circle, radianSeperation) {
    if (un(radianSeperation)) radianSeperation = 0.2;
    var polygon = [];
    var a = 2 * Math.PI;
    while (a > radianSeperation) {
      addPos(a);
