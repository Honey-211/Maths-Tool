...
draw.convertTable = function (obj) {
  ...
  if (obj.type == "table") {
    obj.widths = [];
    obj.heights = [];
    for (var x = 0; x < obj.xPos.length - 1; x++)
      obj.widths[x] = obj.xPos[x + 1] - obj.xPos[x];
    for (var y = 0; y < obj.yPos.length - 1; y++)
      obj.heights[y] = obj.yPos[y + 1] - obj.yPos[y];
    obj.type = "table2";
  }
  delete obj.cell;
  delete obj.mInputs;
...
draw.compressPath = function (path) {
  ...
  for (var o = path.obj.length - 1; o >= 0; o--) {
    ...
    if (obj.type == "text2") {
      if (
        (un(obj.text) || textArrayCheckIfEmpty(obj.text)) &&
        un(obj.id) &&
        (un(obj.box) || obj.box.type == "none")
      ) {
        path.obj.splice(o, 1);
        continue;
      }
      ...
      if (!un(obj.align))
        tags.align =
          obj.align == -1 ? "left" : obj.align == 1 ? "right" : "center";
      if (!un(obj.text)) {
      ...
      for (var c = 0; c < cells.length; c++) {
        for (var d = 0; d < deleteCellProperties.length; d++)
          delete cells[c][deleteCellProperties[d]];
        if (!un(cells[c].trigger) && cells[c].trigger.length == 0)
          delete cells[c].trigger;
        if (cells[c].color == "none") delete cells[c].color;
        if (!un(cells[c].box) && cells[c].box.type == "none")
          delete cells[c].box;
        if (
          !un(cells[c].box) &&
          cells[c].box.color == "none" &&
          cells[c].box.borderColor == "none"
        )
          delete cells[c].box;
        if (!un(cells[c].text)) {
    ...
    for (var d = 0; d < deleteProperties.length; d++)
      delete obj[deleteProperties[d]];
    for (var key in obj)
      if (
        key.charAt(0) == "_" ||
        (obj[key] instanceof Array && obj[key].isEmpty() == true)
      )
        delete obj[key];
  }
  delete path.border;
  ...
  function stringify(obj) {
    ...
    if (obj instanceof Array) {
      ...
      if (typeof obj !== "undefined")
        console.log("draw.stringify type not included: ", typeof obj, obj);
    }
    return str;
  }
};
...
draw.divMode = function () {
  ...
  div.setZoom = function (sf) {
    if (un(draw.div.scrollBarWidth))
      draw.div.scrollBarWidth = draw.div.getScrollBarWidth();
...
draw.multiPage = {
  ...
  onscroll: function (e) {
    var p = Math.floor(
      (pages.length * draw.div.scrollTop) / draw.div.scrollHeight
    );
    if (p == pIndex) return;
    draw.multiPage.changePage(p);
  },
...
function drawCanvasPaths() {
  ...
  draw.pathCursorOrder = drawFirst
    .concat(unselected)
    .concat(drawLast)
    .concat(selected)
    .concat(overlay);
  ...
  for (var p = 0; p < draw.pathCursorOrder.length; p++) {
    ...
    for (var o = 0; o < path.obj.length; o++) {
      ...
      if (typeof draw[obj.type].drawUnderlay === "function")
        draw[obj.type].drawUnderlay(draw.drawCanvas[0].ctx, obj, path);
    }
  }
  ...
  if (draw.mode == "interact" && draw.appearMode == true) {
    // draw appear button positions
    ...
    for (var p = 0; p < draw.path.length; p++) {
      ...
      if (
        typeof path.appear.visible == "function" &&
        path.appear.visible(path) === false
      )
        continue;
  ...
  if (
    draw.mode === "edit" &&
    !un(window.pathList) &&
    typeof window.pathList.update == "function"
  )
    window.pathList.update();
  draw.ids.update();
...
function drawSelectedPaths(drawOverlay) {
  ...
  if (draw.drawing == true && draw.path.last().selected !== true)
    selected.push(draw.path.last());
  if (selected.length > 0) drawPathsToCanvas(draw.drawCanvas[1], selected);
...
function drawPathsToCanvas(
  ...
  for (var p = 0; p < paths.length; p++) {
    ...
    if (un(path) || (drawAllPaths == false && getPathVis(path) == false))
      continue;
    if (path.notesOverlay === true && draw.notesOverlay !== true) continue;
    if (!un(path.rotation)) {
      ctx.translate(
        path.border[0] + 0.5 * path.border[2],
        path.border[1] + 0.5 * path.border[3]
      );
  ...
  if (typeof path.trigger == "object") {
    for (var l = 0; l <= draw.triggerNum; l++) {
      if (typeof path.trigger[l] == "boolean" && path.trigger[l] == true) {
        ...
        typeof path.trigger[l] == "boolean" &&
        path.trigger[l] == false
      ) {
        vis = false;
      }
    }
...
function drawSelectCanvas() {
  ...
  for (var p = 0; p < paths.length; p++) {
    ...
    if (
      !un(draw.controlPanel) &&
      paths.length == 1 &&
      path.obj.length == 1 &&
      ["three", "qBox"].indexOf(path.obj[0].type) > -1
    )
      draw.controlPanel.draw(ctx, path.obj[0], path);
...
function drawSelectCanvas2() {
  ...
  if (draw.mode == "edit" && draw.appearMode == true) {
    // draw appear button positions
    for (var p = 0; p < draw.path.length; p++) {
      var path = draw.path[p];
      ...
      if (path.selected == true) {
        ...
        drawStar({
          ctx: ctx,
          center: [l + 0.5 * w, t + 0.5 * h],
          radius: 0.4 * Math.min(w, h),
          points: 5,
        });
        ctx.fill();
      } else {
    ...
    switch (buttons[i].buttonType) {
      case "isInput-type":
        text({
          ...
          rect: [l, t, w, h],
          align: [0, 0],
          text: ["<<fontSize:16>>" + buttons[i].type],
          box: { type: "loose", color: "#F96", borderColor: "#F96" },
        });
        break;
      case "isInput-dragArea-snapToggle":
        ...
        text({
          ...
          align: [0, 0],
          text: [
            "<<color:" + textColor + ">><<fontSize:" + h * 0.75 + ">>snap",
          ],
        });
        break;
      case "isInput-drag-value":
        ...
        var value =
          path.isInput.value !== ""
            ? path.isInput.value
            : "<<italic:true>>no value";
        text({
          ctx: ctx,
          rect: [l, t, w, h],
          align: [0, 0],
          text: ["<<fontSize:16>>" + value],
          box: { type: "loose", color: "#F96", borderColor: "#F96" },
        });
        break;
      case "isInput-drag-shuffleToggle":
        ...
        text({
          ...
          align: [0, 0],
          text: [
            "<<color:" + textColor + ">><<fontSize:" + h * 0.75 + ">>shuffle",
          ],
        });
        break;
      ...
      case "select-input-cellToggle":
        var cell =
          draw.path[buttons[i].pathNum].obj[0].cells[buttons[i].row][
            buttons[i].col
          ];
        if (cell.ans == true) {
          ctx.fillStyle = "#060";
      ...
      case "text-input-ansInfo":
        text({
          ...
          rect: [l, t, w, h],
          align: [0, 0],
          text: ["<<fontSize:16>>" + buttons[i].text],
        });
        break;
      ...
      case "text-input-algText":
        ...
        text({
          ...
          rect: [l, t, w, h],
          align: [0, 0],
          text: ["<<fontSize:16>><<font:algebra>>x"],
        });
        break;
      case "text-input-num":
        if (buttons[i].num) {
      ...
      case "text2-algPadding":
        ...
        if (un(obj.algPadding)) {
          ...
          text({
            ...
            rect: [l, t, w, h * 0.9],
            align: [0, 0],
            text: ["<<fontSize:13>>" + obj.algPadding],
            algPadding: 0,
          });
        }
        break;
      ...
      case "triggerTableCell":
        ...
        if (draw.triggerEnabled == true || draw.appearMode == true) {
          if (buttons[i].buttonType == "appear") {
            ...
            if (!un(path.appear) && path.appear.reversible === true)
              reversible = true;
            ...
            if (buttons[i].buttonType == "trigger") {
              ...
              var trigger =
                path.obj[0].cells[buttons[i].r][buttons[i].c].trigger;
            }
            var vis = true;
      ...
      case "angle-showLines":
        if (
          typeof path.obj[0].drawLines == "undefined" ||
          path.obj[0].drawLines == true
        ) {
      ...
      case "angle-showAngle":
        ...
        text({
          ctx: ctx,
          textArray: ["<<fontSize:" + w / 2 + ">>37" + degrees],
          left: l,
          top: t,
          width: w,
          height: h,
      ...
      case "grid-function":
        ...
        text({
          ctx: ctx,
          textArray: ["<<font:algebra>><<fontSize:" + w * 0.5 + ">>f(x)"],
          left: l,
          top: t,
          width: w,
          height: h,
      ...
      case "grid-showGrid":
        if (path.obj.length == 1 && path.obj[0].type == "grid") {
          if (
            typeof path.obj[0].showGrid == "undefined" ||
            path.obj[0].showGrid == true
          ) {
      ...
      case "grid-showScales":
        if (
          path.obj.length == 1 &&
          (path.obj[0].type == "grid" || path.obj[0].type == "numberline")
        ) {
          if (
            typeof path.obj[0].showScales == "undefined" ||
            path.obj[0].showScales == true
          ) {
            ctx.fillStyle = colorA("#F96", 0.5);
            ctx.fillRect(l, t, w, h);
          }
      ...
      case "grid-showLabels":
        if (path.obj.length == 1 && path.obj[0].type == "grid") {
          if (
            typeof path.obj[0].showLabels == "undefined" ||
            path.obj[0].showLabels == true
          ) {
            ctx.fillStyle = colorA("#F96", 0.5);
            ctx.fillRect(l, t, w, h);
          }
          ...
          text({
            ctx: ctx,
            textArray: ["<<fontSize:" + w / 2 + ">><<font:algebra>>xy"],
            left: l,
            top: t,
            width: w,
            height: h,
      ...
      case "grid-showBorder":
        if (path.obj.length == 1 && path.obj[0].type == "grid") {
          if (
            typeof path.obj[0].showBorder == "undefined" ||
            path.obj[0].showBorder == true
          ) {
            ctx.fillStyle = colorA("#F96", 0.5);
            ctx.fillRect(l, t, w, h);
          }
      ...
      case "grid-originStyle":
        if (path.obj.length == 1 && path.obj[0].type == "grid") {
          if (
            typeof path.obj[0].originStyle == "undefined" ||
            path.obj[0].originStyle == "circle"
          ) {
            ctx.fillStyle = colorA("#F96", 0.5);
            ctx.fillRect(l, t, w, h);
          }
          ...
          text({
            ctx: ctx,
            textArray: ["<<fontSize:" + w / 3 + ">><<font:algebra>>(0,0)"],
            left: l,
            top: t,
            width: w,
            height: h,
...
function calcCursorPositions() {
  ...
  pos.push({
    shape: "rect",
    dims: [0, 0, draw.drawArea[2], draw.drawArea[3]],
  ...
  switch (draw.drawMode) {
    ...
    case "floodFill":
      pos.push({
        shape: "rect",
        dims: draw.drawArea,
        cursor: draw.cursors.fill,
        func: drawClickFloodFillClick,
        highlight: -1,
      });
      break;
    case "pen":
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        if (path.selected == true) {
          ...
          pos.push({
            ...
            cursor: draw.cursors.move1,
            func: drawClickGridStartDrag,
            pathNum: i,
          });
          pos.push({
            shape: "rect",
            dims: [obj.xZero - 15, obj.top, 30, obj.height],
            cursor: draw.cursors.ns,
            func: drawClickGridStartRescaleY,
            pathNum: i,
          });
          pos.push({
            shape: "rect",
            dims: [obj.left, obj.yZero - 15, obj.width, 30],
            cursor: draw.cursors.ew,
            func: drawClickGridStartRescaleX,
            pathNum: i,
          });
            ...
            for (var j = 0; j < path.borderButtons.length; j++) {
              if (
                [
                  ...
                  "grid-yMinorPlus",
                  "grid-yMinorMinus",
                ].indexOf(path.borderButtons[j].buttonType) > -1
              ) {
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        if (path.selected == true) {
          ...
          pos.push({
            ...
            func: drawClickGridPlotPoint,
            pathNum: i,
          });

          if (path.borderButtons !== "undefined") {
            for (var j = 0; j < path.borderButtons.length; j++) {
              if (path.borderButtons[j].buttonType == "grid-plot")
                pos.push(path.borderButtons[j]);
            }
          }
        }
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        if (path.selected == true) {
          ...
          pos.push({
            ...
            dims: [obj.left, obj.top, obj.width, obj.height],
            cursor: draw.cursors.pointer,
            func: drawClickGridStartLineSegment,
            pathNum: i,
          });

          if (path.borderButtons !== "undefined") {
            for (var j = 0; j < path.borderButtons.length; j++) {
              if (path.borderButtons[j].buttonType == "grid-lineSegment")
                pos.push(path.borderButtons[j]);
            }
          }
        }
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        if (path.selected == true) {
          ...
          pos.push({
            ...
            dims: [obj.left, obj.top, obj.width, obj.height],
            cursor: draw.cursors.pointer,
            func: drawClickGridStartLine,
            pathNum: i,
          });

          if (path.borderButtons !== "undefined") {
            for (var j = 0; j < path.borderButtons.length; j++) {
              if (path.borderButtons[j].buttonType == "grid-line")
                pos.push(path.borderButtons[j]);
            }
          }
        }
    ...
    case "textEdit":
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        for (var j = 0; j < path.obj.length; j++) {
          ...
          if (
            !un(draw[obj.type]) &&
            !un(draw[obj.type].getCursorPositionsUnselected)
          ) {
            pos = pos.concat(
              draw[obj.type].getCursorPositionsUnselected(obj, i)
            );
            continue;
          }
          switch (obj.type) {
            case "pen":
              pos.push({
                shape: "path",
                dims: [obj.pos, draw.selectTolerance],
                cursor: draw.cursors.pointer,
            ...
            case "line":
              pos.push({
                shape: "line",
                dims: [obj.startPos, obj.finPos, draw.selectTolerance],
                cursor: draw.cursors.pointer,
                func: drawClickSelect,
                pathNum: i,
                highlight: -1,
            ...
            case "square":
              ...
              if (obj.fillColor !== "none") {
                ...
                pos.push({
                  shape: "openRect",
                  dims: [x1, y1, x2 - x1, y2 - y1, draw.selectTolerance],
                  cursor: draw.cursors.pointer,
                  func: drawClickSelect,
                  pathNum: i,
                  highlight: -1,
            ...
            case "curve":
              pos.push({
                shape: "path",
                dims: [obj.points, draw.selectTolerance],
                cursor: draw.cursors.pointer,
                func: drawClickSelect,
                pathNum: i,
                highlight: 1,
            ...
            default:
              if (obj.type === "grid" && obj._interactMode === "move") {
                ...
                pos.push({
                  shape: "rect",
                  dims: [obj._left, obj._top, obj._width, obj._height],
                  cursor: draw.cursors.pointer,
                  func: drawClickSelect,
                  pathNum: i,
                  highlight: -1,
                ...
                pos.push({
                  shape: "rect",
                  dims: draw[obj.type].getRect(obj),
                  cursor: draw.cursors.pointer,
                  func: drawClickSelect,
                  pathNum: i,
                  highlight: -1,
      ...
      for (var i = 0; i < draw.path.length; i++) {
        ...
        if (path.selected == true) {
          ...
          if (path.obj.length == 1) {
            ...
            if (
              !un(draw[obj.type]) &&
              !un(draw[obj.type].getCursorPositionsSelected)
            ) {
              pos = pos.concat(
                draw[obj.type].getCursorPositionsSelected(obj, i)
              );
            }
          }
    ...
    case "textStart":
      pos.push({
        ...
        dims: draw.drawArea,
        cursor: "text",
        func: draw.text2.start,
        pathNum: draw.path.length,
      });
      break;
    case "table":
    case "tableChange":
  ...
  if (draw.mode == "interact") {
    if (!un(draw.pathCursorOrder)) {
      for (var p = 0; p < draw.pathCursorOrder.length; p++) {
        ...
        if (
          pathInteract.allowInteraction !== false &&
          pathInteract.disabled !== true &&
          pathInteract._disabled !== true
        ) {
          if (typeof pathInteract.dragPathCircle == "string") {
            ...
            if (circleObj !== false) {
              pos.push({
                shape: "rect",
                dims: clone(path.border.slice(0, 4)),
                cursor: draw.cursors.move1,
                func: draw.interact.dragStart,
                interact: true,
                path: path,
                dragType: "circle",
            ...
            var lineSegmentObj = draw.getObjById(
              pathInteract.dragPathLineSegment
            );
            if (lineSegmentObj !== false) {
              pos.push({
                ...
                interact: true,
                path: path,
                dragType: "lineSegment",
                lineSegment: lineSegmentObj,
              });
            }
          } else if (pathInteract.draggable == true) {
            pos.push({
              shape: "rect",
              dims: clone(path.tightBorder.slice(0, 4)),
              cursor: draw.cursors.move1,
              func: draw.interact.dragStart,
              interact: true,
              path: path,
          ...
          if (pathInteract.cubeBuilding == "build") {
            var positions = draw.three.cubeDrawing.getCursorPositionsBuild(
              path.obj[0]
            );
            draw.color = "#000";
            draw.cursors.update();
            ...
            var positions = draw.three.cubeDrawing.getCursorPositionsRemove(
              path.obj[0]
            );
            draw.color = "#F00";
            draw.cursors.update();
            ...
            pos = pos.concat(
              draw.three.getCursorPositionsSelected(path.obj[0], p, true)
            );
          }
          if (typeof pathInteract.click == "function") {
            pos.push({
              ...
              dims: clone(path.border.slice(0, 4)),
              cursor: draw.cursors.pointer,
              func: draw.interact.click,
              click: pathInteract.click,
              interact: true,
              path: path,
              obj: path.obj[0],
            });
        ...
        for (var o2 = 0; o2 < path.obj.length; o2++) {
          ...
          if (!un(path.interact) && path.interact.overlay == true)
            isOverlayObj = true;
          if (!un(obj.interact) && obj.interact.overlay == true)
            isOverlayObj = true;
          ...
          if (typeof draw[obj.type].getCursorPositionsInteract == "function") {
            pos = pos.concat(
              draw[obj.type].getCursorPositionsInteract(obj, path, p)
            );
          }
          ...
          if (obj.type == "slider") {
            ...
            objInteract.allowInteraction === false ||
            objInteract.disabled === true ||
            objInteract._disabled === true
          ) {
            continue;
      ...
      for (var p = 0; p < draw.path.length; p++) {
        ...
        if (
          ...
          continue;
        if (
          typeof path.appear.visible == "string" &&
          getPathVis(path.appear.visible) == false
        )
          continue;
        if (un(path.appear.pos)) {
          if (un(path.border)) updateBorder(path);
    ...
    for (var p = 0; p < draw.path.length; p++) {
      // add cursor positions for overlay buttons
      var path = draw.path[p];
      for (var o = 0; o < path.obj.length; o++) {
        var obj = path.obj[o];
        var isOverlayObj = false;
        ...
        if (obj.type == "colorPicker" && draw.colorSelectVisible == false)
          continue;
        if (
          obj.type == "lineWidthSelect" &&
        ...
        if (obj.type == "slider") {
          ...
          !un(obj.interact) &&
          !un(obj.interact.click) &&
          obj.interact.click.length > 0
        ) {
          pos.push({
            shape: "rect",
            dims: draw[obj.type].getRect(obj),
          ...
          !un(obj.interact) &&
          typeof obj.interact.click == "function"
        ) {
          pos.push({
            shape: "rect",
            dims: draw[obj.type].getRect(obj),
    ...
    for (var p = 0; p < draw.path.length; p++) {
      ...
      if (!un(path.isInput) && path.isInput._mode === "addAnswers") {
        if (path.isInput.type === "drag") {
          pos.push({
            shape: "rect",
            dims: path.tightBorder.slice(0, 4),
            cursor: draw.cursors.move1,
            func: draw.interact.dragStart,
            path: draw.path[p],
          });
    ...
    for (var p = 0; p < draw.path.length; p++) {
      // add cursor positions for appear buttons
      var path = draw.path[p];
      if (un(path.appear)) continue;
      if (un(path.appear.pos)) {
        var l = path.border[0] + 0.5 * path.border[2] - 20;
      ...
      pos.push({
        buttonType: "appear-dont-draw",
        shape: "rect",
        dims: [l, t, 40, 40],
        cursor: draw.cursors.move1,
        func: drawClickAppearMoveStart,
        pathNum: p,
      });
    }
  }
  if (!un(draw.controlPanel))
...
function drawCanvasStart(e) {
  ...
  if (!un(draw.currCursor)) {
    if (
      un(draw.currCursor.obj) ||
      (draw.currCursor.obj.type !== "colorPicker" &&
        draw.currCursor.obj.type !== "buttonColorPicker")
    ) {
      draw.colorSelectVisible = false;
    }
    if (
      un(draw.currCursor.obj) ||
      (draw.currCursor.obj.type !== "lineWidthSelect" &&
        draw.currCursor.obj.type !== "buttonLineWidthPicker")
    ) {
      draw.lineWidthSelectVisible = false;
    }
...
draw.cursorPosHitTest = function (pos, x, y) {
  ...
  if (pos.shape == "table") {
    ...
    (pos.shape == "openCircle" &&
      dist(x, y, dims[0], dims[1]) >= dims[2] - dims[3] &&
      dist(x, y, dims[0], dims[1]) <= dims[2] + dims[3]) ||
    (pos.shape == "ellipse" &&
      isPointInEllipse([x, y], [dims[0], dims[1]], dims[2], dims[3]) == true) ||
    (pos.shape == "openEllipse" &&
      isPointOnEllipse([x, y], [dims[0], dims[1]], dims[2], dims[3], dims[4]) ==
        true) ||
    (pos.shape == "line" &&
      distancePointToLineSegment([x, y], dims[0], dims[1]) < dims[2]) ||
...
function cursorPosHighlight(clr) {
  ...
  switch (c.shape) {
    ...
    case "sector":
      ...
      ctx.lineTo(
        c.dims[0] + c.dims[2] * Math.cos(c.dims[3]),
        c.dims[1] + c.dims[2] * Math.sin(c.dims[3])
      );
...
function updateBorder(path, override) {
  ...
  if (
    ...
    ([
      ...
      "text2",
      "table2",
    ].indexOf(path.obj[0].type) > -1 ||
      (path.obj[0].type == "angle" && un(path.obj[0].d)) ||
      (!un(draw[path.obj[0].type]) && draw[path.obj[0].type].resizable == true))
  ) {
    ...
    buttons.push({
      buttonType: "resize-path",
      shape: "rect",
      dims: [x2 - 20, y2 - 20, 20, 20],
      cursor: draw.cursors.nw,
      func: drawClickStartResizePath,
      pathNum: pathNum,
    });
  }
  ...
  if (
    path.obj.length == 1 &&
    !un(draw[path.obj[0].type]) &&
    !un(draw[path.obj[0].type].getButtons)
  ) {
    buttons = buttons.concat(
      draw[path.obj[0].type].getButtons(x1, y1, x2, y2, pathNum, path)
    );
  }

  if (!un(path.isInput)) {
    buttons.push({
      buttonType: "isInput-type",
      shape: "rect",
      dims: [(x1 + x2) / 2 - 40, y1, 80, 20],
      cursor: draw.cursors.default,
      func: function () {},
      pathNum: pathNum,
      type: path.isInput.type,
    });

    if (
      path.isInput.type == "text" &&
      path.obj.length == 1 &&
      path.obj[0].type == "text2"
    ) {
    } else if (
      !un(draw.selectInput) &&
      ...
      buttons.push({
        buttonType: "isInput-drag-value",
        shape: "rect",
        dims: [x1 + 20, y2 - 20, 80, 20],
        cursor: draw.cursors.pointer,
        func: draw.isInput.dragSetValue,
        pathNum: pathNum,
      });

      buttons.push({
        buttonType: "isInput-drag-shuffleToggle",
        shape: "rect",
        dims: [x2 - 80, y2 - 20, 60, 20],
        cursor: draw.cursors.pointer,
        func: draw.isInput.dragShuffleToggle,
        pathNum: pathNum,
      });
      ...
      buttons.push({
        buttonType: "isInput-dragArea-snapToggle",
        shape: "rect",
        dims: [x2 - 80, y2 - 20, 60, 20],
        cursor: draw.cursors.pointer,
        func: draw.isInput.dragAreaSnapToggle,
        pathNum: pathNum,
      });
    }
  }
  ...
  buttons.push({
    buttonType: "orderPlus",
    shape: "rect",
    dims: [x1, y2 - 40, 20, 20],
    cursor: draw.cursors.pointer,
    func: drawClickOrderPlus,
    pathNum: pathNum,
  });
  buttons.push({
    buttonType: "orderMinus",
    shape: "rect",
    dims: [x1, y2 - 20, 20, 20],
    cursor: draw.cursors.pointer,
    func: drawClickOrderMinus,
    pathNum: pathNum,
  });
  ...
  if (draw.appearMode == true) {
    ...
    buttons.push({
      ...
      dims: [x1, y1, 20, 20],
      cursor: draw.cursors.pointer,
      func: drawClickTrigger,
      pathNum: pathNum,
    });
  }

  buttons.push({
    buttonType: "toggle-draggable",
    shape: "rect",
    dims: [x1 + 20, y1, 20, 20],
    cursor: draw.cursors.pointer,
    func: draw.togglePathDraggable,
    pathNum: pathNum,
    path: path,
  });

  buttons.push({
    buttonType: "toggle-notesOverlay",
    shape: "rect",
    dims: [x1 + 40, y1, 20, 20],
    cursor: draw.cursors.pointer,
    func: draw.toggleNotesOverlay,
    pathNum: pathNum,
    path: path,
  });
...
draw.getPathFunctions = function (path) {
  ...
  for (var key in path)
    if (typeof path[key] === "function")
      funcs.push([path, key, "path." + key + "()"]);
  if (!un(path.interact)) {
    for (var key in path.interact)
      if (typeof path.interact[key] === "function")
        funcs.push([path.interact, key, "path.interact." + key + "()"]);
  }
  if (!un(path.isInput)) {
    for (var key in path.isInput)
      if (typeof path.isInput[key] === "function")
        funcs.push([path.isInput, key, "path.isInput." + key + "()"]);
  }
  return funcs;
};
draw.getObjFunctions = function (obj) {
  var funcs = [];
  for (var key in obj)
    if (typeof obj[key] === "function") funcs.push([obj, key, "obj." + key]);
  if (!un(obj.interact)) {
    for (var key in obj.interact)
      if (typeof obj.interact[key] === "function")
        funcs.push([obj.interact, key, "obj.interact." + key + "()"]);
  }
  return funcs;
};
...
draw.undo = {
  ...
  saveState: function () {
    ...
    if (
      un(draw.undo.undoStack[0]) ||
      draw.undo.pathsEqual(draw.path, draw.undo.undoStack[0]) == false
    ) {
  ...
  undo: function () {
    while (
      !un(draw.undo.undoStack[0]) &&
      draw.undo.pathsEqual(draw.path, draw.undo.undoStack[0])
    ) {
  ...
  redo: function () {
    while (
      !un(draw.undo.redoStack[0]) &&
      draw.undo.pathsEqual(draw.path, draw.undo.redoStack[0])
    ) {
  ...
  pathsEqual: function (a, b) {
    ...
    for (var p = 0; p < a.length; p++) {
      ...
      if (
        (!un(path1.appear) || !un(path2.appear)) &&
        isEqual(path1.appear, path2.appear) == false
      )
        return false;
  ...
  if (e.getModifierState("Control")) {
    ...
    if (e.key == "z" || e.key == "Z") {
      ...
      if (typeof textMenu !== "undefined")
        textMenu.applyValue("bold", !textEdit.menu.currentStyle.bold);
    } else if (e.key == "i" || e.key == "I") {
      e.preventDefault();
      if (typeof textMenu !== "undefined")
        textMenu.applyValue("italic", !textEdit.menu.currentStyle.italic);
...
draw.keyboard = {
  ...
  setPosition: function (pos) {
    ...
    for (i = 0; i < key1[pageIndex].length; i++) {
      if (boolean(key1[pageIndex][i].static, false) == false) {
        ...
        resizeCanvas2(
          key1[pageIndex][i],
          key1Data[pageIndex][i][100],
          key1Data[pageIndex][i][101]
        );
      }
    }
  },
  moveAwayFromTextInput: function () {
    ...
    if (textRect[0] >= keyboardRect[2]) {
      draw.keyboard.setPosition([
        textRect[0] - keyboardRect[2],
        keyboardRect[1],
      ]);
...
function drawClickStartDraw() {
  ...
  switch (draw.drawMode) {
    ...
    case "polygon":
      ...
      if (draw.gridSnap == true) {
        pos = [
          roundToNearest(startX, draw.gridSnapSize),
          roundToNearest(startY, draw.gridSnapSize),
        ];
      }
      if (snapToObj2On) {
...
function lineDrawStop(e) {
  ...
  var obj = path.obj[0];
  if (
    typeof obj.finPos == "undefined" ||
    getDist(obj.startPos, obj.finPos) < 5
  ) {
    removePathObject(draw.path.length - 1);
...
function rectDrawStop(e) {
  ...
  if (
    typeof draw.path[draw.path.length - 1].obj[0].finPos == "undefined" ||
    dist(
      draw.path[draw.path.length - 1].obj[0].startPos[0],
      draw.path[draw.path.length - 1].obj[0].startPos[1],
      draw.path[draw.path.length - 1].obj[0].finPos[0],
      draw.path[draw.path.length - 1].obj[0].finPos[1]
    ) < 5
  ) {
    draw.path.pop();
  } else if (draw.gridSnap == true) {
    draw.path[draw.path.length - 1].obj[0].finPos[0] = roundToNearest(
      draw.path[draw.path.length - 1].obj[0].finPos[0],
      draw.gridSnapSize
    );
    draw.path[draw.path.length - 1].obj[0].finPos[1] = roundToNearest(
      draw.path[draw.path.length - 1].obj[0].finPos[1],
      draw.gridSnapSize
    );
  }
...
function polygonDrawMove(e) {
  ...
  if (shiftOn == true) {
    ...
    if (Math.abs(dx / dy) >= 1) {
      ...
      var newPoint = [
        Math.min(
          Math.max(x, draw.drawArea[0]),
          draw.drawArea[0] + draw.drawArea[2]
        ),
        obj.points[obj.points.length - 2][1],
      ];
    } else {
      // vertical line
      var newPoint = [
        obj.points[obj.points.length - 2][0],
        Math.min(
          Math.max(y, draw.drawArea[1]),
          draw.drawArea[1] + draw.drawArea[3]
        ),
      ];
    }
  } else {
...
function polygonDrawStop(e) {
  ...
  if (
    obj.points.length > 2 &&
    dist(x, y, obj.points[0][0], obj.points[0][1]) < draw.selectTolerance * 2
  ) {
    // close polygon
...
function circleDrawMove(e) {
  ...
  var x = draw.mouse[0];
  var y = draw.mouse[1];
  draw.path[draw.path.length - 1].obj[0].radius = Math.abs(
    dist(
      x,
      y,
      draw.path[draw.path.length - 1].obj[0].center[0],
      draw.path[draw.path.length - 1].obj[0].center[1]
    )
  );
...
function circleDrawStop(e) {
  ...
  if (draw.gridSnap == true) {
    draw.path[draw.path.length - 1].obj[0].radius = roundToNearest(
      draw.path[draw.path.length - 1].obj[0].radius,
      draw.gridSnapSize
    );
  }
...
function ellipseDrawMove(e) {
  ...
  var x = draw.mouse[0];
  var y = draw.mouse[1];
  draw.path[draw.path.length - 1].obj[0].radiusX = Math.abs(
    x - draw.path[draw.path.length - 1].obj[0].center[0]
  );
  draw.path[draw.path.length - 1].obj[0].radiusY = Math.abs(
    y - draw.path[draw.path.length - 1].obj[0].center[1]
  );
...
function ellipseDrawStop(e) {
  ...
  if (draw.gridSnap == true) {
    draw.path[draw.path.length - 1].obj[0].radiusX = roundToNearest(
      draw.path[draw.path.length - 1].obj[0].radiusX,
      draw.gridSnapSize
    );
    draw.path[draw.path.length - 1].obj[0].radiusY = roundToNearest(
      draw.path[draw.path.length - 1].obj[0].radiusY,
      draw.gridSnapSize
    );
  }
...
draw.interact = {
  standardProperties: {
    ...
    path: [
      ...
      {
        key: "checkAllowDrag",
        value: function (path, pos) {},
        condition: function (path) {
          var pathInteract = draw.getPathInteract(path);
          return (
            pathInteract.draggable === true ||
            !un(pathInteract.dragPathCircle) ||
            !un(pathInteract.dragPathLineSegment)
          );
        },
      },
      {
      ...
      {
        key: "onDrop",
        value: function (dragAreaObj, dragPath) {},
        condition: function (path) {
          var pathInteract = draw.getPathInteract(path);
          return pathInteract.draggable === true;
        },
  ...
  appear: function () {
    ...
    if (
      path._visible == true &&
      !un(path.appear) &&
      typeof path.appear.onappear == "function"
    ) {
    ...
    if (
      path._visible == false &&
      !un(path.appear) &&
      typeof path.appear.ondisappear == "function"
    ) {
  ...
  buttons: {
    ...
    tickCrossTableHoriz: {
      ...
      click: function () {
        ...
        var obj = {
          ...
          widths: [150, 150],
          heights: [100],
          text: { font: "Arial", size: 28, color: "#000" },
    ...
    drag: {
      drawButton: function (ctx, size) {
        ...
        text({
          ctx: ctx,
          text: ["<<fontSize:13>><<bold:true>><<font:algebra>>12"],
          align: [0, 0],
          rect: [10, 10, 25, 25],
          box: {
            type: "loose",
      ...
      click: function () {
        ...
        for (var p = 0; p < paths.length; p++) {
          ...
          if (obj.type == "text2") {
            var color =
              !un(obj.box) && !un(obj.box.color) ? obj.box.color : "#CFF";
            obj.box = {
              type: "loose",
              borderWidth: 4,
              borderColor: "#000",
    ...
    dragArea: {
      ...
      click: function () {
        ...
        for (var p = 0; p < paths.length; p++) {
          ...
          if (obj.type == "text2") {
            var color =
              !un(obj.box) && !un(obj.box.color) ? obj.box.color : "#FFF";
            obj.box = {
              type: "loose",
              borderWidth: 4,
              borderColor: "#333",
          ...
          obj: [
            {
              type: "text2",
              align: [0, 0],
              text: ["<<fontSize:36>><<font:Hobo>>Check Answer"],
              rect: [20, 700 - 80, 260, 60],
              box: {
                type: "loose",
                color: "#6F9",
                borderColor: "#000",
    ...
    feedback: {
      ...
      click: function () {
        draw.path.push({
          obj: [{ type: "feedback", rect: [300, 700 - 80, 880, 60] }],
          selected: true,
        });
        updateBorder(draw.path.last());
        drawCanvasPaths();
  ...
  processFuncObject: function (obj, funcObject) {
    ...
    var obj2 =
      un(funcObject.id) || funcObject.id == "self"
        ? obj
        : draw.getObjById(funcObject.id);
    ...
    if (!un(funcObject.value)) {
      ...
      var conditionObj =
        un(funcObject.condition.id) || funcObject.condition.id == "self"
          ? obj2
          : draw.getObjById(funcObject.condition.id);
      var conditionProperty = draw.interact.getPropertyObject(
        conditionObj,
        funcObject.condition.property
      );
  ...
  getPropertyObject: function (obj, property) {
    if (!un(draw.obj.properties[property])) {
      ...
      !un(draw[obj.type].properties) &&
      !un(draw[obj.type].properties[property])
    ) {
  ...
  dragStart: function (e) {
    ...
    draw.drag.offset = [
      draw.mouse[0] - path.tightBorder[0],
      draw.mouse[1] - path.tightBorder[1],
    ];
    ...
    draw.objs(function (obj) {
      if (
        !un(obj.interact) &&
        (obj.interact.type == "dragArea" || obj.interact.dragArea == true)
      ) {
        process(obj.interact);
      } else if (
        !un(obj.isInput) &&
        (obj.isInput.type == "dragArea" || obj.isInput.dragArea == true)
      ) {
        process(obj.isInput);
      }
  ...
  dragMove: function (e) {
    ...
    if (draw.drag.dragType == "circle") {
      ...
      if (
        !un(circle.interact) &&
        typeof circle.interact.checkAllowDrag == "function" &&
        circle.interact.checkAllowDrag(circle, obj, draw.mouse) == false
      )
        return;
      ...
      var pos = closestPointOnLineSegment(
        [draw.mouse[0], draw.mouse[1]],
        draw.drag.lineSegment.startPos,
        draw.drag.lineSegment.finPos
      );
      var dx = pos[0] - (p.border[0] + 0.5 * p.border[2]);
      var dy = pos[1] - (p.border[1] + 0.5 * p.border[3]);
      repositionPath(p, dx, dy);
      ...
      if (
        !un(obj.interact) &&
        typeof obj.interact.checkAllowDrag == "function" &&
        obj.interact.checkAllowDrag(obj, pos) == false
      )
        return;
      if (
        !un(path.interact) &&
        typeof path.interact.checkAllowDrag == "function" &&
        path.interact.checkAllowDrag(path, pos) == false
      )
        return;
      ...
      if (!un(path.interact) && !un(path.interact.dragRect)) {
        ...
        pos2[0] = Math.min(
          pos2[0],
          dragRect[0] + dragRect[2] - path.tightBorder[2]
        );
        pos2[1] = Math.max(pos2[1], dragRect[1]);
        pos2[1] = Math.min(
          pos2[1],
          dragRect[1] + dragRect[3] - path.tightBorder[3]
        );
      } else {
        pos2[0] = Math.max(pos2[0], draw.drawArea[0]);
        pos2[0] = Math.min(
          pos2[0],
          draw.drawArea[0] + draw.drawArea[2] - path.tightBorder[2]
        );
        pos2[1] = Math.max(pos2[1], draw.drawArea[1]);
        pos2[1] = Math.min(
          pos2[1],
          draw.drawArea[1] + draw.drawArea[3] - path.tightBorder[3]
        );
      }
      var posRel = [
        pos2[0] - draw.drag.startPos[0],
        pos2[1] - draw.drag.startPos[1],
      ];
      resizeCanvas2(
        draw.selectedCanvas,
        (posRel[0] + draw.drawRelPos[0]) * draw.scale,
        (posRel[1] + draw.drawRelPos[1]) * draw.scale
      );
      ...
      var pos4 = [
        path._dragPos[0] + 0.5 * path.tightBorder[2],
        path._dragPos[1] + 0.5 * path.tightBorder[3],
      ]; // centre of dragObj
      delete draw.drag.path.interact._dragAreaHit;
      for (d = 0; d < draw.drag.dragAreas.length; d++) {
        var dragArea = draw.drag.dragAreas[d];
        if (
          dist(pos4[0], pos4[1], dragArea.center[0], dragArea.center[1]) < 40
        ) {
          ...
          if (dragArea.snap === true) {
            ...
            var pos3 = [
              border2[0] + 0.5 * border2[2],
              border2[1] + 0.5 * border2[3],
            ]; // center of dragArea
            var posNew = [
              pos3[0] - 0.5 * path.tightBorder[2],
              pos3[1] - 0.5 * path.tightBorder[3],
            ];
            var posRel = [
              posNew[0] - draw.drag.startPos[0],
              posNew[1] - draw.drag.startPos[1],
            ];
            resizeCanvas2(
              draw.selectedCanvas,
              (posRel[0] + draw.drawRelPos[0]) * draw.scale,
  ...
  dragStop: function (e) {
    ...
    if (
      !un(path.interact._dragAreaHit) &&
      !un(path.interact._dragAreaHit.obj) &&
      !un(path.interact._dragAreaHit.obj.interact) &&
      !un(path.interact._dragAreaHit.obj.interact.value)
    ) {
      if (
        path.interact._dragAreaHit.obj.interact.value == path.interact.value
      ) {
    ...
    if (!un(path._dragPos))
      positionPath(path, path._dragPos[0], path._dragPos[1]);
  ...
  checkPage: function (setFeedback) {
    ...
    if (boolean(setFeedback, true) == true)
      draw.interact.setFeedback(feedback, color);
    return checkVars;
  },
};
draw.obj = {
...
draw.animate = function (onmove, onend, onframe) {
  ...
  draw._animation.frame = function () {
    ...
    draw._animation.frameID = window.requestAnimationFrame(
      draw._animation.frame
    );
  };
  draw._animation.stop = function (e) {
    ...
    if (draw.mode === "edit" && !un(draw.contextMenu))
      draw.contextMenu.update();
  };
...
function selectRectMove(e) {
  ...
  for (var i = 0; i < draw.path.length; i++) {
    ...
    if (
      getPathVis(path) &&
      path._center[0] >= selectLeft &&
      path._center[0] <= selectRight &&
      path._center[1] >= selectTop &&
      path._center[1] <= selectBottom
    ) {
      path.selected = true;
    } else {
      path.selected = false;
...
function drawClickStartDragObject() {
  ...
  draw.selectedCanvases.push(
    draw.drawCanvas[1],
    draw.drawCanvas[draw.drawCanvas.length - 1]
  );
  ...
  draw.dragOffsetX =
    draw.mouse[0] - draw.path[draw.selectedPaths[0]].tightBorder[0];
  draw.dragOffsetY =
    draw.mouse[1] - draw.path[draw.selectedPaths[0]].tightBorder[1];
...
function selectDragMove(e) {
  ...
  if (snapToObj2On) {
    ...
    if (obj.type == "circle" || obj.type == "point") {
      ...
      snapPos = canvasPosToDrawPos([
        snapPos[0] - obj.center[0],
        snapPos[1] - obj.center[1],
      ]);
      x = snapPos[0];
      y = snapPos[1];
        ...
        for (var o = 0; o < path2.obj.length; o++) {
          ...
          for (var q = 0; q < obj2._textSnapPos.length; q++) {
            ...
            if (arraysEqual(pos.align, [-1, 0])) {
              if (
                dist(pos.pos[0], pos.pos[1], xLeft, yMiddle) <
                draw.snapTolerance
              ) {
                x += pos.pos[0] - xLeft;
                y += pos.pos[1] - yMiddle;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xLeft, yTop) < draw.snapTolerance
              ) {
                x += pos.pos[0] - xLeft;
                y += pos.pos[1] - yTop;
                found = true;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xCenter, yTop) < draw.snapTolerance
              ) {
                x += pos.pos[0] - xCenter;
                y += pos.pos[1] - yTop;
                found = true;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xRight, yTop) < draw.snapTolerance
              ) {
                x += pos.pos[0] - xRight;
                y += pos.pos[1] - yTop;
                found = true;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xRight, yMiddle) <
                draw.snapTolerance
              ) {
                x += pos.pos[0] - xRight;
                y += pos.pos[1] - yMiddle;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xRight, yBottom) <
                draw.snapTolerance
              ) {
                x += pos.pos[0] - xRight;
                y += pos.pos[1] - yBottom;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xCenter, yBottom) <
                draw.snapTolerance
              ) {
                x += pos.pos[0] - xCenter;
                y += pos.pos[1] - yBottom;
              ...
              if (
                dist(pos.pos[0], pos.pos[1], xLeft, yBottom) <
                draw.snapTolerance
              ) {
                x += pos.pos[0] - xLeft;
                y += pos.pos[1] - yBottom;
    ...
    var snapDiff = snapBorders(
      draw.selectedPaths[0],
      draw.mouse[0] - draw.dragOffsetX,
      draw.mouse[1] - draw.dragOffsetY
    );
    x += snapDiff[0];
    y += snapDiff[1];
    draw.dragSnapped = true;
  ...
  for (var c = 0; c < draw.selectedCanvases.length; c++) {
    if (!un(draw.div)) {
      var w =
        (1200 / 1235) * canvas.getBoundingClientRect().width * draw.div.zoom;
      var h = w * (1700 / 1200);
      draw.selectedCanvases[c].style.left =
        (w / 1200) * (x + draw.drawRelPos[0]) * draw.scale + "px";
      draw.selectedCanvases[c].style.top =
        (h / 1700) * (y + draw.drawRelPos[1]) * draw.scale + "px";
    } else {
      resizeCanvas2(
        draw.selectedCanvases[c],
        (x + draw.drawRelPos[0]) * draw.scale,
        (y + draw.drawRelPos[1]) * draw.scale
      );
    }
  }
  if (!un(draw.gridMenu) && draw.gridMenu.showing == true) {
...
draw.rotateMove = function (e) {
  ...
  var x = draw.mouse[0];
  var y = draw.mouse[1];
  draw.currPath.rotation =
    getAngleTwoPoints(
      [
        draw.currPath.border[0] + 0.5 * draw.currPath.border[2],
        draw.currPath.border[1] + 0.5 * draw.currPath.border[3],
      ],
      [x, y]
    ) +
    Math.PI / 2;
...
function drawClickAppearMoveStart() {
  ...
  draw.animate(
    drawClickAppearMoveMove,
    drawClickAppearMoveStop,
    drawSelectCanvas2
  );
}
function drawClickAppearMoveMove(e) {
  updateMouse(e);
  draw.selPath.appear.pos = [
    round(draw.mouse[0], 10),
    round(draw.mouse[1], 10),
  ];
...
function sendToBack(paths) {
  ...
  for (var p = draw.path.length - 1; p >= 0; p--) {
    if (applyToAllSel == true && draw.path[p].selected == true)
      paths.push(draw.path[p]);
        ...
        if (shiftOn === false) {
          for (var c = 0; c < paths.length; c++) {
            ...
            positionPath(
              path,
              path.tightBorder[0],
              40 - draw.drawRelPos[1] + path.tightBorder[1] - top
            );
          }
        }
        for (var c = 0; c < paths.length; c++) {
...
function clonePaths2(paths) {
  ...
  for (var p = 0, pMax = paths.length; p < pMax; p++) {
    ...
    for (var o = 0; o < path.obj.length; o++) {
      ...
      if (!un(draw[obj.type]) && typeof draw[obj.type].clone === "function")
        draw[obj.type].clone(obj);
    }
    clones.push(path);
  }
  return clones;
...
function selectAllPaths() {
  for (var p = 0; p < draw.path.length; p++)
    draw.path[p].selected = getPathVis(draw.path[p]);
  drawCanvasPaths();
  calcCursorPositions();
}
function deselectAllPaths(redraw) {
  ...
  if (!un(draw.controlPanel2) && !un(draw.controlPanel2.ctx))
...
function groupPaths() {
  var selected = [];
  for (var i = 0; i < draw.path.length; i++) {
    if (draw.path[i].obj.length == 1 && draw.path[i].obj[0].type === "qBox")
      continue;
...
function textSnap(path, obj, dl, dt, dw) {
  ...
  if (Math.abs(left - draw.gridMargin[0]) < draw.selectTolerance * 2)
    left = draw.gridMargin[0];
  if (Math.abs(top - draw.gridMargin[1]) < draw.selectTolerance * 2)
    top = draw.gridMargin[1];
  if (
    Math.abs(
      width - (draw.drawArea[2] - draw.gridMargin[0] - draw.gridMargin[2])
    ) <
    draw.selectTolerance * 2
  )
    width = draw.drawArea[2] - draw.gridMargin[0] - draw.gridMargin[2];
...
function repositionObj(obj, dl, dt, dw, dh) {
  ...
  switch (obj.type) {
    ...
    case "square":
      ...
      if (dw !== 0 || dh !== 0) {
        var newSize = Math.min(
          mouse.x - obj.startPos[0],
          mouse.y - obj.startPos[1]
        );
    ...
    case "image":
      ...
      if (dw !== 0 || dh !== 0) {
        var sf = Math.min(
          (mouse.x - obj.left) / obj.naturalWidth,
          (mouse.y - obj.top) / obj.naturalHeight
        );
    ...
    case "table2":
      ...
      obj.top += dt;
      if (!un(obj.xPos))
        for (var x = 0; x < obj.xPos.length; x++) obj.xPos[x] += dl;
      if (!un(obj.yPos))
        for (var y = 0; y < obj.yPos.length; y++) obj.yPos[y] += dt;
      if (dw !== 0) {
    ...
    case "angle":
      ...
      if (dw !== 0 || dh !== 0) {
        obj.radius = Math.min(
          Math.abs(mouse.x - draw.drawRelPos[0] - obj.b[0]),
          Math.abs(mouse.y - draw.drawRelPos[1] - obj.b[1])
        );
        obj.a = [
...
function snapToObj2(pos, pathNum) {
  ...
  for (var p = 0; p < draw.path.length; p++) {
    ...
    for (var o = 0; o < draw.path[p].obj.length; o++) {
      var obj = draw.path[p].obj[o];
      if (!un(draw.snapToObjs) && draw.snapToObjs.includes(obj.type) == false)
        continue;
  ...
  for (var p = 0; p < draw.path.length; p++) {
    // if no point found, search for line to snap to
    if (p == pathNum) continue;
    for (var o = 0; o < draw.path[p].obj.length; o++) {
      var obj = draw.path[p].obj[o];
      switch (obj.type) {
...
function snapBorders(pathNum, x, y) {
  ...
  if (Math.abs(x - draw.gridMargin[0]) < closeX) {
    ...
    Math.abs(
      x + b1[2] - (draw.drawArea[0] + draw.drawArea[2] - draw.gridMargin[2])
    ) < closeX
  ) {
    closeX = -(
      x +
      b1[2] -
      (draw.drawArea[0] + draw.drawArea[2] - draw.gridMargin[2])
    );
  }
  ...
  if (Math.abs(y - draw.gridMargin[1]) < closeY) {
    ...
    Math.abs(
      y + b1[3] - (draw.drawArea[1] + draw.drawArea[3] - draw.gridMargin[3])
    ) < closeY
  ) {
    closeY = -(
      y +
      b1[3] -
      (draw.drawArea[1] + draw.drawArea[3] - draw.gridMargin[3])
    );
  }
  for (var p = 0; p < draw.path.length; p++) {
    if (p == pathNum) continue;
    ...
    for (var i = 0; i < draw.path.length; i++) {
      if (draw.path[i].selected == true) {
        ...
        for (var j = 0; j < draw.path[i].obj.length; j++) {
          ...
          xMax = Math.max(
            xMax,
            draw.path[i].obj[j].left + draw.path[i].obj[j].width
          );
          yMin = Math.min(yMin, draw.path[i].obj[j].top);
          yMax = Math.max(
            yMax,
            draw.path[i].obj[j].top + draw.path[i].obj[j].height
          );
        }
        var dx, dy;
        if (draw.horizSnap == "center") {
          dx =
            roundToNearest((xMin + xMax) / 2, draw.gridSnapSize) -
            (xMin + xMax) / 2;
        ...
        if (draw.vertSnap == "middle") {
          dy =
            roundToNearest((yMin + yMax) / 2, draw.gridSnapSize) -
            (yMin + yMax) / 2;
...
function updateSnapPoints() {
  // handles intersection points line snapping - for constructions tool
  /*if (draw.snap == true) {
...
function distributeHoriz() {
  ...
  if (
    s.length == 1 &&
    s[0].obj.length == 1 &&
    (s[0].obj[0].type == "table" || s[0].obj[0].type == "table2")
  ) {
    tableSelected == true;
  }
  ...
  if (tableCellsSelectionTest() == true || tableSelected == true) {
    for (var i = 0; i < draw.path.length; i++) {
      if (draw.path[i].selected == true) {
        for (var j = 0; j < draw.path[i].obj.length; j++) {
          if (
            draw.path[i].obj[j].type == "table" ||
            draw.path[i].obj[j].type == "table2"
          ) {
            ...
            if (obj.type == "table") {
              ...
              draw.path[i].obj[j].width =
                table.xPos[table.xPos.length - 1] - draw.path[i].obj[j].left;
              draw.path[i].obj[j].height =
                table.yPos[table.yPos.length - 1] - draw.path[i].obj[j].top;
            }
          }
        }
    ...
    while (sel2.length > 0) {
      // reorder sel2 & pos2 by tightBorder[0] (left)
      var index = 0;
      var minLeft;
...
function distributeVert() {
  ...
  if (tableCellsSelectionTest() == true || tableSelected == true) {
    ...
    if (sel2.length < 2) return;
    while (sel2.length > 0) {
      // reorder sel2 & pos2 by tightBorder[1] (top)
      var index = 0;
      var minTop;
      for (var p = 0; p < pos2.length; p++) {
        if (p == 0) {
...
function matchWidth() {
  ...
  paths.sort(function (a, b) {
    if (
      Math.abs(a.obj[0].left - b.obj[0].left) >
      Math.abs(a.obj[0].top - b.obj[0].top)
    ) {
...
function hitTestPathRect(path, l, t, w, h) {
  // checks if center of path is in rect
...
function getObjWidth(obj) {
  if (!un(draw[obj.type]) && typeof draw[obj.type].getWidth == "function")
    return draw[obj.type].getWidth();
  switch (obj.type) {
    case "table2":
...
function getObjHeight(obj) {
  if (!un(draw[obj.type]) && typeof draw[obj.type].getHeight == "function")
    return draw[obj.type].getHeight();
  switch (obj.type) {
    case "table2":
...
draw.pathsMatchSize = function (paths) {
  ...
  for (var p = 1; p < paths.length; p++) {
    path = paths[p];
    if (
      path.tightBorder[0] - firstPath.tightBorder[0] < -40 ||
      path.tightBorder[1] - firstPath.tightBorder[1] < -40
    ) {
      firstPath = path;
    }
  }
  for (var p = 0; p < paths.length; p++) {
    ...
    repositionPath(
      ...
      0,
      0,
      firstPath.tightBorder[2] - path.tightBorder[2],
      firstPath.tightBorder[3] - path.tightBorder[3]
    );
  }
  drawCanvasPaths();
};
draw.pathsArrangeToGrid = function (paths) {
  ...
  for (var p = 0; p < paths.length; p++) {
    path = paths[p];
    path.__rect = {
      left: path.tightBorder[0],
      top: path.tightBorder[1],
      width: path.tightBorder[2],
      height: path.tightBorder[3],
      right: path.tightBorder[0] + path.tightBorder[2],
      bottom: path.tightBorder[1] + path.tightBorder[3],
      center: path.tightBorder[0] + 0.5 * path.tightBorder[2],
      middle: path.tightBorder[1] + 0.5 * path.tightBorder[3],
    };
    left = un(left) ? path.__rect.center : Math.min(left, path.__rect.center);
    right = un(right)
      ? path.__rect.center
      : Math.max(right, path.__rect.center);
    top = un(top) ? path.__rect.middle : Math.min(top, path.__rect.middle);
    bottom = un(bottom)
      ? path.__rect.middle
      : Math.max(bottom, path.__rect.middle);
  }
  paths.sort(function (a, b) {
    if (Math.abs(a.__rect.middle - b.__rect.middle) > 40)
      return a.__rect.middle - b.__rect.middle;
    if (Math.abs(a.__rect.center - b.__rect.center) > 40)
      return a.__rect.center - b.__rect.center;
  });
  var cols = 0;
  var rows = 0;
  ...
  for (var i = 0; i < cols; i++)
    xPos.push(left + (i * (right - left)) / Math.max(1, cols - 1));
  for (var i = 0; i < rows; i++)
    yPos.push(top + (i * (bottom - top)) / Math.max(1, rows - 1));
  for (var p = 0; p < paths.length; p++) {
    path = paths[p];
...
draw.setObjFillStyle = function (obj, color) {
  ...
  if (["table2"].indexOf(obj.type) > -1) {
    ...
    var cells =
      selCount == 0
        ? draw.table2.getAllCells(obj)
        : draw.table2.getSelectedCells(obj);
    for (var c = 0; c < cells.length; c++) {
      var cell = cells[c];
      if (!un(cell.box)) {
...
draw.controlPanel = {
  ...
  draw: function (ctx, obj, path, controlPanelData, pos) {
    ...
    for (var e = 0; e < elements.length; e++) {
      ...
      text({
        ctx: ctx,
        rect: [pos[0] + 10 + margin, top, pos[2] - 20, 40],
        align: [-1, -1],
        text: [
          "<<fontSize:" + fontSize + ">><<bold:" + bold + ">>" + element.name,
        ],
      });
      switch (element.type) {
        case "style":
        ...
        case "toggle":
          ...
          ctx.strokeRect(rect[0], rect[1], rect[2], rect[3]);
          if (element.get(obj))
            drawTick(ctx, 16, 20, "#060", pos[0] + pos[2] * 0.1, top, 3);
          cursorPos.push({
            shape: "rect",
            dims: [pos[0] + pos[2] * 0.1, top, 0.8 * pos[2], 20],
            cursor: draw.cursors.pointer,
            element: element,
            set: element.set,
            type: "toggle",
        ...
        case "slider":
          ...
          var xPos =
            sliderLeft +
            (sliderWidth * (value - element.min)) / (element.max - element.min);
          ctx.fillStyle = "#00F";
          ctx.beginPath();
          ctx.arc(xPos, top + 15, 8, 0, 2 * Math.PI);
          ctx.fill();
        ...
        case "multiSelect":
          ...
          for (var c = 0; c < cols + 1; c++)
            widths.push((0.8 * pos[2]) / (cols + 1));
          var table = drawTable3({
            ctx: ctx,
            left: pos[0] + pos[2] * 0.1,
            top: top + 30,
            widths: widths,
            heights: heights,
            cells: cells,
            outerBorder: { width: 2, color: "#000", dash: [] },
            innerBorder: { width: 1, color: "#000", dash: [] },
          });
          ...
          for (var r = 0; r < table.cell.length; r++) {
            for (var c = 0; c < table.cell[r].length; c++) {
              ...
              cursorPos.push({
                shape: "rect",
                dims: [cell.left, cell.top, cell.width, cell.height],
                cursor: draw.cursors.pointer,
                element: element,
                set: element.set,
                type: "multiSelect",
                value: element.options[r][c],
                obj: obj,
                func: draw.controlPanel.start,
              });
            }
    ...
    if (!un(draw.controlPanel.colorMenu)) {
      ...
      var width = 30,
        ...
        cols = 4;
      if (
        draw.drawArea[0] + draw.drawArea[2] - pos2[0] <
        width * cols + 2 * padding
      ) {
        pos2[0] =
          draw.drawArea[0] + draw.drawArea[2] - (width * cols + 2 * padding);
      }
      roundedRect(
        ctx,
        pos2[0],
    ...
    cursorPos.unshift({
      shape: "rect",
      dims: [pos[0], pos[1], pos[2], top + 10 - pos[1]],
      cursor: draw.cursors.default,
    });
  ...
  start: function () {
    ...
    if (type == "slider") {
      draw.drag = {
        ...
        element: element,
        obj: obj,
        sliderLeft: draw.currCursor.sliderLeft,
        sliderWidth: draw.currCursor.sliderWidth,
      };
      draw.animate(
        draw.controlPanel.move,
        draw.controlPanel.stop,
        drawCanvasPaths
      );
      //addListenerMove(window,draw.controlPanel.move);
      //addListenerEnd(window,draw.controlPanel.stop);
      draw.drawCanvas[draw.drawCanvas.length - 2].style.cursor =
        draw.currCursor.cursor;
    } else if (type == "multiSelect") {
      draw.currCursor.set(obj, draw.currCursor.value);
      drawSelectedPaths();
    } else if (type == "openLineColor") {
      if (
        !un(draw.controlPanel.colorMenu) &&
        draw.controlPanel.colorMenu.type == "lineColor"
      ) {
        delete draw.controlPanel.colorMenu;
      } else {
        var dims = draw.currCursor.dims;
        draw.controlPanel.colorMenu = {
          type: "lineColor",
          pos: [dims[0], dims[1] + dims[3]],
        };
      }
    } else if (type == "openFillColor") {
      if (
        !un(draw.controlPanel.colorMenu) &&
        draw.controlPanel.colorMenu.type == "fillColor"
      ) {
        delete draw.controlPanel.colorMenu;
      } else {
        var dims = draw.currCursor.dims;
  ...
  move: function (e) {
    ...
    if (element.type == "slider") {
      var value = bound(
        element.min +
          ((element.max - element.min) * (mouse.x - draw.drag.sliderLeft)) /
            draw.drag.sliderWidth,
        element.min,
        element.max,
        element.step
      );
...
function protractorRotateMove(e) {
  ...
  draw.protractor.angle += measureAngle({
    c: draw.mouse,
    b: [draw.protractor.center[0], draw.protractor.center[1]],
    a: [draw.protractor.prevX, draw.protractor.prevY],
    angleType: "radians",
  });
...
function rulerRotateMove1(e) {
  ...
  dAngle = measureAngle({
    c: draw.mouse,
    b: [draw.ruler.centerX2, draw.ruler.centerY2],
    a: [draw.ruler.prevX, draw.ruler.prevY],
    angleType: "radians",
  });
  draw.ruler.angle += dAngle;
  draw.ruler.left =
    draw.ruler.centerX2 - 0.98 * draw.ruler.length * Math.cos(draw.ruler.angle);
  draw.ruler.top =
    draw.ruler.centerY2 - 0.98 * draw.ruler.length * Math.sin(draw.ruler.angle);
  recalcRulerValues();
  //drawToolsCanvas();
...
function rulerRotateMove2(e) {
  ...
  draw.ruler.angle += measureAngle({
    c: draw.mouse,
    b: [draw.ruler.centerX1, draw.ruler.centerY1],
    a: [draw.ruler.prevX, draw.ruler.prevY],
    angleType: "radians",
  });
  recalcRulerValues();
...
function rulerDrawMove1(e) {
  ...
  var newPos = closestPointOnLineSegment(
    draw.mouse,
    draw.ruler.edgePos1,
    draw.ruler.edgePos2
  );
...
function compassDrawMove(e) {
  ...
  var dAngle = measureAngle({
    c: draw.mouse,
    b: [draw.compass.center1[0], draw.compass.center1[1]],
    a: [draw.compass.prevX, draw.compass.prevY],
    angleType: "radians",
  });
  if (dAngle > Math.PI) {
  ...
  draw.compass.center3[0] =
    draw.compass.center1[0] +
    draw.compass.radius * Math.cos(draw.compass.angle);
  draw.compass.center3[1] =
    draw.compass.center1[1] +
    draw.compass.radius * Math.sin(draw.compass.angle);
  ...
  if (draw.compass.drawOn == "right") {
    draw.compass.center2[0] =
      draw.compass.center1[0] +
      0.5 * draw.compass.radius * Math.cos(draw.compass.angle) +
      draw.compass.h * Math.sin(draw.compass.angle);
    draw.compass.center2[1] =
      draw.compass.center1[1] +
      0.5 * draw.compass.radius * Math.sin(draw.compass.angle) -
      draw.compass.h * Math.cos(draw.compass.angle);
  } else {
    draw.compass.center2[0] =
      draw.compass.center1[0] +
      0.5 * draw.compass.radius * Math.cos(draw.compass.angle) -
  ...
  draw.path[draw.path.length - 1].obj[0].startAngle = Math.min(
    draw.path[draw.path.length - 1].obj[0].startAngle,
    draw.compass.angle
  );
  draw.path[draw.path.length - 1].obj[0].finAngle = Math.max(
    draw.path[draw.path.length - 1].obj[0].finAngle,
    draw.compass.angle
  );
...
function drawClickCompassStartMove2(e) {
  updateMouse(e);
  draw.compass.dragRelPos = [
    draw.compass.center3[0] - draw.mouse[0],
    draw.compass.center3[1] - draw.mouse[1],
  ];
...
function compassMove1Move(e) {
  ...
  draw.compass.center2 = [
    center1[0] + draw.compass.relCenter2[0],
    center1[1] + draw.compass.relCenter2[1],
  ];
  draw.compass.center3 = [
    center1[0] + draw.compass.relCenter3[0],
    center1[1] + draw.compass.relCenter3[1],
  ];
  draw.compass.lockCenter = [
    center1[0] + draw.compass.relLockCenter[0],
    center1[1] + draw.compass.relLockCenter[1],
  ];
  //drawToolsCanvas();
}
function compassMove2Move(e) {
  updateMouse(e);
  var newcenter3 = [
    draw.mouse[0] + draw.compass.dragRelPos[0],
    draw.mouse[1] + draw.compass.dragRelPos[1],
  ];
  ...
  if (draw.compass.radiusLocked == false) {
    ...
    var newRadius = Math.sqrt(
      Math.pow(newcenter3[0] - draw.compass.center1[0], 2) +
        Math.pow(newcenter3[1] - draw.compass.center1[1], 2)
    );
    ...
    if (newRadius <= 1.85 * draw.compass.armLength) {
      ...
      if (draw.compass.center3[0] >= draw.compass.center1[0]) {
        draw.compass.angle = Math.atan(
          (draw.compass.center3[1] - draw.compass.center1[1]) /
            (draw.compass.center3[0] - draw.compass.center1[0])
        );
      } else {
        draw.compass.angle =
          Math.PI +
      ...
      if (newcenter3[0] >= draw.compass.center1[0]) {
        draw.compass.angle = Math.atan(
          (newcenter3[1] - draw.compass.center1[1]) /
            (newcenter3[0] - draw.compass.center1[0])
        );
      } else {
        draw.compass.angle =
          Math.PI +
      ...
      draw.compass.center3[0] =
        draw.compass.center1[0] +
        1.85 * draw.compass.armLength * Math.cos(draw.compass.angle);
      draw.compass.center3[1] =
        draw.compass.center1[1] +
        1.85 * draw.compass.armLength * Math.sin(draw.compass.angle);
    ...
    if (
      Math.abs(
        dist(snapNewCenter3, draw.compass.center1) - draw.compass.radius
      ) < 0.1
    ) {
      pos = [
        pos[0] - draw.compass.dragRelPos[0],
        pos[1] - draw.compass.dragRelPos[1],
      ];
    }
  ...
  draw.compass.h = Math.sqrt(
    Math.pow(draw.compass.armLength, 2) - Math.pow(0.5 * draw.compass.radius, 2)
  );
  if (draw.compass.drawOn == "right") {
    draw.compass.center2[0] =
      draw.compass.center1[0] +
