...
function textArrayCombineAdjacentText(arr) {
  for (var gg = arr.length - 1; gg >= 0; gg--) {
    if (typeof arr[gg] == "object") {
      ...
      if (
        gg < arr.length - 1 &&
        typeof arr[gg] == "string" &&
        typeof arr[gg + 1] == "string"
      ) {
        arr[gg] += arr[gg + 1];
        arr.splice(gg + 1, 1);
      }
...
function textArrayRemoveDefaultTags(arr, tags) {
  //also removes redundant tags at end
...
function removeTags(elem) {
  if (typeof elem == "string") {
    ...
    for (var char = elem.length - 1; char > -1; char--) {
      if (
        elem.slice(char).indexOf(">>") == 0 &&
        elem.slice(char - 1).indexOf(">>>") !== 0
      ) {
...
function getDateString() {
  ...
  var dateString =
    String(dateNum) +
    ([1, 21, 31].includes(dateNum)
      ? "st"
      : [2, 22].includes(dateNum)
      ? "nd"
      : [3, 23].includes(dateNum)
      ? "rd"
      : "th");
...
function text(obj) {
  ...
  if (typeof mode !== "undefined" && mode == "present")
    textArray = textArrayReplace(clone(textArray), "{date}", getDateString());
  ...
  if (box.type !== "none" && !un(box.color) && box.color !== "none")
  ...
  for (var w = 0; w < subWords.length; w++) {
    ...
    if (arraysEqual(subWord, [br])) {
      ...
      if (
        w > 1 &&
        subWords[w - 1].text.length == 1 &&
        subWords[w - 2].tab == true
      ) {
        var dw = tabWidth;
      } else {
      ...
      if (line.width > 0 && line.width + subWords[w].width > maxWidth)
        newLine();
  ...
  for (var l = 0; l < lines.length; l++) {
    ...
    if (l > 0) {
      line.relY =
        lines[l - 1].relY +
        (0.5 * lines[l - 1].height + 0.5 * line.height) * lineSpacingFactor;
    } else {
  ...
  if (box.type == "loose") {
    ...
    if (box.dir == "left") {
      var points = [
        [right, top],
        [left + box.arrowWidth / 2, top],
        [left - box.arrowWidth / 2, (top + bottom) / 2],
        [left + box.arrowWidth / 2, bottom],
        [right, bottom],
      ];
    } else {
      var points = [
        [left, top],
        [right - box.arrowWidth / 2, top],
        [right + box.arrowWidth / 2, (top + bottom) / 2],
        [right - box.arrowWidth / 2, bottom],
        [left, bottom],
      ];
    }
    drawPolygon({
  ...
  if (boolean(obj.showText, true) == true) {
    for (var l = 0; l < lines.length; l++) {
      ...
      var align2 =
        line.align == 1 ? "right" : line.align == 0 ? "center" : "left";
      ...
      if (
        line.hardBreak !== true &&
        typeof line.text.last() == "string" &&
        line.text.last().slice(-1) == " " &&
        l < lines.length - 1
      ) {
  ...
  for (var i = returnTextLoc2.length - 1; i >= 1; i--) {
    if (
      typeof returnTextLoc2[i][0] == "object" &&
      typeof returnTextLoc2[i - 1][0] == "object"
    ) {
