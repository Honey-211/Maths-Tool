...
var keyboardElements = [
  ...
  { display: [["hat", [""]]], name: "hat", func: mathsInputHat },
  {
    display: [["recurring", [""]]],
    name: "recurring",
    func: mathsInputRecurring,
  },
  {
    display: [["colVector2d", [""], [""]]],
    name: "colVector2d",
    func: mathsInputColVector2d,
  },
  {
    display: [["colVector3d", [""], [""], [""]]],
    name: "colVector3d",
    func: mathsInputColVector3d,
  },
  {
    display: [["mixedNum", [""], [""], [""]]],
    name: "mixedNum",
    func: mathsInputMixedNum,
  },
...
function createKeyboard(object) {
  ...
  contextInstance.fillRect(
    0.5 * width - 20 - 0.5 * contextInstance.measureText("Keyboard").width - 10,
    20,
    contextInstance.measureText("Keyboard").width + 20,
    10
  );
  ...
  for (var rowNum = 0; rowNum < keyArray.length; rowNum++) {
    ...
    if (align == "left") {
      ...
      keyLeft =
        left +
        width -
        keyArray[rowNum].length * (keySize + keyPadding) -
        keyPadding * 2;
    } else {
      keyLeft =
        left + 0.5 * (width - keyArray[rowNum].length * (keySize + keyPadding));
    }
    var keyTop =
      top + keyPadding + 40 + keyPadding + rowNum * (keySize + keyPadding);
    ...
    for (var colNum = 0; colNum < keyArray[rowNum].length; colNum++) {
      ...
      if (
        canvasInstance.element == true ||
        ["leftArrow", "rightArrow", "del", "delete"].indexOf(
          canvasInstance.name
        ) > -1
      ) {
      ...
      drawKey(
        canvasInstance.ctx,
        canvasInstance.name,
        canvasInstance.algText,
        false,
        canvasInstance.keySize,
        canvasInstance.fontSize
      );
...
function updateKeyboardCurrFont() {
  ...
  if (currMathsInput.selected == true && currMathsInput.selectPos.length > 0) {
    var cursorMapPos =
      currMathsInput.cursorMap[
        Math.max(currMathsInput.selectPos[0], currMathsInput.selectPos[1])
      ];
  } else {
  ...
  for (var pos = 0; pos < currMathsInput.allMap.length; pos++) {
    ...
    posText = posText.slice(
      currMathsInput.allMap[pos][currMathsInput.allMap[pos].length - 1]
    );
    var tagType = "none";
  ...
  if (
    keyboardCurrFont !== prev &&
    typeof key1 !== "undefined" &&
    typeof key1[pageIndex] !== "undefined"
  ) {
    for (var k = 0; k < key1[pageIndex].length - 1; k++) {
      if (key1[pageIndex][k].keyType == "letter") {
        drawKey(
          key1[pageIndex][k].ctx,
          key1[pageIndex][k].name,
          key1[pageIndex][k].algText,
          false,
          key1[pageIndex][k].keySize,
          key1[pageIndex][k].fontSize
        );
      }
    }
  }
  ...
  switch (name) {
    ...
    case "left":
      text({
        ...
        align: "center",
        vertAlign: "middle",
        box: { type: "loose", radius: 6, color: color, borderWidth: 3 },
      });
      ctx.lineWidth = 2;
      ctx.beginPath();
    ...
    default:
      ...
      text({
        ...
        font: font,
        fontSize: fontSize,
        rect: [1.5, 1.5, keySize - 3, keySize - 3],
        align: [0, 0],
        box: { type: "loose", radius: 6, color: color, borderWidth: 3 },
      });
...
function showKeyboard2(lightUp) {
  ...
  if (keyboard[pageIndex].parentNode !== container) {
    ...
    if (showKeys[pageIndex].parentNode == container) {
      container.removeChild(showKeys[pageIndex]);
    }
...
function hideKeyboard2(lightDown) {
  ...
  if (
    keyboard[pageIndex].parentNode == container &&
    keyboardHardOpen == false
  ) {
    ...
    if (hideKeys[pageIndex].parentNode == container) {
      container.removeChild(hideKeys[pageIndex]);
    }
...
function dragKeyboardStart(e) {
  ...
  e.target.dragOffset = [
    mouse.x - e.target.data[100],
    mouse.y - e.target.data[101],
  ];
  drag = e.target;
