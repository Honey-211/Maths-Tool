...
function drawGrid3(
  ...
  if (mode == "rad") {
    ...
    if (typeof gridDetails.xMinorStep == "number") {
      ...
      var xMinorStep =
        (Math.PI * gridDetails.xMinorStep[0]) / gridDetails.xMinorStep[1];
    }
    if (typeof gridDetails.xMajorStep == "number") {
      var xMajorStep = Math.PI * gridDetails.xMajorStep;
    } else {
      var xMajorStep =
        (Math.PI * gridDetails.xMajorStep[0]) / gridDetails.xMajorStep[1];
    }
  } else {
  ...
  var showScales = boolean(
    gridDetails.showScales,
    boolean(opt_showScales, true)
  );
  var showXScale = boolean(gridDetails.showXScale, showScales);
  var showYScale = boolean(gridDetails.showYScale, showScales);
  var showLabels = boolean(
    gridDetails.showLabels,
    boolean(opt_showLabels, true)
  );
  ...
  var backColor =
    gridDetails.backColor || opt_backColor || mainCanvasFillStyle || "#FFC";
  var invertedBackColor = getShades(backColor, true);
  if (["#FFF", "#fff", "#FFFFFF", "#ffffff"].indexOf(backColor) == -1) {
    var minorColor =
      gridDetails.minorColor ||
      opt_minorColor ||
      /*invertedBackColor[10] || */ "#999";
  } else {
    var minorColor =
      gridDetails.minorColor ||
      opt_minorColor ||
  ...
  var majorColor =
    gridDetails.majorColor ||
    gridDetails.color ||
    opt_majorColor ||
    /*invertedBackColor[9] || */ "#999";
  var originColor =
    gridDetails.axesColor ||
    opt_originColor ||
    /*invertedBackColor[4] || */ "#666";
  var xAxisColor =
    gridDetails.axesColor ||
    opt_xAxisColor ||
    /*invertedBackColor[2] || */ "#000";
  var yAxisColor =
    gridDetails.axesColor ||
    opt_yAxisColor ||
    /*invertedBackColor[2] || */ "#000";
  ...
  var xScaleColor =
    gridDetails.xScaleColor || gridDetails.scaleColor || xAxisColor;
  var yScaleColor =
    gridDetails.yScaleColor || gridDetails.scaleColor || yAxisColor;
  ...
  if (dots == true) {
    ...
    var xMin2 =
      Math.floor(Math.abs(xMin) / xMajorStep) *
      xMajorStep *
      (xMin / Math.abs(xMin));
    var xMax2 =
      Math.floor(Math.abs(xMax) / xMajorStep) *
      xMajorStep *
      (xMax / Math.abs(xMax));
    var yMin2 =
      Math.floor(Math.abs(yMin) / yMajorStep) *
      yMajorStep *
      (yMin / Math.abs(yMin));
    var yMax2 =
      Math.floor(Math.abs(yMax) / yMajorStep) *
      yMajorStep *
      (yMax / Math.abs(yMax));
  ...
  if (showLabels == true) {
    if (!un(gridDetails.axisLabels)) {
      if (!un(draw) && !un(draw.hiddenCanvas)) {
        ...
        if (!un(window.hiddenCanvas))
          window.hiddenCanvas = newctx({ vis: false });
        var ctx2 = hiddenCanvas.ctx;
      }

      var xDist = gridDetails.axisLabels[0].dist || 30;
      var measureTextX = text({
        ctx: ctx2,
        text: ["<<fontSize:" + labelFontSize + ">>"].concat(
          gridDetails.axisLabels[0].text
        ),
        rect: [0, 0, gridDetails.width, 100],
        measureOnly: true,
      }).tightRect;
      var xLabelRect = [
        gridDetails.left + gridDetails.width - measureTextX[2],
        gridDetails.top + gridDetails.height + xDist,
        measureTextX[2],
        measureTextX[3],
      ];
      ...
      var measureTextY = text({
        ctx: ctx2,
        text: ["<<fontSize:" + labelFontSize + ">>"].concat(
          gridDetails.axisLabels[1].text
        ),
        rect: [0, 0, gridDetails.height, 100],
        measureOnly: true,
      }).tightRect;
      var yLabelRect = [
        gridDetails.left - yDist - measureTextY[3],
        gridDetails.top,
        measureTextY[3],
        measureTextY[2],
      ];
      ...
      text({
        ...
        rect: [0, 0, measureTextY[2], measureTextY[3]],
        align: [0, 0],
      });
      context.restore();
    } else {
      ...
      labelPos[2] = Math.max(
        labelPos[2],
        yLabel.tightRect[0] + yLabel.tightRect[2]
      );
      labelPos[3] = Math.max(
        labelPos[3],
        yLabel.tightRect[1] + yLabel.tightRect[3]
      );
      ...
      var xLabel = drawMathsText(
        context,
        xAxisLabel2,
        labelFontSize,
        left + width + 10,
        y0DisplayPos - labelFontSize,
        true,
        [],
        "left",
        "top",
      ...
      labelPos[2] = Math.max(
        labelPos[2],
        xLabel.tightRect[0] + yLabel.tightRect[2]
      );
      labelPos[3] = Math.max(
        labelPos[3],
        xLabel.tightRect[1] + yLabel.tightRect[3]
      );
    }
  }
  ...
  if (showScales == true) {
    ...
    if (showXScale == true) {
      ...
      if (mode == "rad") {
        ...
        var major = 1;
        while (
          roundToNearest(xAxisPoint, 0.001) <=
          roundToNearest(left + width, 0.001)
        ) {
          if (xAxisPoint >= left) {
            if (typeof gridDetails.xMajorStep == "number") {
              ...
              var frac = {
                num: gridDetails.xMajorStep[0] * major,
                denom: gridDetails.xMajorStep[1],
              };
            }
            ...
            var params = {
              ...
              left: xAxisPoint - 50,
              width: 100,
              top: y0DisplayPos + xScaleOffset - 4,
              height: 50,
              minTightWidth: 1,
              minTightHeight: 1,
              align: "center",
            ...
            labelPos[2] = Math.max(
              labelPos[2],
              labelText.tightRect[0] + labelText.tightRect[2]
            );
            labelPos[3] = Math.max(
              labelPos[3],
              labelText.tightRect[1] + labelText.tightRect[3]
            );
          }
          major += 1;
          xAxisPoint += xMajorSpacing;
        ...
        major = -1;
        while (
          roundToNearest(xAxisPoint, 0.001) >= roundToNearest(left, 0.001)
        ) {
          if (xAxisPoint < left + width) {
            if (typeof gridDetails.xMajorStep == "number") {
              var frac = { num: gridDetails.xMajorStep * major, denom: 1 };
        ...
        while (
          ...
          if (xAxisPoint >= left) {
            ...
            var labelText = drawMathsText(
              ...
              gridFontSize,
              xAxisPoint,
              y0DisplayPos + xScaleOffset + 0.5 * gridFontSize,
              true,
              [],
              "center",
              "middle",
        ...
        var placeValue = Math.pow(
          10,
          Math.floor(Math.log(xScaleStep) / Math.log(10))
        );
        while (
          roundToNearest(xAxisPoint, 0.001) <=
          roundToNearest(left + width, 0.001)
    ...
    if (showYScale == true) {
      ...
      var placeValue = Math.pow(
        10,
        Math.floor(Math.log(yScaleStep) / Math.log(10))
      );
      while (roundToNearest(yAxisPoint, 0.001) >= roundToNearest(top, 0.001)) {
        if (yAxisPoint <= top + height) {
          ...
          context.fillStyle = backColor;
          context.fillRect(
            x0DisplayPos - textWidth - 11 * sf - yScaleOffset,
            yAxisPoint - gridFontSize * 0.5,
            textWidth + 3 * sf,
            gridFontSize
          );
          var labelText = drawMathsText(
            ...
            String(axisValue),
            gridFontSize,
            x0DisplayPos - 10 * sf - yScaleOffset,
            yAxisPoint - 2,
            true,
            [],
            "right",
  ...
  if (!un(gridDetails.plot)) {
    for (var c = 0; c < gridDetails.plot.length; c++) {
      ...
      if (plot.type == "histogram") {
        ...
        var coords = convertCoordsGridToCanvas(
          0,
          0,
          gridDetails,
          plot.chartData
        ...
        for (var c2 = 1; c2 < coords.length; c2++)
          context.lineTo(coords[c2][0], coords[c2][1]);
        context.stroke();
        drawScatter(context, gridDetails, plot);
        /* eg.
					sel().originStyle = 'numbers';
  ...
  var labelBorder = [
    labelPos[0],
    labelPos[1],
    labelPos[2] - labelPos[0],
    labelPos[3] - labelPos[1],
    labelPos[2],
    labelPos[3],
  ];
  /*
...
function gridLabelPos(ctx, gridDetails, obj) {
  ...
  if (x0 > gridDetails.left + gridDetails.width)
    x0 = gridDetails.left + gridDetails.width;
  var y0 = getPosOfCoordY2(
    0,
    gridDetails.top,
    gridDetails.height,
    gridDetails.yMin,
  ...
  if (y0 > gridDetails.top + gridDetails.height)
    y0 = gridDetails.top + gridDetails.height;

  var x1 = getPosOfCoordX2(
    obj.x,
    gridDetails.left,
    gridDetails.width,
    gridDetails.xMin,
  ...
  if (obj.labelX === true) {
    ...
    text({
      ctx: ctx,
      textArray: ["<<color:" + xColor + ">>" + String(obj.x)],
      font: "algebra",
      fontSize: gridFontSize,
      left: x1 - 50,
      width: 100,
  ...
  if (obj.labelY === true) {
    ...
    text({
      ctx: ctx,
      textArray: ["<<color:" + xColor + ">>" + String(obj.y)],
      font: "algebra",
      fontSize: gridFontSize,
      left: x0 - 100 - 10 * sf,
      width: 100,
  ...
  if (
    boolean(gridDetails.showScales, true) == true &&
    boolean(gridDetails.showOrigin, true) == true
  ) {
    // redraw origin
    if (
      x0 >= gridDetails.left &&
...
function drawGridRadians(
  ...
  // work out the spacing for minor and major steps
  var xMinorSpacing =
    (width * (xMinorStep.num / xMinorStep.denom) * Math.PI) / (xMax - xMin);
  var xMajorSpacing =
    (width * (xMajorStep.num / xMajorStep.denom) * Math.PI) / (xMax - xMin);
  var yMinorSpacing = (height * yMinorStep) / (yMax - yMin);
  var yMajorSpacing = (height * yMajorStep) / (yMax - yMin);
  ...
  wrapText(
    ...
    50,
    40,
    "italic " + labelFontSize + "px Times New Roman",
    "#000"
  );
...
function multOfPiText(frac) {
  // frac is an object with num and denom properties
...
function drawCoord(
  ...
  if (mode == "deg") {
    ...
    if (
      x < (Math.PI * gridDetails.xMin[0]) / gridDetails.xMin[1] ||
      x > (Math.PI * gridDetails.xMax[0]) / gridDetails.xMax[1]
    )
      return;
    var xPos = getPosOfCoordX2(
      x,
      left,
      gridDetails.width,
      (Math.PI * gridDetails.xMin[0]) / gridDetails.xMin[1],
      (Math.PI * gridDetails.xMax[0]) / gridDetails.xMax[1]
    );
  }
  var yPos = getPosOfCoordY2(
    y,
    top,
    gridDetails.height,
    gridDetails.yMin,
  ...
  context.lineWidth =
    gridDetails.pointWidth * sf || opt_lineWidth * sf || 1.5 * sf;
  context.beginPath();
...
function getPosOfCoord(pos, gridDetails) {
  // given ([x,y],gridDetails)
  return [
    getPosOfCoordX2(
      pos[0],
      gridDetails.left,
...
function drawLine(
  ...
  if (
    intersects2(x1, y1, x2, y2, xMin, yMin, xMax, yMin) == false &&
  ...
  if (lineSegment == true) {
    if (x1Pos == x2Pos) {
      // special case: vertical line, infinite gradient
      if (y1Pos < top) {
        y1Pos = top;
      }
      if (y1Pos > top + height) {
    ...
    if (x1Pos == x2Pos) {
      ...
      for (var p = 0; p < 4; p++) {
        // check if line goes through corner points
        var point = polygon[p];
      ...
      for (var p = 0; p < 4; p++) {
        ...
        if (
          ...
          for (var i = 0; i < intersectionPoints.length; i++) {
            if (
              intersectionPoints[i][0] === point[0] &&
              intersectionPoints[i][1] === point[1]
            ) {
              found = true;
              break;
            }
...
function calcFunc(
  ...
  for (var xPos = 0; xPos <= width + drawDensity; xPos = xPos + drawDensity) {
    ...
    if (
      (domainMin == "none" || x >= domainMin) &&
      (domainMax == "none" || x <= domainMax)
    ) {
      thisPoint.push(true);
    } else {
...
function drawFunc(
  ...
  for (var pos = 0; pos < funcPoints.length; pos++) {
    ...
    if (typeof asymptotes == "object" && pos > 0) {
      for (var i = 0; i < asymptotes.length; i++) {
        if (
          funcPoints[pos - 1][2] <= asymptotes[i][0] &&
          funcPoints[pos][2] >= asymptotes[i][0]
        ) {
          asymp = i;
        }
      }
    ...
    if (currPlot == "high") {
      if (prevPlot == "ok") {
        path[pathNum].push([
          funcPoints[pos - 1][0] -
            contextLeft +
            ((funcPoints[pos][0] - funcPoints[pos - 1][0]) *
              (funcPoints[pos - 1][1] - top)) /
              (funcPoints[pos - 1][1] - funcPoints[pos][1]),
          top - contextTop,
        ]);
      }
    } else {
        ...
        if (prevPlot == "ok") {
          path[pathNum].push([
            funcPoints[pos - 1][0] -
              contextLeft +
              ((funcPoints[pos][0] - funcPoints[pos - 1][0]) *
                (top + height - funcPoints[pos - 1][1])) /
                (funcPoints[pos][1] - funcPoints[pos - 1][1]),
            top + height - contextTop,
          ]);
        }
      } else {
        if (prevPlot == "ok") {
          path[pathNum].push([
            funcPoints[pos][0] - contextLeft,
            funcPoints[pos][1] - contextTop,
          ]);
        } else {
          if (prevPlot == "high") {
            pathNum++;
  ...
  context.lineWidth = def([
    opt_thickness,
    gridDetails.funcWidth,
    gridDetails.lineWidth,
    3,
  ]);
  context.lineCap = "round";
  context.beginPath();
...
function calcFuncImplicitGridPos(gridDetails, func, increment) {
  function getLines(SW, SE, NW, NE) {
    // returns 0=S,1=W,2=N,3=E
    if (SW < 0) {
      if (SE < 0) {
        if (NE < 0) {
          if (NW < 0) {
...
function calcFuncImplicitCanvasPos(gridDetails, funcPos) {
  ...
  for (var p = 0; p < funcPos.length; p++) {
    var pos = funcPos[p];
    canvasPos.push([
      getPosOfCoord(pos[0], gridDetails),
      getPosOfCoord(pos[1], gridDetails),
    ]);
  }
  return canvasPos;
}
    ...
    for (var x = xMin; x < xMax; x += xInc) {
      ...
      while (lines.length > 1) {
        ...
        for (var i = 0; i < 2; i++) {
          var xPos2 = Math.min(
            xPos + density,
            gridDetails.left + gridDetails.width
          );
...
function slowPlotFunc(
  ...
  var slowPlotInterval = setCorrectingInterval(function () {
    if (plotPointNum == 0) {
      context.beginPath();
      context.moveTo(
        plotPath[plotPathNum][plotPointNum][0],
        plotPath[plotPathNum][plotPointNum][1]
      );
    } else {
      context.lineTo(
        plotPath[plotPathNum][plotPointNum][0],
...
function convertToRadians(funcString) {
  ...
  for (var char = 0; char < funcString.length; char++) {
    if (
      funcString.slice(char).indexOf("Math.sin(") == 0 ||
      funcString.slice(char).indexOf("Math.cos(") == 0 ||
      funcString.slice(char).indexOf("Math.tan(") == 0
    ) {
      var bracketCount = 1;
      var charCount = 9;
      do {
      ...
      if (inner.indexOf("Math.PI") == -1) {
        ...
        funcString =
          funcString.slice(0, char + 9) +
          inner +
          funcString.slice(char + charCount - 1);
      }
    }
...
function calcAreaUnderFunc(gridDetails, funcString, min, max, opt_drawDensity) {
  ...
  var funcPoints = [
    [
      getPosOfCoordX2(min, left, width, xMin, xMax),
      getPosOfCoordY2(0, top, height, yMin, yMax),
    ],
  ];

  for (
    var xPos = getPosOfCoordX2(min, left, width, xMin, xMax) - left;
    xPos <= getPosOfCoordX2(max, left, width, xMin, xMax) - left;
    xPos = xPos + drawDensity
  ) {
    var x = xMin + (xPos / width) * (xMax - xMin);
    try {
      eval("var y = " + funcString + ";");
    ...
    var yPos = Math.min(
      Math.max(top + height - height * ((y - yMin) / (yMax - yMin)), top),
      top + height
    );
  ...
  funcPoints.push(
    [
      getPosOfCoordX2(max, left, width, xMin, xMax),
      getPosOfCoordY2(0, top, height, yMin, yMax),
    ],
    [
      getPosOfCoordX2(min, left, width, xMin, xMax),
...
function drawFuncArea(
  ...
  if ((boundaryLines = true)) {
    ...
    context.moveTo(
      funcAreaPoints[funcAreaPoints.length - 3][0],
      funcAreaPoints[funcAreaPoints.length - 3][1]
    );
    context.lineTo(
      funcAreaPoints[funcAreaPoints.length - 2][0],
      funcAreaPoints[funcAreaPoints.length - 2][1]
    );
    context.stroke();
  }
  context.restore();
...
function convertCoordsGridToCanvas(
  contextLeft,
  contextTop,
  gridDetails,
  pointsArray
  ...
  for (var i = 0; i < pointsArray.length; i++) {
    returnArray[i] = [
      getPosOfCoordX2(pointsArray[i][0], left, width, xMin, xMax),
      getPosOfCoordY2(pointsArray[i][1], top, height, yMin, yMax),
    ];
  }
...
function drawBoxPlot(
  ...
  // fill
  if (
    boolean(options.rangeOnly, false) == false &&
    boolean(options.dashesOnly, false) == false &&
    boolean(options.fill, false) == true
  ) {
