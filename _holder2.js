...
function arraysEqual(array1, array2) {
  ...
  for (var arr = 0; arr < array1.length; arr++) {
    ...
    if (
      typeof array1[arr] == "object" &&
      arraysEqual(array1[arr], array2[arr]) == false
    )
      return false;
    if (typeof array1[arr] !== "object" && array1[arr] !== array2[arr])
      return false;
  }
  return true;
}
  ...
  function mapArray2(array, includeAll) {
    for (var aa = 0; aa < array.length; aa++) {
      if (typeof array[aa] !== "undefined") {
        if (
          typeof array[aa].left == "undefined" &&
          typeof array[aa] !== "boolean"
        ) {
          if (mapString == "") {
            mapString = String(aa);
          } else {
            mapString = mapString + "," + String(aa);
          }
          mapArray2(array[aa], includeAll);
        } else if (
          includeAll == true ||
          typeof array[aa].markupTag == "undefined" ||
          array[aa].markupTag == false
        ) {
          if (mapString == "") {
            mapString = String(aa);
          } else {
          ...
          for (var bb = 0; bb < mapArr.length; bb++) {
            mapArr[bb] = Number(mapArr[bb]);
          }
          map.push(mapArr);
          mapString = mapString.slice(
            0,
            mapString.lastIndexOf(",") - mapString.length
          );
        }
      }
    }
...
function loop(obj, func, recursive) {
  ...
  if (obj instanceof Array) {
    for (var i = 0; i < obj.length; i++) {
      if (
        boolean(recursive, true) == true &&
        (obj[i] instanceof Array || typeof obj[i] == "object")
      ) {
...
if (scriptsToLoad.includes("clone.js") == false)
  scriptsToLoad.unshift("clone.js");
if (scriptsToLoad.includes("_draw3.js") == false)
  scriptsToLoad.push("_draw3.js");
...
function printCanvases(arr) {
  // needs cross-browser testing
  var data = [];
  var windowContent = "<!DOCTYPE html>";
  windowContent += "<html>";
  windowContent +=
    '<head><title>Print Worksheet</title><style type="text/css" media="print">';
  ...
  windowContent +=
    ".pageimg {page-break-after: always; margin-top: 10mm; margin-bottom: 10mm;}";
...
if (typeof mainCanvasFillStyle == "undefined")
...
window.addEventListener(
  ...
  function (e) {
    updateMouse(e);
    if (un(e.target.allowDefault) || e.target.allowDefault == false)
      e.preventDefault();
    if (logPointerEvents) console.log("touchstart", e.target);
  },
  true
);
window.addEventListener(
  ...
  function (e) {
    ...
    if (un(e.target.allowDefault) || e.target.allowDefault == false)
      e.preventDefault();
    if (logPointerEvents) console.log("touchmove", e.target);
  },
  true
);
window.addEventListener(
  ...
  function (e) {
    if (un(e.target.allowDefault) || e.target.allowDefault == false)
      e.preventDefault();
    if (logPointerEvents) console.log("touchend", e.target);
  },
  true
);
window.addEventListener(
...
window.addEventListener(
  ...
  function (e) {
    ...
    if (un(e.target.allowDefault) || e.target.allowDefault == false)
      e.preventDefault();
    if (logPointerEvents) console.log("mousemove");
  },
  true
);
window.addEventListener(
...
window.addEventListener("blur", function () {
  ...
  if (typeof currMathsInput !== "undefined") endMathsInput();
});
  ...
  if (loadedScriptCount >= scriptsToLoad.length) {
    ...
    if (isTask == true) {
      ...
      if (Number(taskLogData.status) < 3) {
        TimeMe.callAfterTimeElapsedInSeconds(
...
if (isTask == true) {
  ...
  function logTime() {
    ...
    if (
      Number(taskLogData.percentage) == 100 ||
      Number(taskLogData.status) == 3
    ) {
      clearReportTimeouts();
      return;
    }
  ...
  function createHolder() {
    var prev = playButton(20, 20, 50, prevPage, {
      dir: "left",
      fillColor: "#3FF",
      lineWidth: 4,
      radius: 8,
    ...
    var next = playButton(1200 - 20 - 50, 20, 50, nextPage, {
      dir: "right",
      fillColor: "#3FF",
      lineWidth: 4,
      radius: 8,
    ...
    showObj(canvas);
    var check = newctx({
      rect: [1200 - 340, 20, 200, 50],
      pe: true,
      z: 10000000,
    }).canvas;
    text({
      ctx: check.ctx,
      textArray: ["<<fontSize:32>><<font:Hobo>>Check Answer"],
      left: 2,
      top: 2,
      width: 196,
      height: 46,
    ...
    text({
      ctx: home2.ctx,
      rect: [2, 2, 382, 76],
      text: ["<<font:Hobo>><<fontSize:32>>Return to My Profile    "],
      align: "right",
      vertAlign: "middle",
      box: {
        type: "loose",
    ...
    return {
      ...
      feedbackButton: [],
      completed: [],
      disablePeCanvas: disablePeCanvas,
      summary: summary,
      summary2: summary2,
      home2: home2,
    };
    ...
    if (un(page[pageIndex])) {
      ...
      loadPage(
        ...
        function () {
          ...
          Notifier.error(
            "Error connecting to the server. The page cannot be loaded."
          );
        }
      );
    } else {
      ...
      if (boolean(window.logPages, false) == false) {
        ...
        if (pages[pageIndex].completed == false && un(pages[pageIndex].prev)) {
          ...
          if (pointsMode == true)
            holder.canvas.ctx.rect2({
              rect: [650 + 2, 20 + 2, 200 - 4, 50 - 4],
              lineWidth: 4,
              color: "#000",
              radius: 8,
            });
          ...
          text({
            ...
            vertAlign: "middle",
            textArray: [
              "<<fontSize:36>><<font:Hobo>><<color:#090>>Page Completed!",
            ],
          });
        ...
        textBox(
          holder.canvas.ctx,
          [80, 20, 100, 50],
          ["<<fontSize:28>><<font:Hobo>>" + String(filename)],
          "#CCF"
        );
        if (typeof preview !== "undefined" && !un(preview.button))
          showObj(preview.button);
      }
      showObj(holder.prev);
      showObj(holder.next);
  ...
  function loadPage(callback, errorCallback) {
    ...
    if (!un(pages[pageIndex].prev)) {
      ...
      holder.completed[pageIndex] = textCanvas(
        [600 - 450 / 2, 285, 450, 170],
        [""],
        "#EEE"
      ).canvas;
      resizeCanvas3(holder.completed[pageIndex]);
      var date = new Date(
        pages[pageIndex].prev.slice(0, pages[pageIndex].prev.indexOf(" "))
      );
      pages[pageIndex].prevDate =
      ...
      if (pointsMode == true) {
        text({
          ctx: holder.completed[pageIndex].ctx,
          left: 0,
          top: 16,
          width: 450,
          height: 100,
          textArray: [
            "<<fontSize:50>><<font:Hobo>>Page Completed!<<fontSize:36>>" +
              br +
              "<<font:Arial>><<fontSize:20>>Completed on " +
              pages[pageIndex].prevDate +
              br +
              "Click to practise again for an extra point",
          ],
          align: "center",
        });
      } else {
        ...
        text({
          ...
          height: 30,
          textArray: [
            "<<fontSize:26>><<font:Arial>>" + times + "<<fontSize:14>>Dismiss",
          ],
          align: "center",
          vertAlign: "middle",
        });
    ...
    if (pointsMode == true) {
      for (var s = 0; s < pages[pageIndex].maxPoints; s++) {
        var star = newctx({
          rect: [l + s * 35 - 17.5, 45 - 17.5, 35, 35],
          z: 100000000,
        }).canvas;
        star.color = ["#FC3", "#000"];
        star.draw = function () {
        ...
        star.click = function () {
          do {
            var color = ["#FC3", "#F00", "#33F", "#3FF", "#0C0", "#CCC"][
              ran(0, 5)
            ];
    ...
    var feedbackButton = textCanvas(
      [1130, 90, 50, 50],
      ["<<font:Hobo>><<fontSize:30>>!"],
      "#FC9"
    ).canvas;
    ...
    for (var p2 = 0; p2 < paths.length; p2++) {
      ...
      for (var o = 0; o < path.obj.length; o++) {
        ...
        if (obj.type == "text2" && path.isInput.type == "text") {
          ...
          var input = createMathsInput2({
            left: obj.rect[0],
            top: obj.rect[1],
            width: obj.rect[2],
            height: obj.rect[3],
            fontSize: 32,
            algText: boolean(path.isInput.algText, false),
          });
          input.pathObj = obj;
          input.type = "text";
          if (!un(path.isInput.tickStyle))
            input.tickStyle = path.isInput.tickStyle;
          p.inputs.push(input);
          ...
          var input = {
            ...
            checkSelectCount: checkSelectCount,
            shuffle: shuffle,
          };
          p.inputs.push(input);
          ...
          for (var r = 0; r < obj.cells.length; r++) {
            ...
            for (var c = 0; c < obj.cells[r].length; c++) {
              var button = newctx({
                rect: [xPos, yPos, obj.widths[c], obj.heights[r]],
                pE: true,
                z: 100,
              }).canvas;
              button.input = input;
          ...
          if (dragCheckMode == "dragCanvas") {
            ...
            p.inputs.push({
              type: "drag",
              canvas: dragCanvas,
              shuffle: path.isInput.shuffle,
              value: path.isInput.value,
            });
          }
  ...
  function checkPage() {
    ...
    if (c.check == false) {
      ...
      xmlHttp.errorCallback = function () {
        Notifier.error(
          "Error connecting to the server. Please try again shortly..."
        );
        hideObj(holder.disablePeCanvas);
        delete holder.checkPageIndex;
        setTimeout(function () {
      ...
      xmlHttp.onreadystatechange = function () {
        ...
        if (!un(document.origin) && document.origin.indexOf("localhost") > -1)
          console.log(this.responseText);
        var response = JSON.parse(this.responseText);
        if (!un(document.origin) && document.origin.indexOf("localhost") > -1)
          console.log("check php response:", response);
        pages[holder.checkPageIndex].pageLogKey = response.pageLogKey;
        if (un(pages[holder.checkPageIndex].checkCount))
          pages[holder.checkPageIndex].checkCount = 0;
        ...
        if (response.correct == 1) {
          ...
          if (
            boolean(
              page[holder.checkPageIndex].allowInteractionAfterCompletion,
              false
            ) == false
          ) {
            ...
            for (var m = 0; m < mathsInput[holder.checkPageIndex].length; m++) {
              mathsInput[holder.checkPageIndex][
                m
          ...
          if (pointsMode == true) {
              ...
              if (star.data[100] <= 613 - 17.5) {
                ...
                if (window.count == pages[holder.checkPageIndex].points) {
                  ...
                  setTimeout(function () {
                    for (
                      s = 0;
                      s < pages[holder.checkPageIndex].maxPoints;
                      s++
                    ) {
                      ...
                      if (s < pages[holder.checkPageIndex].points) {
                        ...
                        star.data[100] =
                          600 -
                          (pages[holder.checkPageIndex].points - 1) * 17.5 +
                          s * 35 -
                          17.5;
                        star.data[101] = 360;
                        resizeCanvas3(star);
                    ...
                    holder.completed[holder.checkPageIndex] = textCanvas(
                      [600 - 450 / 2, 285, 450, 130],
                      [""],
                      ["#F6F", "#6FF", "#FF6", "#6F6", "#66F", "#F66"].ran()
                    ).canvas;
                    showObj(holder.completed[holder.checkPageIndex]);
                    text({
                      ctx: holder.completed[holder.checkPageIndex].ctx,
                      left: 0,
                      top: 12,
                      width: 450,
                      height: 100,
                      textArray: [
                        "<<fontSize:50>><<font:Hobo>>Page Completed!",
                      ],
                      align: "center",
                    });
                    holder.completed[
                      holder.checkPageIndex
                    ].style.pointerEvents = "auto";
                    holder.completed[
                      holder.checkPageIndex
                    ].style.zIndex = 10000000;
                    addListener(
                      holder.completed[holder.checkPageIndex],
                      function () {
                        hideObj(holder.completed[holder.checkPageIndex]);
                        for (
                          var s = 0;
                          s < pages[holder.checkPageIndex].stars.length;
                          s++
                        ) {
            ...
            text({
              ...
              width: 450,
              height: 100,
              textArray: ["<<fontSize:50>><<font:Hobo>>Page Completed!"],
              align: "center",
            });
            ...
            holder.completed[holder.checkPageIndex].style.pointerEvents =
              "auto";
          ...
          if (holder.checkPageIndex === pageIndex) {
            setTimeout(function () {
              if (
                boolean(page[pageIndex].showHolderCheckButton, true) == true
              ) {
                showObj(holder.check);
              } else {
                hideObj(holder.check);
            ...
            if (pointsMode == true && pages[holder.checkPageIndex].points > 1) {
              ...
              var star =
                pages[holder.checkPageIndex].stars[
                  pages[holder.checkPageIndex].points
                ];
              star.color = ["#CFF", "#999"];
              star.draw();
      ...
      if (!un(pages[holder.checkPageIndex].pageLogKey))
        c.pageLogKey = pages[holder.checkPageIndex].pageLogKey;
      c.userKey = userKey;
      c.userType = userType;
  ...
  function drawPoints() {
    ...
    text({
      ...
      height: 46,
      vertAlign: "middle",
      textArray: ["<<fontSize:24>>" + userName],
      box: {
        type: "loose",
        radius: 8,
        color: "#CFF",
    ...
    if (pointsMode == true) {
      text({
        ...
        vertAlign: "middle",
        textArray: [
          "<<fontSize:24>><<color:#00F>><<bold:true>>" + userPoints + "       ",
        ],
        allowSpaces: true,
      });
  ...
  function showTaskCompleteMessage() {
    ...
    taskCompleteMessage.interval = setInterval(function () {
      taskCompleteMessage.rotate();
    }, 25);

    taskCompleteMessage.rotate = function () {
      for (i = 1; i < taskCompleteMessage.stars.length; i++) {
        taskCompleteMessage.stars[i].ctx.clear();
        taskCompleteMessage.stars[i].ctx.translate(
          taskCompleteMessage.stars[i].data[2] / 2,
          taskCompleteMessage.stars[i].data[3] / 2
        );
      ...
      taskCompleteMessage.stars[1].ctx.drawImage(
        taskCompleteMessage.starYellow,
        20,
        20,
        86,
        86
      );
      taskCompleteMessage.stars[2].ctx.drawImage(
        taskCompleteMessage.starYellow,
        20,
        20,
        86,
      ...
      taskCompleteMessage.stars[3].ctx.drawImage(
        taskCompleteMessage.starYellow,
        60,
        60,
        172,
      ...
      taskCompleteMessage.stars[4].ctx.drawImage(
        taskCompleteMessage.starWhite6points,
        24,
        33,
        141,
        123
      );
      taskCompleteMessage.stars[5].ctx.drawImage(
        taskCompleteMessage.starWhite6points,
        24,
        33,
        141,
      ...
      taskCompleteMessage.stars[6].ctx.drawImage(
        taskCompleteMessage.starYellowPointy,
        20,
        21,
        86,
        84
      );
      taskCompleteMessage.stars[7].ctx.drawImage(
        taskCompleteMessage.starYellowPointy,
        20,
        21,
        86,
      ...
      taskCompleteMessage.stars[8].ctx.drawImage(
        taskCompleteMessage.starYellowPointy,
        20,
        21,
        86,
      ...
      taskCompleteMessage.stars[9].ctx.drawImage(
        taskCompleteMessage.starYellowPointy,
        40,
        42,
        152,
...
function yWindowToCanvas(yCoord) {
  return (
    mainCanvasHeight * ((yCoord - canvasMetrics.top) / canvasDisplayHeight)
  );
}
...
function resize() {
  var totalWidth =
    mainCanvasWidth + mainCanvasMargins[0] + mainCanvasMargins[2];
  var totalHeight =
    mainCanvasHeight + mainCanvasMargins[1] + mainCanvasMargins[3];
  ...
  canvasDisplayLeft =
    (window.innerWidth - totalDisplayWidth) / 2 +
    mainCanvasMargins[0] * (totalDisplayWidth / totalWidth);
  canvasDisplayTop =
    (window.innerHeight - totalDisplayHeight) / 2 +
    mainCanvasMargins[1] * (totalDisplayHeight / totalHeight);
  ...
  if (boolean(isTask, true) == true) {
    ...
    for (var i = 0; i < holder.feedbackButton.length; i++) {
      if (!un(holder.feedbackButton[i]))
        resizeCanvas3(holder.feedbackButton[i]);
    }
...
function clearCanvas() {
  ...
  ctx.lineTo(
    mainCanvasWidth - mainCanvasBorderWidth / 2,
    mainCanvasBorderWidth / 2
  );
  ctx.lineTo(
    mainCanvasWidth - mainCanvasBorderWidth / 2,
    mainCanvasHeight - mainCanvasBorderWidth / 2
  );
  ctx.lineTo(
    mainCanvasBorderWidth / 2,
    mainCanvasHeight - mainCanvasBorderWidth / 2
  ...
  if (isTask == true) {
    ctx.fillStyle = "#CFF";
    ctx.fillRect(
      mainCanvasBorderWidth,
      mainCanvasBorderWidth,
      1200 - 2 * mainCanvasBorderWidth,
      80 - mainCanvasBorderWidth
    );
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(0, 80);
...
function newctx(obj) {
  ...
  if (!un(canvases[pageIndex]) && boolean(obj.page, true) == true)
    canvases[pageIndex].push(cnv);
  var ctx = cnv.ctx;
  ctx.canvas = cnv;
  ctx.data = cnv.data;
  resizeCanvas3(cnv);
...
function arrayCheck(array, element) {
  for (var i = 0; i < array.length; i++) {
    ...
    if (typeof array[i] == "object" && arraysEqual(array[i], element) == true)
      return true;
  }
  return false;
}
...
function dragStart(e) {
  ...
  if (!un(page) && typeof page[pageIndex].clear == "function")
    page[pageIndex].clear();
}
function dragMove(e) {
  e.preventDefault();
  ...
  if (!un(canvas.dragPath)) {
    var path = canvas.dragPath;
    var pos = [
      canvas.data[100] + 0.5 * canvas.data[102],
      canvas.data[101] + 0.5 * canvas.data[103],
    ];
    ...
    for (d = 0; d < draw._dragAreas.length; d++) {
      ...
      if (dist(pos[0], pos[1], area.center[0], area.center[1]) < 50) {
        ...
        if (area.snap == true) {
          dragObj.data[100] =
            area.rect[0] + 0.5 * area.rect[2] - 0.5 * canvas.data[102];
          dragObj.data[101] =
            area.rect[1] + 0.5 * area.rect[3] - 0.5 * canvas.data[103];
          resizeCanvas3(dragObj);
        }
